...
\;\ge\;2^{-86}\;\gg\;2^{-128}
\]
for every $\delta\in[1-\rho-2^{-9},1-\rho)$ at rates $1/2,1/4,1/8$, and for every $\delta\in[1-\rho-2^{-10},1-\rho)$ at rate $1/16$. The same lower bound holds for $\eca(C,\delta)$ on the integer-admissible subinterval ending at $1-\rho-1/n$. If $|\F|\ge2n$, the displayed lower bound improves to $2^{-42}$. Consequently $\delta^*_C(2^{-128})\le 1-\rho-2^{-9}$ for $\rho\in\{1/2,1/4,1/8\}$ and $\delta^*_C(2^{-128})\le 1-\rho-2^{-10}$ for $\rho=1/16$, throughout the $|\F|<2^{256}$ challenge envelope.
...
$q:=|\F|$ and $n:=|D|$.  Let $\delta\in(0,1)$ and set
...
\[
        a:=n-f
\]
points of $D$.  The hypothesis $f<n-k-1$ gives $a>k+1$, in particular
$a>k$.
...
\[
        ...
        \le k\binom L2 .
\]
Averaging this collision count and using Cauchy--Schwarz gives the standard
coarse bound
\[
        M(\alpha)\ge \frac{L(q-n)}{q-n+kL}
\]
...
The deep-point conversion is an integer-radius theorem with the condition $\lfloor\delta n\rfloor<n-k-1$. Therefore the no-loss CA lower bound is stated only for
\[
        \delta < 1-\rho-\frac1n .
\]
