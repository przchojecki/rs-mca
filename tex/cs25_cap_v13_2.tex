...
\begin{remark}[threshold scope of the identically-split residual of slope
...
\cref{prop:capfr1-slope-elimination}.  Part~\textup{(b)} of
\cref{thm:capfp-slope-elim} only translates the line to a sparse pair; it
does not delete support-wise bad slopes.  If the threshold $a'$ under
consideration also satisfies $n-a'\le t$, then
\cref{thm:capg-tangentborne} applies to the constant section
$W(Z,X)=\Lambda_A(X)$ and gives at most
$\max(\deg\Lambda_A,1)$ MCA-bad finite slopes; this includes the
deficiency-one use above.  No such payment follows at arbitrary deficiency.
More generally, \cref{thm:capg-tangentborne} proves that a rational
identically split kernel section is constant in $Z$, but its slope payment
has the same threshold guard $n-a'\le t$.  Outside that guard the result is
a structural route only: the fixed and family identically split branches
remain in the support-wise rank-one/spread residual and are not banked here.
The ``known unconditional strata'' list of \cref{prob:capfp-R1} may still
cite deficiency one without the residual caveat because
\cref{cor:capg-spib} makes those charts eliminant-or-tangent outright.
\end{remark}
...
Let $(u,v)$ be an affine line at agreement $m$, write $w=m-K\ge1$, and assume
\[
        3w\le n-K.
\]
Then the number of MCA-bad finite slopes with $d_1(u+zv)\le w$ and nonzero census is at most $2w$.  Consequently,
\[
        N_{\mathrm{MCA\text{-}bad}}(u,v;m)
        \le
        2w+\#\{z:\ d_1(u+zv)\ge w+1,\ \Cen(u+zv;m)>0\}.
\]
The condition holds at all four deployed adjacent rows.
\end{corollary}

\begin{proof}
Let $L$ be the set of MCA-bad slopes in the near-rational stratum.  If
$|L|\le1$ the assertion is immediate.  Otherwise choose distinct
$z_0,z_1\in L$.  By \cref{thm:capfr1-near-rational-dichotomy}, for every
$z\in L$ there are a unique codeword $c_z$ and an error word $\eta_z$ with
\[
        u+zv=c_z+\eta_z,
        \qquad \wt(\eta_z)\le w.
\]
...
\[
 ...
 c_u=c_{z_0}-z_0c_v,\qquad
 e_v=v-c_v,\qquad e_u=u-c_u.
\]
Then $e_u+z_ie_v=\eta_{z_i}$; explicitly
\[
 e_v=\frac{\eta_{z_1}-\eta_{z_0}}{z_1-z_0},\qquad
 e_u=\eta_{z_0}-z_0e_v,
\]
so $E:=\operatorname{supp}(e_u)\cup\operatorname{supp}(e_v)$ has size at most $2w$.  For every
$z\in L$, the codeword
\[
 c_z-(c_u+zc_v)=e_u+ze_v-\eta_z
\]
is supported on at most $3w$ coordinates.  The hypothesis says that this
support is smaller than $n-K+1$, the minimum distance of the code;
hence this codeword is zero and $\eta_z=e_u+ze_v$.

Choose a bad witness $(S,h)$ for $z$ with $|S|\ge m$.  On
$S\setminus\operatorname{supp}(\eta_z)$ the polynomial $h-c_z$ has at least
$m-w=K$ roots, so $h=c_z$.  Thus $\eta_z$ vanishes on $S$.  Pair
noncontainment on this same support supplies an $x\in S$ with
$(e_u(x),e_v(x))\ne(0,0)$.  Since
$e_u(x)+ze_v(x)=0$, necessarily $e_v(x)\ne0$ and
\[
        z=-e_u(x)/e_v(x).
\]
Distinct slopes give distinct coordinate ratios, all on $E$.  Therefore
$|L|\le|E|\le2w$.  Every remaining bad slope has nonzero census and belongs
to the displayed balanced stratum.
\end{proof}
...
Fix a line $(u,v)$ over the challenge field, an agreement $a=m>n/2$, and $w=m-K\ge1$.  Call $T$ \emph{rank-one} if the vectors $\alpha(T):=(S_r(u,T))_{r<w}$ and $\beta(T):=(S_r(v,T))_{r<w}$ are proportional and not both zero, with $\beta(T)\ne0$; each rank-one support determines the single slope $z(T)$ with $\alpha(T)=-z(T)\beta(T)$.  Then:
\begin{enumerate}[label=\textup{(\alph*)}]
...
\item if some $m$-support is common, subtraction of its explaining codeword pair produces a pair supported on at most $n-m$ coordinates and preserves the MCA-bad slope set exactly;
\item in every case,
\[
...
\textup{(b)}  A common support gives polynomials $c_u,c_v$ of degree $<K$ with $u|_T=c_u|_T$, $v|_T=c_v|_T$.  Put $u'=u-c_u$ and $v'=v-c_v$.  Then $u',v'$ vanish on $T$, so their union support has size at most $n-m$.  For every slope $z$, support $S$, and codeword $c$, the relation $(u+zv)|_S=c|_S$ is equivalent to $(u'+zv')|_S=(c-c_u-zc_v)|_S$, and the same translation equivalence holds for simultaneous explanations of the pair.  Thus the MCA-bad slope sets agree exactly.
\textup{(c)}  Let $S$ be a bad witness support, initially of size at least
$m$.  It contains a non-common $m$-subset.  Indeed, otherwise choose one
$m$-subset $T_0\subseteq S$ and its common explaining pair.  For each
$x\in S\setminus T_0$, the set consisting of $x$ and any $m-1$ points of
$T_0$ would also be common; its explaining pair agrees with the pair on
$T_0$ at $m-1\ge K$ points and hence is the same polynomial pair.  That pair
would explain all of $S$, contradicting the witness.  Thus every MCA-bad
slope has a non-common exact $m$-support.  By \textup{(a)} it has
$\beta\ne0$ and determines only that slope.  Choosing one canonical
non-common witness support for every bad slope gives an injection into
$\mathrm{R1}(u,v;m)$, whether or not a different common support exists.
...
The rank-one census, and with it input \textup{(A)}, now admits a second unconditional reduction: the twisted census of \emph{every} word is the split locus of an explicit rank-two $\F[X]$-lattice and its near-rational stratum resolves exactly.  The support-wise near-rational payment is instead \cref{cor:capfp-line}: under the stronger guard $w\ge1$ and $3w\le n-K$, it costs at most $2w$ distinct bad slopes per line.  Throughout, $\omega:=n-m$, $\widehat U$ denotes the degree-$\le n-1$ interpolant of the word $U$ on $D$, and we assume the band condition $2w\le n-K$.
...
Let $(u,v)$ be a line at agreement $m$, write $w=m-K\ge1$, and assume
$3w\le n-K$.  The number of MCA-bad finite slopes with
$d_1(u+zv)\le w$ and nonzero census is at most $2w$.  Hence
\[
        N_{\mathrm{MCA\text{-}bad}}(u,v;m)
        \le 2w+\#\{z:\ d_1(u+zv)\ge w+1,\ \mathrm{cen}(u+zv;m)>0\}.
\]
...
\begin{proof}
The proof of \cref{cor:capfr1-balanced-line} uses only the near-rational
dichotomy, the Reed--Solomon minimum distance, and the support-wise definition,
so it applies verbatim with \cref{thm:capfp-dichotomy}(i).  It gives at most
$2w$ near-rational bad slopes.  Slopes with zero census are not close at
agreement $m$, and every other bad slope lies in the displayed balanced
stratum.
\end{proof}
