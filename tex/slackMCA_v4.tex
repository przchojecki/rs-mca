...
\newcommand{\ImgFib}{\mathcal F^{\rm img}}
...
This strengthened version also promotes the stable high-agreement tangent material from the experimental notes into the main theory: the affine, no-loss CA, and projective-line numerators are exactly $r+1$ in the range $r\le (n-k)/3$, interleaved lists are unique for $r\le (n-k)/2$, and degree-$d$ finite power-curve ledgers have a theorem-backed $d(r+1)$ upper envelope with exactness under a split moving-root hypothesis.  Thus the high-agreement boundary layer is now solved, and the remaining frontier is explicitly the near-capacity quotient-profile and aperiodic local-limit problem.
\end{abstract}
...
\]
Define the associated codeword-image fiber
\[
...
\begin{proposition}[Image fibers are exact lists]\label{prop:imgfib-list}
For every received word $y$ with interpolant $U$,
\[
        \ImgFib_U(s)
        =
        \{P\in\F_q[X]_{<k}:\ |\{x\in H:U(x)=P(x)\}|\ge s\}.
\]
Consequently
\[
...
\[
        |\Fib_U(s)|
        =
        \sum_{P\in\ImgFib_U(s)}\binom{a_P}{s}.
\]
\end{proposition}

\begin{proof}
If $S\in\Fib_U(s)$, then $P_S:=U\bmod L_S$ has degree $<k$ and agrees with $U$
on $S$, so $P_S$ belongs to the displayed list.  Conversely, if
$P\in\F_q[X]_{<k}$ agrees with $U$ on a set $A$ of size at least $s$, then for
every $s$-subset $S\subseteq A$ we have $S\in\Fib_U(s)$ and
$U\bmod L_S=P$.  Thus the image fiber is exactly the list.  The final identity
counts, for each listed codeword $P$, the number of $s$-subsets of its agreement
set with $U$.
\end{proof}
...
\[
        \max_{\deg U<n}|\ImgFib_U(k+\sigma)|\quad\text{should be polynomial above the corrected reserve.}
\]
...
\]
except for quotient-periodic and low-denominator structured families.  The raw
support fiber $\Fib_U(k+\sigma)$ can be exponentially larger than the list when
many supports explain the same codeword; the image fiber is the object matched
exactly by \cref{prop:imgfib-list}.
\end{conjecture}
...
\begin{proof}
The arbitrary-word claim follows from \cref{prop:imgfib-list}, which identifies
the codeword list with $\ImgFib_U(k+\sigma)$ exactly, combined with
\cref{conj:arbitrary-local}.  The monomial-prefix claim is
\Cref{prop:monomial-fiber}.
\end{proof}
...
\section{Solved high-agreement boundary layer}\label{sec:solved-high-agreement-boundary}

\textbf{Status: proved finite Reed--Solomon/MDS boundary theorem.}
The quotient and entropy mechanisms in the preceding sections control the near-capacity
side of the problem.  There is also a complementary boundary layer, at very high
agreement and small Hamming radius, where the staircase is exactly computable and no
smoothness or quotient structure is involved.  This section promotes the stable
high-agreement material from the experimental notes into the main slack theory.
...
\[
        ...
        \qquad R=n-k,
\]
so that \(r\) is the integer Hamming radius and \(a\) is the agreement threshold.
For a line ledger, let \(\operatorname{LD}_{\rm sw}(C,a)\) be the largest number of
finite slopes on a received affine line that are support-wise noncontained at
agreement at least \(a\).  Let \(\operatorname{LD}_{\rm ca}(C,a)\) be the analogous
no-loss correlated-agreement numerator, and let
\(\operatorname{LD}_{\rm sw,proj}(C,a)\) be the projective-slope version where the
parameter space is \(\mathbb P^1(F)\).

\begin{theorem}[high-agreement tangent line staircase]\label{thm:B-high-agreement-line-staircase}
...
\begin{proof}
For the lower bound, choose a set \(A\subset D\) of size \(a-1\) and let
\(T=D\setminus A\), so \(|T|=r+1\).  The displayed range implies
\(2a-n-1\ge k\): indeed, with \(R=n-k\) and \(r=n-a\), the hypothesis is
\(r\le \lfloor R/3\rfloor\), hence \(2a-n-1-k=R-2r-1\ge0\).  Choose distinct finite parameters
\(\theta(t)\in F\), \(t\in T\), and define words
\[
        f|_A=g|_A=0,
        \qquad f(t)=-\theta(t),\quad g(t)=1\quad(t\in T).
\]
For \(z=\theta(t)\), the word \(f+zg\) is zero on \(A\cup\{t\}\), hence is
explained by the zero codeword on \(a\) positions.  It is support-wise
noncontained: if \(g\) were explained on \(A\cup\{t\}\) by a degree-\(<k\)
polynomial, it would vanish on \(|A|=a-1\ge k\) points and hence be zero, but
\(g(t)=1\).  In the displayed range the same example is globally far from
\(C^2\) at agreement \(a\), so the same slopes are no-loss CA-bad.

For the upper bound, take two bad affine slopes with witnesses on supports
\(S_1,S_2\).  Their intersection has size at least \(2a-n\ge k\), so the two
explaining codewords recover a common code-line \((c_f,c_g)\) on that
intersection by solving a two-by-two linear system in the slopes.  Let \(S_0\)
be the maximal support on which \(f=c_f\) and \(g=c_g\).  Since
\(|S_0|\ge2a-n\), every further bad slope agrees with its recovered codeword on
at least
\[
        a+|S_0|-n\ge 3a-2n\ge k
\]
points of \(S_0\), and therefore must be explained by the same common code line.
The remaining agreement must come from residual zeros outside \(S_0\).  No
coordinate outside \(S_0\) can be a residual zero for two different finite
slopes, and each noncontained witness needs at least
\(\max\{1,a-|S_0|\}\) such zeros.  The residual charging bound gives at most
\(r+1\) bad affine slopes.

For projective slopes, if more than \(r+1\) projective points were bad, either
choose a nonbad point as infinity and reduce all bad points to finite affine
slopes, or, if every projective point is bad, choose any point as infinity.  The
remaining finite bad slopes would contradict the affine upper bound.  Thus the
projective numerator is also \(r+1\).  Since CA-bad slopes are support-wise
MCA-bad and the lower construction is CA-bad in the displayed range, the CA
numerator is exactly \(r+1\) as well.
\end{proof}

\begin{theorem}[interleaved uniqueness in the high-agreement layer]\label{thm:B-interleaved-unique-high-agreement}
Let \(C\subseteq F^D\) be an MDS linear code of dimension \(k\) and length
\(n\).  For every interleaving arity \(\mu\ge1\), if \(a=n-r\) and
\[
        2a-n\ge k,
        \qquad\text{equivalently}\qquad
        r\le \frac{n-k}{2},
\]
then every \(\mu\)-row received word has at most one interleaved codeword tuple
agreeing on \(a\) common positions.  Thus the maximum interleaved list numerator
is exactly \(1\).
\end{theorem}

\begin{proof}
If two distinct \(\mu\)-row codeword tuples agree with the same received word on
supports of size at least \(a\), then their common supports intersect in at least
\(2a-n\ge k\) positions.  On that intersection every row of the two tuples is
equal.  Since each row belongs to an MDS code of dimension \(k\), equality on
\(k\) positions forces equality of that row everywhere.  Hence the tuples are
equal.  A received word that is itself a codeword tuple gives one element, so the
maximum is exactly \(1\).
\end{proof}

For finite-parameter curves the high-agreement upper bound is just as clean, but
one must state the lower bound with the correct splitting hypothesis.  This
avoids confusing a theorem about finite power-curve models with arbitrary
protocol curve samplers.

\begin{definition}[split moving-root set]\label{def:B-split-moving-root}
Let \(R\subseteq D\) have size \(r+1\).  A polynomial \(\phi\in F[T]\) of degree
at most \(d\) is \emph{\(d\)-split over \(R\)} if
\[
        \#\{\gamma\in F:\phi(\gamma)\in R\}=d(r+1).
\]
\end{definition}

\begin{theorem}[degree-\(d\) curve envelope and split exactness]\label{thm:B-curve-envelope}
Let \(C=\RS[F,D,k]\), let \(a=n-r\), and fix \(d\ge1\).  In the range
\[
        (d+2)a-(d+1)n\ge k,
        \qquad\text{equivalently}\qquad
        r\le \frac{n-k}{d+2},
\]
every finite power curve
\[
        W_\gamma=f_0+\gamma f_1+\cdots+\gamma^d f_d,
        \qquad \gamma\in F,
\]
has at most \(d(r+1)\) support-wise curve-MCA-bad parameters at agreement
\(a\); the same upper bound applies to no-loss curve-CA.  Conversely, if there
is a set \(R\subseteq D\) of size \(r+1\) and a degree-\(\le d\) polynomial
\(\phi\) that is \(d\)-split over \(R\), then the support-wise upper bound is attained:
\[
        \operatorname{CurveLD}^{(d)}_{\rm sw}(C,a)=d(r+1).
\]
Moreover, since \(r\le \lfloor(n-k)/(d+2)\rfloor\le \lfloor(n-k)/3\rfloor\), the same construction is no-loss curve-CA-bad, so
\[
        \operatorname{CurveLD}^{(d)}_{\rm ca}(C,a)=d(r+1).
\]
Without the splitting hypothesis, the unconditional high-agreement statement is
the envelope
\[
        r+1\le \operatorname{CurveLD}^{(d)}_{\rm sw}(C,a)\le d(r+1).
\]
The lower bound in this envelope is the ordinary moving-root line witness,
viewed as a degree-\(d\) power curve by setting the higher curve coefficients to
zero.
\end{theorem}

\begin{proof}
The upper bound is a residual-root count.  Choose \(d+1\) bad parameters and
intersect their supports.  The intersection has size at least
\[
        (d+1)a-dn.
\]
Interpolating the \(d+1\) explaining codewords in the parameter \(\gamma\)
recovers a common code-valued curve on that intersection.  For any further bad
parameter, the witness intersects the common support in at least
\[
        a+((d+1)a-dn)-n=(d+2)a-(d+1)n\ge k
\]
positions, so MDS rigidity forces the explaining codeword to be the value of the
same recovered code curve.  The remaining agreement must therefore come from
residual coordinate polynomials in \(\gamma\) of degree at most \(d\).  No
residual coordinate carries the zero polynomial after maximalizing the common
support, so each coordinate contributes at most \(d\) parameter roots.  The same
residual budget as in the line case gives at most \(d(r+1)\) bad parameters.

For the lower bound under the split hypothesis, choose
\(A=D\setminus R\) and put \(B_A(X)=\prod_{x\in A}(X-x)\).  Define
\[
        W_\gamma=\bigl((X-\phi(\gamma))B_A(X)\bigr)_{\ge k}.
\]
Expanding \(\phi(\gamma)\) gives a power curve of degree at most \(d\).  Whenever
\(\phi(\gamma)=t\in R\), the word \(W_\gamma\) is explained on
\(A\cup\{t\}\), and the tangent-star noncontainment argument applies.  In the
same high-agreement range the coefficient tuple is globally far from
\(C^{d+1}\), yielding no-loss curve-CA badness as well.
\end{proof}

\begin{corollary}[multiplicative subgroup split criterion]\label{cor:B-mult-split}
Let \(D=H\subset F^\times\) be a multiplicative subgroup of order \(n\).  Suppose
\(\operatorname{char}F\nmid d\), \(d\mid(|F|-1)\), and
\[
        r+1\le \frac{n}{\gcd(d,n)}.
\]
Then \(\phi(\gamma)=\gamma^d\) is \(d\)-split over a suitable residual set
\(R\subset H\), and the exact degree-\(d\) curve formula of
\Cref{thm:B-curve-envelope} holds in the displayed high-agreement range.
\end{corollary}

\begin{proof}
The image \(H^d\) has size \(n/\gcd(d,n)\).  Choose \(R\subseteq H^d\) of size
\(r+1\).  Since \(d\mid(|F|-1)\) and \(\operatorname{char}F\nmid d\), every
nonzero element of \(R\) has exactly \(d\) preimages under \(\gamma\mapsto
\gamma^d\).
\end{proof}

\begin{corollary}[finite-grid tangent compiler]\label{cor:B-tangent-compiler}
Fix a target \(2^{-\lambda}\) and a sampler denominator \(Q\).  Define
\[
        B(Q,\lambda)=\left\lfloor \frac{Q}{2^\lambda}\right\rfloor .
\]
Inside the exact line range \(r\le\lfloor(n-k)/3\rfloor\), a single line, CA, or
projective-line term is safe at target \(2^{-\lambda}\) exactly when
\[
        r+1\le B(Q,\lambda).
\]
If an interleaved-list term over the same denominator is also consumed, the
exact high-agreement ledger is safe exactly when
\[
        r+2\le B(Q,\lambda).
\]
For a degree-\(d\) curve term in its split-exact range, the corresponding
conditions are
\[
        d(r+1)\le B(Q,\lambda),
        \qquad
        d(r+1)+1\le B(Q,\lambda)
\]
without and with one same-denominator list term, respectively.  With different
denominators, the numerators must not be merged; one must check the rational
sum term by term.
\end{corollary}

\begin{remark}[effect on the frontier]
Theorems~\ref{thm:B-high-agreement-line-staircase}--\ref{thm:B-curve-envelope}
solve the small-radius/high-agreement boundary layer.  They do not address the
near-capacity region \(a=k+\sigma\), where \(r=n-k-\sigma\) is generally much
larger than \((n-k)/3\).  The remaining positive frontier is therefore unchanged
in substance but sharper in scope: quotient-profile floors must be budgeted, and
after quotient/tangent structures are removed one needs finite-field local-limit
upper bounds for arbitrary locator fibers and all-line residue packing.
\end{remark}
...
The composition above is extracted, sharpened, and made the main theorem of \cite{UniversalCap}: running the pigeonhole on slack-two locator fibers over the field of definition, rather than on quotient cores, improves the gap to $2^{-9}$ ($2^{-10}$ at rate $1/16$) under $2^{10}\mid n$ (resp.\ $2^{11}\mid n$), yields error $\frac1{2k}(1-n/q)\ge2^{-86}$ with no condition relating $n$ and $q$, and covers extension fields $\F\supseteq B\supseteq H$ directly through the subfield pigeonhole.  In its v5 form, that companion cap is self-contained for the main MCA statement.  \Cref{thm:cap} is retained here because the quotient-core mechanism is the one native to this paper's framework and the proof is self-contained given Part~I plus the imported conversion; for citation purposes, the cap of \cite{UniversalCap} supersedes \cref{cor:captiers}.
\end{remark}
...
\Cref{thm:capimport} is used as a black box in the older quotient-core route retained in this section, and three items must be checked against \cite{CS25} before that route is cited as unconditional: (i) the exact admissible range of $\delta$ in their Theorem~2, and the definition of the augmented code $C^+$ together with the monotonicity $\Lst(C,\delta)\le\Lst(C^+,\delta)$; (ii) the normalization of $\eca$ (density over $z$, including extension-field sampling of $z$ in the sense of the proximity-gaps literature); (iii) the constants in the displayed implication.  Should any constant need repair, the slacked conversion of Ben-Sasson--Carmon--Hab\"ock--Kopparty--Saraf \cite{BCHKS25}, recorded as Theorem~5.2 of \cite{ABF26}, yields the same corollary with $2^{-42}$ replaced by a slightly smaller explicit constant and $2^{-11}$ by a slightly smaller gap.  This import audit is no longer a caveat on the main companion cap: Paper D v12 proves the main MCA cap directly and is now the active cap-paper draft.
\end{remark}
...
\[
        ...
        \sigma\log_2q\ge(1+\eps)\log_2\binom n{k+\sigma}.
\]
Equivalently, after quotient-periodic classes are removed, the distinct codeword
images of the map $S\mapsto U\bmod L_S$ behave like a random
codimension-$\sigma$ hash on $(k+\sigma)$-subsets.
\end{conjecture}
