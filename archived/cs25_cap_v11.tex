...
\title{A Universal Field-Size Cap and Discharged Completion Program\\ for Mutual Correlated Agreement on Smooth Reed--Solomon Domains}
...
The mechanism composes a pigeonhole lower bound on quotient-locator list fibers---a slack-two and subfield-pigeonhole sharpening of \cite[Main Thm.\,(d)]{Cho26a}---with a self-contained deep-point list-to-correlated-agreement conversion. This conversion is the same shallow consequence previously imported from Crites--Stewart \cite[Thm.~2]{CS25}, but it is proved here directly with the integer-radius condition. The composition itself was first carried out, conditionally and with weaker constants, in the universal-cap theorem of \cite{Cho26b} (error $2^{-42}$ at gap $2^{-11}$, assuming $|\F|\ge2n$); the present note sharpens the gap, removes the field-size condition, and extends the cap to extension fields. No value-set or equidistribution input is used, so the per-fiber collision problem isolated in \cite{Cho26b} is routed around rather than solved. The proof also adds a quantitative inverse form of the deep-point conversion, heaviest quotient-remainder prefix fibers, an extension-pole conversion producing genuinely extension-valued witness lines, an exact divisor-block coefficient formula for the support-union ledger, a gcd/lcm quotient-image ledger that coalesces duplicate finite, projective, and curve parameters inside declared quotient branches, and a canonical maximal-minor gcd ledger for regular overdetermined Hankel buckets. We instantiate the bound at the KoalaBear-sextic parameters of \cite[\S6.3]{ABF26}, including the interleaved rows of their Tables~2--3, give an independent slacked variant through \cite[Thm.~1.9]{BCHKS25}, and prove a two-radius refinement of the subfield-confinement theorem of \cite{Cho26b}, showing that over extension fields every explicit base-rational locator line is inert while the extension-pole floor gives a concrete source of genuinely $\F$-valued certifying lines whenever a large list is present. The cap is extended beyond multiplicative smoothness: a map-smooth, divisibility-free generalization of the fiber lemma and of the universal-cap theorem; exact Chebyshev fibers on twin-coset $x$-coordinate domains of the circle group over $p\equiv3\pmod4$; a torus uniformization identifying bivariate circle codes over fields containing $i$ with diagonally equivalent Reed--Solomon codes on torus twin cosets; and the resulting universal caps for circle codes and circle-FRI line rounds, instantiated at the deployed Mersenne-$31$/QM31 parameters of Circle STARKs (error $>2^{-23}$ at gap $2^{-7}$ below capacity), together with a mixed-radix corollary that removes the integrality condition $\rho N\in\mathbb Z$. The genus-one (ECFFT) case is reduced to an explicit lacunarity obstruction. Finally, the completion obligations T1--T8 of the prize program are treated one by one: an intrinsic stabilizer partition and an exact quotient descent discharge the structural ledger obligations on both multiplicative and Chebyshev towers; a self-contained deep-regime theorem shows that mutual correlated agreement \emph{holds} with error $(\lfloor\delta n\rfloor+1)/q$ below one third of the minimum distance, for every linear code, resolving the singular Hankel buckets there; an exact restriction-of-scalars lift eliminates the extension-line bucket; an elementary Johnson bound, uniform in the interleaving arity, determines the grand list staircase outside the same band; a sound staircase scanner is specified and run on the deployed rows; and every ECFFT row acquires an unsafe first staircase step. The grand MCA threshold of every deployed row is thereby pinned between $(1-\rho)/3$ and the printed capacity gap --- known up to a factor of three --- and the residue of the entire program is a single aperiodic band conjecture.
\end{abstract}
...
The counting side of the composition is not tied to the multiplicative structure of $D$. \Cref{sec:isogeny} proves the fiber lemma for arbitrary polynomial folding maps with complete uniform fibers (\cref{def:map-smooth,lem:phi-fiber}), removes the divisibility hypothesis $a\mid k$, and instantiates the resulting universal cap (\cref{thm:phi-cap}) on the deployed isogeny-smooth family: twin-coset $x$-coordinate domains of the circle group over $p\equiv3\pmod4$ carry exact Chebyshev fibers at every dyadic scale, and over challenge fields containing $i$ the bivariate circle code is diagonally equivalent to a Reed--Solomon code on a torus twin coset. The Mersenne-$31$/QM31 circle family of \cite{HLP24} thereby acquires the same negative bands as the multiplicative rows --- error $>2^{-23}$ at gap $2^{-7}$ at the deployed sizes, gaps $2^{-9}$--$2^{-11}$ universally (\cref{cor:circle-grand,cor:circle-deployed}) --- settling the circle case of the isogeny-smooth problem, with the elliptic (ECFFT) case reduced to a precise lacunarity obstruction (\cref{rem:ecfft-obstruction}, \cref{prob:isogeny}).

The paper also closes the loop on its completion program. \Cref{sec:discharge} treats the obligations T1--T8 of \cref{sec:prize-program} individually rather than leaving them as a list: the structural and extension-line obligations are discharged exactly (\cref{thm:stabilizer-partition,thm:fiber-descent,thm:weil-lines}); the deep regime is settled unconditionally on the safe side by a new, self-contained theorem (\cref{thm:deep-mca}: below one third of the distance, mutual correlated agreement holds with error $(\lfloor\delta n\rfloor+1)/q$ for every linear code, hence also for circle and ECFFT rows); the grand interleaved-list staircase is determined outside the Johnson-to-capacity band (\cref{thm:johnson-list}, \cref{cor:list-sandwich}); the certificate scanner demanded by T6 is specified, proved sound, and run (\cref{tab:scanner}); and every ECFFT row acquires an unsafe first staircase step (\cref{cor:ecfft-onestep}), the first cap of any kind on genus-one domains. The result is a two-sided sandwich: for every deployed row, $\delta^*_C(2^{-128})$ lies between $(1-\rho)/3$ and the printed capacity gap (\cref{thm:sandwich}). What remains of the program after this treatment is exactly one analytic conjecture (\cref{prob:band}), plus the previously stated open problems.

\paragraph{Roadmap.} \Cref{sec:prelim} fixes definitions, aligned with \cite[Defs.~4.1, 4.3]{ABF26}, proves the direct deep-point conversion used for the main cap, and records the separate BCHKS/ABF slacked fallback. \Cref{sec:fiber} proves the fiber and interleaving lemmas and the conservative quotient-support upper ledger, \cref{sec:main} the main theorem, \cref{sec:grand} the challenge-level and deployed corollaries, and \cref{sec:subfield} subfield confinement. \Cref{sec:isogeny} extends the cap beyond multiplicative smoothness: it proves the map-smooth, divisibility-free fiber lemma and universal cap, the exact Chebyshev-fiber and torus twin-coset geometry of circle domains, and the resulting circle-code caps at the deployed Mersenne-$31$/QM31 parameters. \Cref{sec:prize-program} states exactly what this paper contributes toward the Proximity Prize \cite{ProximityPrize}, gives conditional completion theorems for the MCA and interleaved-list thresholds, and isolates the proof obligations that would turn the present cap into a full threshold determination. \Cref{sec:discharge} then treats each of these obligations in turn: it discharges the structural, extension-line, and deep-regime inputs unconditionally, determines the list staircase in the Johnson regime, specifies and runs the certificate scanner, and reduces what remains to a single aperiodic band conjecture together with the previously stated problems. \Cref{sec:discussion} discusses scope, limits, calibration against the conjectural picture of \cite{Cho26b}, and open problems. Throughout, companion results of \cite{Cho26a,Cho26b} are cited by their printed theorem names rather than numbers, since the numbering of those manuscripts is not yet frozen.
...
\begin{remark}[status of T1 after the lower-ledger upgrade]
...
\paragraph{Residual T2 certificate theorem.}
...
A singular or underdetermined T2 residual certificate consists of:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=2.2em]
...
This does not prove that the hard T2 residual buckets are small.  It does remove
...
\item the elimination ideal in $F[Z]$ contains a nonzero polynomial, and its
squarefree generator gives a finite parameter envelope containing the exact
support-image lcm of that chart;
\item the elimination ideal in $F[Z]$ is zero, equivalently the projection of the
...
supplied in scanner-ready form by
...
split into four certified pieces.  The regular overdetermined nonsingular branch
...
projection bound, or by a named structural obstruction.  T3 extension-line
...
$B$, or an exact zero-dimensional point count.  T5 is upgraded for finite-parameter
curves and projective lines in parallel: quotient branches have exact gcd/lcm
...
denominator $|B|$ printed separately from the line denominator $q$. [\emph{Treated in \cref{sec:discharge-T1}: discharged --- \cref{thm:stabilizer-partition,thm:fiber-descent,cor:periodic-support-count} prove the structural exhaustion at the support level, exactly at the image level for fiber-constant lines, with $q$-free, $|B|$-free upper numerators.}]
...
quotient cores budgeted. [\emph{Treated in \cref{sec:discharge-T4}: \cref{thm:johnson-list} supplies the base bound uniformly in $m$ throughout the Johnson regime; \cref{thm:list-recursion} makes the quotient term exact for fiber-constant words; \cref{prop:list-calibration} proves the band term is necessarily exponential.}]
...
\cref{thm:conditional-list} applies. [\emph{Treated in \cref{sec:discharge-T6}: discharged --- the scanner is specified in \cref{def:scanner}, proved sound in \cref{thm:scanner-sound}, and run on the deployed rows in \cref{tab:scanner}.}]
...
checking and directly strengthen a prize submission. [\emph{Treated in \cref{sec:discharge-T7}: certificate format, decidability, and the dependency chart are supplied (\cref{prop:finite-certificate}, \cref{tab:core}); the machine formalization itself remains outstanding.}]
...
\cref{prob:isogeny}, for which the lacunarity collapse of
\cref{rem:ecfft-obstruction} blocks the present route beyond the first
staircase step. [\emph{Treated in \cref{sec:discharge-T8}: the ledger half is discharged at tower level by the Chebyshev cases of \cref{thm:stabilizer-partition,thm:fiber-descent}; \cref{cor:ecfft-onestep} settles the first-step floor, leaving the macroscopic band of \cref{prob:isogeny}.}]
\end{enumerate}
...
After the treatment of \cref{sec:discharge}, the recommended claim strengthens to a \emph{two-sided} partial result: a universal unsafe band at capacity (\cref{cor:grand,cor:mixed-radix,cor:circle-grand,cor:ecfft-onestep}) together with an unconditional safe region below one third of the distance (\cref{thm:deep-mca}) and a list-side safe region up to the Johnson radius, uniform in the interleaving arity (\cref{thm:johnson-list}); equivalently, $\delta^*_C(2^{-128})$ is determined up to a factor of three for every deployed row (\cref{thm:sandwich}, \cref{tab:scanner}). The submission should continue not to claim the exact threshold: what separates the sandwich from a one-step determination is precisely \cref{prob:band}.


\section{Discharging the completion program: a treatment of obligations T1--T8}\label{sec:discharge}

This section treats the eight obligations of \cref{sec:prize-program} one by one. The outcome, stated once here and justified below, is the following. The structural halves of T1 and T8 are discharged unconditionally: an intrinsic stabilizer notion of quotient-periodicity partitions all witness supports over the divisor (resp.\ dyadic Chebyshev) lattice, and for fiber-constant lines the support-level ledger collapses to an exact quotient-image ledger (\cref{thm:stabilizer-partition,thm:fiber-descent}). T2 is resolved unconditionally in the deep regime by a new, self-contained positive theorem: below one third of the distance, mutual correlated agreement \emph{holds} with error $O(\delta n)/q$ for every linear code (\cref{thm:deep-mca}), which in particular resolves the underdetermined and rank-drop singular Hankel buckets there (\cref{lem:singular-pencil,cor:singular-deep}); the surviving content of T2 is confined to a single band conjecture (\cref{prob:band}). T3 is discharged by an exact restriction-of-scalars lift (\cref{thm:weil-lines}): extension lines are not a new bucket but a base-field interleaved sampler, verbatim. For T4 we prove an elementary Johnson list bound that is uniform in the interleaving arity (\cref{thm:johnson-list}), an exact divisor-lattice list recursion for fiber-constant words (\cref{thm:list-recursion}), and a calibration proposition showing that the quotient term of any T4 theorem is necessarily exponential (\cref{prop:list-calibration}); together these determine the grand interleaved-list staircase outside the same band (\cref{cor:list-sandwich}). T5 receives the deep-regime curve theorem (\cref{thm:deep-curve}). T6 is discharged: the scanner is specified as a finite procedure, proved sound, and run on the deployed rows (\cref{def:scanner,thm:scanner-sound,tab:scanner}). T7 is treated by an explicit finite-certificate format, a decidability proposition, and a dependency chart of the formal core (\cref{prop:finite-certificate,tab:core}). T8 is discharged on its ledger half by the tower-level form of the T1 theorems, and its genus-one half acquires an unconditional first-step cap: every ECFFT row has an unsafe first staircase step (\cref{prop:rational-floor,cor:ecfft-onestep}). \Cref{thm:sandwich} assembles the resulting two-sided picture; what remains of the entire program is \cref{prob:band} together with \cref{prob:explicit,prob:errorone,prob:isogeny}.

\subsection{T1: stabilizer exhaustion and exact quotient descent}\label{sec:discharge-T1}
...
\]
with an extra factor $d$ for degree-$d$ power-curve samplers \textup(\cref{lem:one-support-d-curve}\textup). The numerator is independent of $q$ and of $|\B|$; the subfield order enters only the lower floors, as demanded by T1.
\end{corollary}
...
\begin{corollary}[status of T1]\label{cor:T1-status}
With the intrinsic \cref{def:periodicity-scale}, the structural exhaustion demanded by T1 is a theorem: every bad parameter is quotient-periodic \textup(periodically witnessed, hence counted exactly at the support level by \cref{cor:periodic-support-count}, and exactly at the image level by \cref{thm:fiber-descent} whenever the received line is fiber-constant\textup) or belongs to the aperiodic bucket. The subfield order $|\B|$ appears only in lower floors; all upper numerators above are $q$-free. The entire residual content of T1 is thereby the aperiodic bucket, treated next.
\end{corollary}

\subsection{T2: the deep regime, the singular pencils, and the residual band}\label{sec:discharge-T2}
...
The CA statement holds in the two-radius form: for any $\delta_{\mathrm{int}}\ge\delta_{\mathrm{fld}}=\delta$ with $3\lfloor\delta n\rfloor\le w-1$, $\eca(C,\delta,\delta_{\mathrm{int}})\le(r+1)/q$, since the tangent alternative kills the far condition at every $\delta_{\mathrm{int}}\ge\delta$. The constant $\frac13$ can be improved toward $\frac12$ of the distance by the methods of \cite{BCHKS25}; we deliberately keep the three-line-per-step, formalization-ready proof --- it feeds T7 --- and record the import separately. Note also that \cref{thm:deep-mca} applies to the bivariate circle code over $\F_p$ itself \textup(minimum weight $n_c-k_c+1$, by extending scalars to $\F_{p^2}$ and using \cref{lem:circle-rs}\textup), so the deep regime of \cref{prob:isogeny}\textup{(b)} is closed as well.
\end{remark}

We next resolve the named obstruction of T2 --- the singular Hankel buckets --- in the regime where \cref{thm:deep-mca} applies. First, the structural normal form of the identically singular branch.
...
The unsafe mass produced by this paper does not press on the aperiodic bucket. The fiber witnesses of \cref{lem:fiber,lem:phi-fiber} have agreement supports that are unions of complete fibers, hence periodicity scale $\ge a\ge2$; by \cref{thm:stabilizer-partition} they are charged to the quotient side of the ledger, and by \cref{thm:fiber-descent} even at the image level. So a polynomially bounded aperiodic numerator in the band remains consistent with every theorem in this paper. This motivates stating the entire residue of T2 as a single conjecture.
\end{remark}
...
\]
the number of MCA-bad finite slopes of any received line, all of whose witness supports are aperiodic in the sense of \cref{def:periodicity-scale}, is at most $P(n)$. By \cref{cor:T1-status,thm:weil-lines} \textup(T1, T3 discharged\textup) and \cref{thm:deep-mca} \textup(deep regime discharged\textup), this conjecture is exactly the missing input \textup{(ii)} of \cref{thm:conditional-mca} throughout the band, for every challenge row.
\end{problem}

\subsection{T3: the exact extension-line lift}\label{sec:discharge-T3}
...
\end{enumerate}
Consequently the extension term $B_{\rm ext}(a)$ of \cref{thm:conditional-mca} is not an independent quantity: it equals, definitionally, a base-field interleaved-sampler numerator. In particular no ``extension-valued counterexample'' in the sense of T3 can exist outside the base ledgers: every genuinely $\F$-valued line is charged, exactly, to a base bucket.
\end{theorem}
...
\begin{corollary}[status of T3]\label{cor:T3-status}
T3 is discharged on the ``lift'' side of its dichotomy: extension lines are covered --- exactly, not merely up to a stable numerator --- by base-field ledgers, namely the interleaved-sampler ledgers of T4/T5 via \cref{thm:weil-lines}. In the deep regime nothing at all remains: \cref{thm:deep-mca} applies over $\F$ directly, uniformly in the field of definition of the pair. On the lower side, \cref{cor:extension-pole-deep-list-floor,cor:extension-pole-quotient-remainder-floor} already produce genuinely extension-valued bad lines from any supplied list, and \cref{lem:confine} \textup(with \cref{cor:Fvalued,cor:circle-Fvalued}\textup) shows the certified deployed failures must be of this kind. What T3 leaves behind is exactly the band input of T4/T5, i.e.\ \cref{prob:band}.
\end{corollary}
\subsection{T4: list bounds --- Johnson uniformly in interleaving, exact recursion, calibration}\label{sec:discharge-T4}
...
Let the hypotheses of \cref{lem:phi-fiber}\textup{(ii)} hold and let $U=u_z$ be the pigeonholed received word at agreement $A_2$. Every listed codeword $c_A=-R_A(\varphi)$ lies in the scale-$a$ quotient image $\{g\circ\varphi:\deg g\le\ell_2-2\}$. Hence in any bound of the shape demanded by T4,
\[
...
\]
No uniform subexponential $L_Q$ exists in the band; only the aperiodic term $L_{\rm ap}$ can be uniformly polynomial, which is consistent with \cref{prob:band} and with \cref{rem:quotient-borne}.
\end{proposition}
...
\end{enumerate}
Thus the interleaved-list staircase is determined outside the same Johnson-to-capacity band as the MCA staircase; inside the band, \cref{prop:list-calibration} shows that any completion of T4 must carry an exponential, word-dependent quotient term, and \cref{thm:list-recursion} shows that for fiber-constant words this term is exactly a divisor-lattice recursion.
\end{corollary}
...
\subsection{T5: curves and projective slopes in the deep regime}\label{sec:discharge-T5}
...
Together with \cref{thm:weil-lines}, \cref{thm:deep-curve} also covers the multi-parameter samplers into which extension lines decompose, since those are handled over $\F$ directly by \cref{thm:deep-mca}; what T5 still lacks is only the band-regime aperiodic pivot atlas, i.e.\ the curve instance of \cref{prob:band}, together with the exceptional vanishing-vector strata already flagged in \cref{sec:aperiodic-hankel-certificates}.

\subsection{T6: the certificate scanner, its soundness, and deployed output}\label{sec:discharge-T6}

\begin{proposition}[small-field degeneracy]\label{prop:small-field}
Let $C\subsetneq\F^n$ be any linear code and $q=|\F|$. For every $\delta\in(0,1)$, $\emca(C,\delta)\ge1/q$. Consequently $\delta^*_C(\varepsilon^*)=0$ whenever $\varepsilon^*<1/q$; in particular every row over a field of size $q\le2^{128}$ --- including the deployed QM31 circle rows, $q=p^4<2^{124}$ --- has a degenerate grand-challenge threshold $\delta^*_C(2^{-128})=0$, and the meaningful staircase question there is posed at a larger target such as $\varepsilon^*=2^{-100}$.
\end{proposition}
...
MCA & $(0.16667,\,0.49218)$ & open & --- & \cref{prob:band} \\
...
List (any $m$) & $(0.29290,\,0.49609)$ & open & --- & \cref{prob:band} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Scanner output (\cref{def:scanner}) for the two deployed rows. The open MCA band for the circle row at $\varepsilon^*=2^{-100}$ is the same interval $(0.16667,\,0.49218)$ as above.}
\label{tab:scanner}
\end{table}

The table realizes the interval output demanded by T6: for the KoalaBear MCA staircase, $a_{\rm unsafe}=1{,}064{,}960$ \textup($\delta=\frac12-2^{-7}$\textup) and $a_{\rm safe}=1{,}747{,}627$ \textup($\delta\approx0.16666$\textup); the interval shrinks to one step exactly when \cref{prob:band} is settled, via \cref{thm:conditional-mca} whose other inputs are now discharged \textup(\cref{rem:conditional-status}\textup).

\subsection{T7: the formal finite core}\label{sec:discharge-T7}
...
\bottomrule
\end{tabular}
\caption{The formal finite core (T7): each statement quantifies over explicitly bounded finite sets and uses only the listed elementary ingredients; the dependency order is top to bottom. Formalizing this table in a proof assistant is the outstanding half of T7.}
\label{tab:core}
\end{table}

The list deliberately includes the new safe-side theorems: a prize submission that contains both an unsafe certificate \textup(\cref{cor:grand} and its relatives\textup) and a machine-checked safe region \textup(\cref{thm:deep-mca,thm:johnson-list}\textup) documents the staircase from both sides in the sense of \cref{prop:onestep}.

\subsection{T8: tower transfer and the genus-one first step}\label{sec:discharge-T8}

The ledger half of T8 is already discharged: \cref{thm:stabilizer-partition,thm:fiber-descent,cor:periodic-support-count} were stated and proved for the Chebyshev case alongside the multiplicative case, with the divisor lattice of $D$ replaced by the dyadic scale lattice of the twin coset, exactly as T8 demanded; and \cref{prob:band} is likewise stated for both cases. It remains to treat the genus-one floor. We first extend \cref{def:map-smooth} to rational maps.
...
\]
so $\delta^*_C(\varepsilon^*)\le1-\rho-2/n$ for every $\varepsilon^*\le\frac1{2k}(1-\frac nq)$; and the list challenge is likewise unsafe at that step, since $\binom{n/2}{(k+2)/2}/|\B|\ge q/k>2^{-128}q$. For odd $k$ the same argument with $\ell=\lfloor k/2\rfloor+1$ gives the step at $1-\rho-1/n$.
\end{corollary}
...
\]
so the grand MCA threshold of every such row is determined up to a multiplicative factor of three, up to the microscopic gap $G$. If $q\le2^{128}$ then $\delta^*_C(2^{-128})=0$ \textup(\cref{prop:small-field}\textup).
\end{theorem}

\begin{proof}
Lower bound: $3r^*\le n-k$ and $r^*+1\le\lfloor2^{-128}q\rfloor\le2^{-128}q$, so \cref{thm:deep-mca} gives $\emca(C,r^*/n)\le(r^*+1)/q\le2^{-128}$, and $r^*/n<(n-k)/n=1-\rho$, so $\delta^*\ge r^*/n$ by \cref{def:dstar} (the case $r^*=0$ being trivial). Upper bound: the applicable cap certifies $\emca(C,\delta)>2^{-128}$ for every $\delta\in[1-\rho-G,\,1-\rho)$, so no such $\delta$ belongs to the supremum set, and by \cref{lem:mca-monotone} neither does any larger sub-capacity radius; hence $\delta^*\le1-\rho-G$. The degenerate clause is \cref{prop:small-field} with $\varepsilon^*=2^{-128}<1/q$.
\end{proof}

For $\rho=\tfrac12$ the sandwich reads $\delta^*_C(2^{-128})\in[0.1666\ldots,\,\tfrac12-G]$: the first two-sided determination of a grand-challenge MCA threshold, exact to within a factor of three.
...
\toprule
Obligation & Outcome & Where \\
\midrule
T1 & Discharged: stabilizer exhaustion exact at the support level; exact at the image level for fiber-constant lines; upper numerators $q$-free and $|\B|$-free. & \cref{sec:discharge-T1} \\
T2 & Deep third discharged unconditionally; remaining band is \cref{prob:band}, with rank-pivot and compressed-eliminant certificates. & \cref{sec:discharge-T2,thm:residual-rank-pivot-cover} \\
T3 & Discharged: exact restriction-of-scalars lift; no independent extension bucket exists. & \cref{sec:discharge-T3} \\
T4 & Johnson regime discharged uniformly in the arity $m$; quotient term exact for fiber-constant words; band term proved necessarily exponential. & \cref{sec:discharge-T4} \\
T5 & Deep regime discharged for degree-$d$ curves and projective parameters; band atlas is the curve instance of \cref{prob:band}. & \cref{sec:discharge-T5} \\
T6 & Discharged: scanner specified, proved sound, and run on the deployed rows. & \cref{sec:discharge-T6} \\
T7 & Specified: finite certificate format, decidability, dependency chart; the machine formalization itself remains outstanding. & \cref{sec:discharge-T7} \\
T8 & Ledger half discharged at tower level; genus-one first-step cap proved; macroscopic genus-one band remains \cref{prob:isogeny}. & \cref{sec:discharge-T8} \\
\bottomrule
\end{tabular}
\caption{Status of the completion obligations after \cref{sec:discharge}.}
\label{tab:status}
\end{table}

\begin{remark}[status of the conditional completion theorems]\label{rem:conditional-status}
Of the four inputs of \cref{thm:conditional-mca}: input \textup{(i)} is discharged \textup(\cref{cor:periodic-support-count} at the support level, \cref{thm:fiber-descent} at the image level for fiber-constant lines\textup); input \textup{(ii)} is discharged on the deep third \textup(\cref{thm:deep-mca,cor:singular-deep}\textup), and outside that range is exactly \cref{prob:band} with the residual rank-pivot and kernel-compressed certificate language of \cref{thm:residual-rank-pivot-cover,thm:kernel-compressed-affine-eliminant,thm:exact-arbitrary-residual-image-ledger}; input \textup{(iii)} is discharged \textup(\cref{thm:weil-lines}\textup); input \textup{(iv)} is definitional once the protocol's sampler is fixed. For \cref{thm:conditional-list}, the required base-code bound is now unconditional throughout the Johnson regime and uniform in $m$ \textup(\cref{thm:johnson-list}, making the codegree recursion unnecessary there\textup), is exactly recursive for fiber-constant words \textup(\cref{thm:list-recursion}\textup), and is provably exponential in the band \textup(\cref{prop:list-calibration}\textup), so any completion must take that shape.
\end{remark}

After this treatment, the mathematical residue of the entire completion program consists of: \cref{prob:band} \textup(the aperiodic band conjecture --- now represented by the finite rank-pivot/compressed-eliminant and exact-image certificate language of \cref{thm:residual-rank-pivot-cover,thm:kernel-compressed-affine-eliminant,thm:exact-arbitrary-residual-image-ledger}, and still the single analytic kernel separating \cref{thm:sandwich} from a one-step threshold determination\textup); the failure-profile upgrade from error $1/k$ to error $1-o(1)$ \textup(\cref{prob:errorone}\textup); explicit deployed-size witnesses \textup(\cref{prob:explicit}\textup); and the macroscopic genus-one band \textup(\cref{prob:isogeny}, whose first staircase step and whose deep regime are now settled by \cref{cor:ecfft-onestep} and \cref{thm:deep-mca}\textup). The next section attacks this residue directly, from both sides at once.

\section{Pushing toward full resolution: a pincer on the band}\label{sec:pincer}
...
\Cref{thm:mca-from-ca} converts the safe side of the band problem into a demand for correlated-agreement inputs. The strongest known inputs are the proximity-gap theorems of \cite{BCIKS20} (unique-decoding range with error $n/q$; Johnson range with error $\operatorname{poly}(n)/q$), whose proofs run through a Berlekamp--Welch decoder over the rational function field $\F(Z)$ and an induction over curves, and the recent strengthening \cite{BCHKS25}. Those we import. But the completion program of \cref{sec:prize-program} also prices \emph{self-contained} inputs, since the formal core of T7 (\cref{tab:core}) is built from elementary counting; the following theorem is the elementary member of the family. Its only ingredients are the pair-interleaved Johnson bound already proved for T4 and a rider double count, and it reaches half the Johnson radius with an explicit list-size error.
...
Let $C=\RS[\F,D,k]$ with $n\ge k+2$, and let $\delta$, $r$, $L_2$ be as in \cref{thm:elementary-ca}, so $(n-2r)^2>(k-1)n$. Then
\[
...
\begin{proof}
Since $n\ge k+2$ we have $\sqrt{(k-1)n}\ge k$, so the hypothesis forces $n-2r>\sqrt{(k-1)n}\ge k$, i.e.\ $2r\le n-k-1$: half the Johnson radius always lies below half the distance. \Cref{thm:mca-from-ca} therefore applies, and with the input of \cref{thm:elementary-ca},
\[
...
The deployed caps of \cref{cor:deployed,cor:circle-deployed} certify the unsafe band $[\tfrac12-2^{-7},\tfrac12)$ using only the top \emph{two} slots of the locator expansion at the single top scale ($N=256$). The quotient-remainder ledger already prices deeper prefixes at the multiplicative level (\cref{lem:quotient-remainder-prefix}); two observations remain to be made. First, the graded pigeonhole runs verbatim on \emph{every} map-smooth scale --- the locator expansion $\prod_{b\in M}(\varphi-b)=\sum_j(-1)^je_j(M)\varphi^{m-j}$ is graded by $\deg\varphi$ for any polynomial $\varphi$, in particular for the Chebyshev scales of the circle tower --- which extends the T8 ledger from the first staircase step to every depth. Second, the floor can be \emph{optimized over the scale}: small scales trade prefix cost $|\B|$ per slot against fiber entropy $\binom{n/c}{m}$, and the optimum widens the certified unsafe band by an order of magnitude at deployed sizes. The route has an intrinsic ceiling, which we identify exactly in \cref{rem:entropy-frontier}.
...
\paragraph{The program after the discharge.} \Cref{sec:discharge} converts the completion program from a list of tasks into a single question. The structural, extension, scanner, and formal-core obligations are discharged or specified exactly; the deep regime and the Johnson list regime are theorems; the near-capacity band is unsafe by the caps of this paper. Everything that separates the two-sided sandwich of \cref{thm:sandwich} from a one-step threshold determination is the aperiodic band conjecture (\cref{prob:band}) --- at most polynomially many aperiodically witnessed bad slopes in the open band --- together with, on genus-one domains, the macroscopic question of \cref{prob:isogeny}. The calibration results (\cref{rem:quotient-borne}, \cref{prop:list-calibration}) show that this conjecture is consistent with every floor proved here: the certified unsafe mass is quotient-borne, so the aperiodic bucket may yet be small.
...
\begin{problem}\label{prob:isogeny}
Genus-one and non-uniformizable isogeny-smooth domains. The circle-group case of the isogeny-smooth program is settled by \cref{sec:isogeny}: Chebyshev locators $\prod_b(T_a(x)-x_b)$ on twin-coset $x$-coordinate domains, and power locators on torus twin cosets, give the universal cap for the circle-FRI line rounds and for the bivariate circle codes over every challenge field containing $i$, including the deployed Mersenne-$31$/QM31 family (\cref{cor:circle-grand,cor:circle-deployed}). Two cases remain. \textup{(a)} ECFFT domains \cite{BCKL21}: the folding maps are $x$-coordinate maps of $2$-isogeny chains, of rational degree type $(a,a-1)$, for which the locator expansion loses lacunarity and the certified band collapses to gap $2/n$ (\cref{rem:ecfft-obstruction}); obtain a macroscopic universal cap for these rows --- via a lacunary basis of isogeny locators, a different received-word family, or a matching safe-side theorem showing the collapse is essential. \textup{(b)} Circle codes over challenge fields \emph{not} containing $i$ (e.g.\ $\F=\F_p$ itself): the torus uniformization of \cref{lem:circle-rs} is unavailable and only the line rounds are covered by \cref{cor:circle-grand}; either extend the deep-point conversion \cref{thm:A} to the bivariate circle-code setting directly, or rule the case out of scope for the deployed parameter families.
\end{problem}
