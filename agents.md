...
- The post-near rank-eleven dense-locator packet now proves seventeen further local route cuts under its pinned 32-anchor/18-dense-root/ten-space setup.  High-core absorption is exactly ten-dimensional; at least `990810934` parts per billion of record/eleven-subset incidences lie on positive-dimensional affine-owner or kernel components; and at least `148639925144138894` records have a 98-percent component star.  In the full-rank star route, the owner of deficiency at most `22320` is unique for its record: two candidates would force `K+22832` common coordinates against the `K-1` RS root cap.  The other routes are a rank-nine owner pencil or kernel plane.  One fixed rank-nine cell carries at most `45567658` records; a lifted plane either has at most `1434405` records or shares at least `134944` received coordinate pairs.  An overlap-correct selector places at least `2578110` records on one fixed nine-subset and retains `5868470021012020` marked component extensions at the shortest endpoint.  A rank-nine chart has marked load at most `981105*(m'-10)*n'`; the boundary demand `6849288576200976639` exceeds its `147748596828055575` cap, eliminating the rank-nine fixed target uniformly.  In the kernel lane, canonical rank bases give capacity `M_d*C(K'-10,d+1)` and exclude domination for `10<=K'<=4598`, with wall `K'=4599`.  In rank eight, each independent added coordinate pair determines one owner in the affine `U^2` flat, giving marked cap `981105*C(n'-9,2)`; demand first exceeds this cap at `K'=37996` by `36370688210984` and the exact ratio increases thereafter.  Thus the unresolved intervals are: rank eight only on `10..4598`, rank eight plus kernel on `4599..37995`, and kernel only on `37996..1048576`.  An explicit official-row weighted-plane model still has `4070408` exact-support, pair-noncontained slopes under the deduplicated local conclusions, so it remains a valid warning against discarding extension weight.  No active-v4 atom moves and error rank eleven is not paid.
