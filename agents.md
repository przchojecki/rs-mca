...
- The post-near rank-eleven dense-locator packet now proves fifteen further local route cuts under its pinned 32-anchor/18-dense-root/ten-space setup.  High-core absorption is exactly ten-dimensional; at least `990810934` parts per billion of record/eleven-subset incidences lie on positive-dimensional affine-owner or kernel components; and at least `148639925144138894` records have a 98-percent component star.  In the full-rank star route, the owner of deficiency at most `22320` is unique for its record: two candidates would force `K+22832` common coordinates against the `K-1` RS root cap.  The other routes are a rank-nine owner pencil or kernel plane.  One fixed rank-nine cell carries at most `45567658` records; a lifted plane either has at most `1434405` records or shares at least `134944` received coordinate pairs.  An overlap-correct selector places at least `2578110` records on one fixed nine-subset and retains `5868470021012020` marked component extensions at the shortest endpoint.  A rank-nine chart has marked load at most `981105*(m'-10)*n'`; the boundary demand `6849288576200976639` exceeds its `147748596828055575` cap, and the ratio increases with `K'`, eliminating the rank-nine fixed target uniformly.  In the kernel lane, canonical rank bases globalize each chart and give capacity `M_d*C(K'-10,d+1)`; summing all nine ranks excludes kernel-lane domination for every `10<=K'<=4598`, with the inequality honestly reversing at `K'=4599`.  The remaining target is therefore rank eight on that initial interval, and fixed-kernel or rank eight above it.  An explicit official-row weighted-plane model still has `4070408` exact-support, pair-noncontained slopes under the deduplicated local conclusions, so it remains a valid warning against discarding extension weight.  No active-v4 atom moves and error rank eleven is not paid.
