...
- The post-near rank-eleven dense-locator packet now proves ten further local route cuts under its pinned 32-anchor/18-dense-root/ten-space setup.  High-core absorption is exactly ten-dimensional; at least `990810934` parts per billion of record/eleven-subset incidences lie on positive-dimensional affine-owner or kernel components; and at least `148639925144138894` records have a 98-percent component star.  In the full-rank star route, the owner of deficiency at most `22320` is unique for its record: two candidates would force `K+22832` common coordinates against the `K-1` RS root cap.  The other routes are a rank-nine owner pencil or kernel plane.  One fixed rank-nine cell carries at most `45567658` records; a lifted plane either has at most `1434405` records or shares at least `134944` received coordinate pairs.  An overlap-correct selector places at least `2578110` records on one fixed nine-subset and routes it to a fixed kernel chart, shared-core rank-nine plane, or rank-eight error-rank-three flat.  An explicit official-row weighted-plane model has `4070408` exact-support, pair-noncontained slopes under the local rank-nine conclusions, so the fixed-chart output alone cannot be the payment.  The model does not instantiate the dense-anchor ancestors or global unsafety.  The continuation must aggregate the intrinsic full-rank owner keys while retaining pre-deduplication weights, couple the other charts, or use a specific dense-anchor identity.  No active-v4 atom moves and error rank eleven is not paid.
