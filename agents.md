...
> **Updated:** 2026-08-13
> **State snapshot:** `main@93fba1be90b7e82d0d02911900b6da1c1c192b24`
...
- The affine-span transverse MCA compiler `thm:affine-span-mca` is refuted even under its printed direction-separation hypothesis.  An exact `GF(1009)` rank-one family has 31 pair-noncontained bad slopes against the claimed bound 23, with direction agreement `20<m=21`.  The old active-row MCA affine-span payments are therefore withdrawn.  The corrected `thm:proper-subspace-mca` replaces the false final `w` extension factor by `max(1,e-(n-m))`; at the first shortened rows it pays KoalaBear affine ranks through 9 and Mersenne rank 1 for every direction support, with exact higher-rank support walls.  A full-explanation lifted-rank dichotomy splits each top cell exactly: lifted rank `K` admits an affine hyperplane of gauges dropping explanation rank to `K-1`, while lifted rank `K+1` admits no rank-dropping gauge.  The resulting rank-drop suffixes are `e>=992852` for KoalaBear and `e>=1037876` for Mersenne.  In the full-lift branch, `W=C+<r_1>` has `d_1(W)=e` and every higher generalized weight MDS-sharp; its generic MDS-endpoint ceilings are still `743896698428332665` and `219426634`, above budget.  The field-general punctured Johnson profile pays every low support through `e=63908` for KoalaBear and `e=65236` for Mersenne.  A centered-incidence Gram rung first extends these walls to `e=64037` and `e=65418`; the stronger PSD mean-centered profile extends them again to `e=64047` and `e=65454`.  The terminal-deficit affine-line cap moves the walls to `e=64048` and `e=65455`, with endpoint profiles `181326343` and `16100647`.  The remaining full-lift intervals are therefore `64049<=e<=1044238` and `65456<=e<=1044241` and require joint control of the last high-deficit layers or stronger extension-specific structure.  The ordinary affine-span LIST theorem, common-core cancellation, directional Johnson compiler, gauge equivalence, and the selector-free all-LineRay error-affine-core set-pair theorem are not refuted by the example.
