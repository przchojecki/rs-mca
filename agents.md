...
- The post-near rank-eleven dense-locator packet now proves fifty-two local results under its pinned 32-anchor/18-dense-root/ten-space setup.  High-core absorption is exactly ten-dimensional; at least `990810934` parts per billion of record/eleven-subset incidences enter positive-dimensional owner or kernel components; and at least `148639925144138894` records have a 98-percent component star.  A full-rank owner of deficiency at most `22320` is record-intrinsic.  One rank-nine cell carries at most `45567658` records; a lifted original-row plane either has at most `1434405` records or shares a `134944` received-pair core.  An overlap-correct fixed nine-subset carries at least `2578110` records and `5868470021012020` marked extensions at the shortest endpoint.  The coarse marked cap first closes rank nine at `K'=20618`; the residual petal-pair cap `floor(981105*(n'-j)*(m'+j-20)/2)`, with `9<=j<=K'-1`, sharpens the crossing to `K'=15635`.  Exact convex packing of those petals gives `981105*(1048577*K'+34798536326)` and moves the first crossing to `K'=15529`, by `3893601214441`, while `K'=15528` remains open by `1296184504470`.  These refinements stay in residual units and do not use the original-row core floor.  Canonical-basis, nine-shadow, projective-paving, bounded-parallel, bounded-point/line, and shortening-weighted arguments remove the fixed-kernel lane through `K'=1048576`.  Rank-eight owner-pair capacity closes `K'>=37996`; on `22526..37995`, one owner has at least `200632` records and deficiency at most four, matching the chronology terminal guarded in #1169.  At `K'=10`, rank eight is impossible; at `K'=11`, the chart-local fence remains above both selector demands.  Thus the exact component boundary is: rank nine only at `K'=10`; rank nine or rank eight on `11..15528`; rank eight only on `15529..22525`; chronology on `22526..37995`; and no component target from `K'=37996` onward.  No active-v4 atom moves, and rank eleven is not paid.
