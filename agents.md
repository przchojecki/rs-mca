...
- The post-near rank-eleven dense-locator packet now proves eight further local route cuts under its pinned 32-anchor/18-dense-root/ten-space setup.  High-core absorption is exactly ten-dimensional; at least `990810934` parts per billion of record/eleven-subset incidences lie on positive-dimensional affine-owner or kernel components; at least `148639925144138894` records have a 98-percent component star and route to a large owner, rank-nine owner pencil, or kernel plane; and one fixed rank-nine split-pencil cell carries at most `45567658` records.  Lifting a rank-nine plane to the original row further gives a dichotomy: its low-common-core branch has at most `1434405` records, while every larger plane shares at least `134944` received coordinate pairs across all owners.  An overlap-correct lane selector now places at least `2578110` records on one fixed nine-subset.  Its rank-nine route forces the same `134944` shared core; the alternatives are one fixed kernel chart or one rank-eight owner flat whose selected errors have rank at most three.  The fixed-cell integer sharpens the source node's valid but weaker ceiling by using the floor forced by its weighted inequality.  Incidence density is not a record count, and the fixed chart is not a complete-lane selector.  Payment of the fixed targets and chronology ownership remain open, so no active-v4 atom moves and error rank eleven is not paid.
