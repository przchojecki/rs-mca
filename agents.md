...
- The post-near rank-eleven dense-locator packet now proves nine further local route cuts under its pinned 32-anchor/18-dense-root/ten-space setup.  High-core absorption is exactly ten-dimensional; at least `990810934` parts per billion of record/eleven-subset incidences lie on positive-dimensional affine-owner or kernel components; at least `148639925144138894` records have a 98-percent component star and route to a large owner, rank-nine owner pencil, or kernel plane; and one fixed rank-nine split-pencil cell carries at most `45567658` records.  Lifting a rank-nine plane to the original row further gives a dichotomy: its low-common-core branch has at most `1434405` records, while every larger plane shares at least `134944` received coordinate pairs across all owners.  An overlap-correct lane selector places at least `2578110` records on one fixed nine-subset.  Its rank-nine route forces the same `134944` shared core; the alternatives are one fixed kernel chart or one rank-eight owner flat whose selected errors have rank at most three.  An explicit official-row weighted-plane model has `4070408` exact-support, pair-noncontained slopes while satisfying those local rank-nine target conclusions, so the fixed-chart output alone cannot be the payment.  The model does not instantiate the dense-anchor ancestors or global unsafety.  The continuation must retain pre-deduplication incidence weights, couple charts, or use a specific dense-anchor identity.  No active-v4 atom moves and error rank eleven is not paid.
