...
- The post-near rank-eleven dense-locator packet now proves fifty local results under its pinned 32-anchor/18-dense-root/ten-space setup.  High-core absorption is exactly ten-dimensional; at least `990810934` parts per billion of record/eleven-subset incidences lie on positive-dimensional affine-owner or kernel components; and at least `148639925144138894` records have a 98-percent component star.  In the full-rank star route, the owner of deficiency at most `22320` is unique for its record: two candidates would force `K+22832` common coordinates against the `K-1` RS root cap.  The other routes are a rank-nine owner pencil or kernel plane.  One fixed rank-nine cell carries at most `45567658` records; an original-row lifted plane either has at most `1434405` records or shares at least `134944` received coordinate pairs.  An overlap-correct selector places at least `2578110` records on one fixed nine-subset and retains `5868470021012020` marked component extensions at the shortest endpoint.  A rank-nine chart has marked load at most `981105*(m'-10)*n'`; its exact weighted crossing is `K'=20618`, with gap `2403530864991`, while `K'=20617` remains open by `7221289203362`.  The former all-row claim mixed the original-row pair-core floor with residual support size after reverse shortening and is retracted.  Canonical kernel bases exclude domination for `10<=K'<=4598`; retaining all bases of each loopless corank-`d` eleven-set gives at least `d+2` decorations and extends the exact exclusion through `K'=11641`.  A support-internal per-record count, combined stratumwise with that ambient cap, extends the exact exclusion through `K'=11772`, with branch pattern `AARRRRRRR` and wall `K'=11773`.  Coupling all coranks through their spanning nine-shadows closes the kernel lane through `K'=15445`; counting all 55 contained nine-subsets supplies a second resource and extends the exact two-resource LP closure through `K'=15670`.  A rank-two contraction shows that every rank-eight nine-shadow loses at least `C(67474,2)=2276336601` support-pair extensions; charging that deficit in the second resource extends the exact closure through `K'=17608`.  Rank-raising pair counts and loopless target-coloop multiplicities then give seven two-step inequalities coupling corank `d` to corank `d-2`; the exact hierarchy-saturated LP extends the closure through `K'=18101`, with all nine coranks positive and wall `K'=18102`.  More generally, coloopless-dual extension counts, support rank-raising sets, and target-coloop multiplicities give all 28 step inequalities for `2<=t<d<=9`; an exact seven-edge hierarchy certificate extends the kernel closure through `K'=18158`, with first wall `K'=18159`.  In the complete shortened corank-one chart, projective-normal pair counting lowers the record cap from `16295594` to `8147918`; the resulting two-root hierarchy certificate extends exact kernel closure through `K'=377673`, with first wall `K'=377674`.  A paving-matroid basis floor gives exact complete-chart caps for all coranks.  The support-local shortening envelope at `K'=377674` gives `M_2=253238254` and `M_3=3935391907`, but rank-three bounded-parallel and rank-four bounded-point/line basis theorems improve these uniformly to `M_2=84416263` and `M_3=983902549`.  The resulting exact hierarchy replays unconditionally close through `K'=796598`, with first wall `K'=796599`. Coupling each shortening's record cap to its extension count then gives a degree-eleven gap with twelve positive Newton coefficients and removes the fixed-kernel lane through `K'=1048576`.  In rank eight, owner-pair capacity excludes `K'>=37996`; on `22526<=K'<=37995`, marked averaging forces one owner with at least `200632` records and deficiency at most four, exactly the chronology terminal guarded in #1169.  At `K'=11`, an eight-petal rank-eight chart has `4070352` pair-noncontained records and `9265216597693056` marked extensions, exceeding both fixed-chart demands; thus the lower interval requires normalized-span ancestry or cross-chart chronology rather than another chart-local cap.  At `K'=10`, the ten-dimensional correction space equals the complete residual RS space, so every nine-set has rank nine and the rank-eight target is empty.  Thus the exact component boundary is: rank nine only at `K'=10`; rank nine or rank eight on `11..20617`; rank eight only on `20618..22525`; the chronology terminal on `22526..37995`; and no component target from `K'=37996` onward.  No active-v4 atom moves and error rank eleven is not paid.
