...
- The post-near rank-eleven dense-locator packet now proves fifty-eight local
  ...
  component target at `K'=10..12`; rank nine or rank eight on `13..15528`;
...
- At the first open residual row `K'=13`, the correction space has codimension three in `RS_{<13}`.  The quotient-line fiber argument does not extend verbatim to the resulting projective quotient plane: the next proof must control low-support points on plane catalecticant sections, while retaining the absolute corank-one and corank-two kernel capacities and the `j=9,10,11,12` core-offset ledger.
