...
\begin{theorem}[Raw-low heavy-core shortening]
\label{thm:mca-raw-low-heavy-core-shortening}
Put \(R=1048576\) and \(d=67472\), and consider a complete shortened row

\[
 (n,K,m)=(R+K,K,d+K).
\]

Let \(Z\) be a selected family of distinct finite slopes whose explanations
lie in one affine translate of a direction subcode \(C'\subseteq
\F[X]_{<K}\) of dimension \(1\le s\le K\).  For each slope retain one exact
size-\(m\) pair-noncontained support \(S_\gamma\), one explanation
\(h_\gamma\), and one direction \(b_\gamma\in C'\) minimizing the raw
support margin
...
Put \(a_\gamma=h_\gamma-\gamma b_\gamma\) and
\(H_\gamma=\{x:r_0(x)=a_\gamma(x),\ r_1(x)=b_\gamma(x)\}\).  If
\(1\le T\le d\), then some coordinate belongs to at least

\[
 I_s(K,L,T)=
 \left\lceil
 \frac{\bigl(L-\lfloor C_s(K)/(T+1)\rfloor\bigr)_+
       (d+K-T)}{R+K}
 \right\rceil
 \tag{RLH}\label{eq:mca-raw-low-heavy-core}
\]

of the pair cores with \(\widetilde\theta_\gamma\le T\), where \(L=|Z|\)
and \(C_s(K)\) is the support-margin resource in
\cref{thm:mca-nonuniform-support-margin}.  At such a coordinate exactly
one of the following source-bound conclusions applies:

\begin{enumerate}
\item a subfamily of at least \(I_s(K,L,T)\) slopes shortens to the row
  \((R+K-1,K-1,d+K-1)\) with explanation direction dimension at most
  \(s-1\); or
\item the coordinate lies in the complete pair core of every selected
  slope, and the whole family shortens to that row without losing a slope or
  changing direction dimension.
\end{enumerate}

The second alternative is impossible when \(K=s\).
\end{theorem}

\begin{proof}
The proved resource concerns
\(\theta_\gamma=\min\{d+1,\widetilde\theta_\gamma\}\).  Since \(T\le d\),
truncation is inactive on \(\widetilde\theta_\gamma\le T\), while every
remaining record contributes at least \(T+1\) to the resource.  Thus at
least \(L-\lfloor C_s(K)/(T+1)\rfloor\) records are raw-low.  On the selected
support, scalar agreement and \(r_1=b_\gamma\) force
\(r_0=a_\gamma\), so each raw-low pair core contains at least \(m-T\)
support coordinates.  Double counting gives \eqref{eq:mca-raw-low-heavy-core}.

Fix a heavy coordinate \(x\), an incident pair
\(e_0=(a_0,b_0)\), and put
...
Every generator vanishes at \(x\).  If \(U_x<C'\), subtract the codeword
pair \(e_0\) from the received pair.  The incident scalar explanations then
lie in \(U_x\), because

\[
 h_\gamma-a_0-\gamma b_0
 =(a_\gamma-a_0)+\gamma(b_\gamma-b_0).
\]

Divide the received pair, explanations, and pair directions by \(X-x\), and
delete \(x\).  Division is injective on \(U_x\), so the child direction has
dimension at most \(s-1\).

If \(U_x=C'\), every word of \(C'\) vanishes at \(x\).  Differences of any
two selected \(a\)-coordinates and any two selected \(b\)-coordinates lie
in \(C'\).  Hence every selected pair has the same value as \(e_0\) at
\(x\), and \(e_0(x)=(r_0(x),r_1(x))\).  Thus \(x\) belongs to every selected
pair core and the same locator division shortens the whole family.

For completeness, the support used in the lift is the complete scalar
agreement domain \(A_\gamma\), not an arbitrary coordinate-swapped
size-\(m\) support.  It contains \(S_\gamma\) and is pair-noncontained.  If a
shortened pair explained the shortened received pair on
\(A_\gamma\setminus\{x\}\), multiplying by \(X-x\) and adding \(e_0\)
would explain the original pair on \(A_\gamma\).  An exact size-\(m-1\) bad
support exists after shortening: otherwise adjacent \((m-1)\)-subsets of
the complete domain,
which overlap in at least \(K-1\) coordinates, glue by Reed--Solomon
uniqueness and connectedness of the Johnson graph to one explaining pair on
the entire domain.

Finally, if \(K=s\) and \(U_x=C'\), then \(C'=\F[X]_{<s}\), which contains
the constant polynomial \(1\); it cannot vanish at \(x\).  This excludes the
second alternative at the endpoint.
\end{proof}

\begin{theorem}[Uniform weighted-line cap in direction rank one]
\label{thm:mca-uniform-rank-one-weighted-line}
Put \(R=1048576\) and \(d=67472\).  On every complete shortened
KoalaBear row
\[
 (n_K,K,m_K)=(R+K,K,d+K),
 \qquad 1\le K\le R,
...
most one contains at most \(4070947\) distinct finite affine slopes.
\end{theorem}

\begin{proof}
The rank-zero case has at most \(n_K\) slopes and is smaller than the
displayed bound, so consider an affine explanation line with direction
\(\langle P\rangle\), where \(0\ne P\in\F[X]_{<K}\).  Delete the
zero-normal coordinates whose incidence equation is identically satisfied
on the entire affine parameter line.  They are roots of \(P\), so if their
number is \(u\), then \(0\le u\le K-1\).  Set
\[
 j=K-u,\qquad n'=R+j,\qquad m'=d+j,\qquad
 q=\left\lfloor\frac{m'}2\right\rfloor.
\]
After division by \(P(x)\) at a nonroot, each coordinate is an affine graph
line in the projective parameter plane; a nonuniversal root gives a vertical
line.  Merge identical lines while retaining their coordinate weights.
Every selected parameter point is incident to total line weight at least
\(m'\), and pair noncontainment forces at least two line classes.

If no line class contributes more than \(q\) coordinates to an exact
\(m'\)-support, that support owns at least \(q(m'-q)\) unordered cross-class
pairs.  Two distinct projective lines meet in at most one point, so the
cross-pair charges are disjoint and the number of selected points is at most
\[
 W_{\rm low}(n',m')=
 \left\lfloor\frac{\binom{n'}2}{q(m'-q)}\right\rfloor.
 \tag{WL1}\label{eq:mca-rank-one-low}
\]
Otherwise the support has a unique dominant line.  Suppose \(t\) globally
distinct lines with coordinate weights \(w_i\) can dominate, and let
\[
 a_i=\max\{1,m'-\min(w_i,m'-1)\},\qquad 1\le i\le t.
\]
Then \(t\le\lfloor n'/(q+1)\rfloor\), \(1\le a_i\le q\), and a point
charged to line \(i\) uses either one of the other \(t-1\) dominant lines or
at least \(a_i\) units of the remaining line weight.  Hence the high part is
at most
\[
 W_{\rm high}(n',m')=
 \max_{\substack{1\le t\le\lfloor n'/(q+1)\rfloor\\
                  ...
                  n'-tm'+\sum_i a_i\ge0}}
 \left\lfloor t(t-1)+
 \left(n'-tm'+\sum_i a_i\right)\sum_i\frac1{a_i}\right\rfloor.
 \tag{WL2}\label{eq:mca-rank-one-high}
\]
The endpoint restriction is exact.  Holding all variables but \(a_i\)
fixed, the variable part is \(f(a)=(C_0+a)(Q_0+1/a)\).  If \(C_0\ge0\),
then \(f''(a)=2C_0/a^3\ge0\), so a maximum is at an endpoint.  If
\(C_0<0\), then \(f'(a)=Q_0-C_0/a^2>0\), so replacing \(a\) by \(q\)
increases the objective and preserves feasibility.

The low and high classes are disjoint.  The release verifier evaluates
\[
 W(R+j,d+j)=W_{\rm low}(R+j,d+j)+W_{\rm high}(R+j,d+j)
\]
for all \(1\le j\le1048576\).  Its maximum is attained at \(j=1\), where
\[
 W_{\rm low}=483,\qquad W_{\rm high}=4070464,\qquad W=4070947.
\]
The exact scan has \(423092\) strict decreases and \(625483\) equal adjacent
values, so no omitted dimension is larger.
\end{proof}

\begin{corollary}[Repaired affine-error-rank-eleven payment]
\label{cor:mca-rank-eleven-repaired}
Put \(R=1048576\) and \(d=67472\), and retain the intrinsic near-rational
deletion, exact record selection, and reversible gauge of
\cref{prop:kb-post-deletion-affine-error-router}.  The complete direct
post-near KoalaBear affine-error-rank-eleven branch fits the deployed budget.
The exact raw-low induction requires at most
\(248706399341288370\) post-near slopes, leaving slack
\(26274328769971774\) below the unsafe load.
\end{corollary}
...
\[
 ...
 \right\}\right\rfloor
\]
and, for a target child load \(M\) and \(1\le T\le d\),
\[
 \Lambda_s(K;M,T)=
 \left\lfloor\frac{C_s(K)}{T+1}\right\rfloor+
 \left\lfloor\frac{(M-1)(R+K)}{d+K-T}\right\rfloor+1.
 \tag{R11L}\label{eq:mca-rank-eleven-raw-low-threshold}
\]
By \cref{thm:mca-raw-low-heavy-core-shortening}, a parent family of size at
least \(\Lambda_s(K;M,T)\) has either a child of size at least \(M\) and
direction dimension at most \(s-1\), or a lossless whole-family shortening.

Set \(L_1=4070948\).  The following fixed thresholds and exact all-dimension
scans define \(L_s\) recursively:
\[
\begin{array}{c|r|r|r|r}
s&T_s&L_{s-1}&L_s&\arg\max_K\Lambda_s(K;L_{s-1},T_s)\\ \hline
...
\end{array}
\tag{R11}\label{eq:mca-rank-eleven-raw-low-recurrence}
\]
For each row the verifier checks every integer \(s\le K\le R\) and proves
\[
 \Lambda_s(K;L_{s-1},T_s)\le L_s.
 \tag{R11I}\label{eq:mca-rank-eleven-raw-low-scan}
\]

Induct on \(s\).  The base \(s=1\) contradicts
\cref{thm:mca-uniform-rank-one-weighted-line}.  For \(s\ge2\), apply
\cref{thm:mca-raw-low-heavy-core-shortening} with \(T_s\).  A proper span
produces at least \(L_{s-1}\) slopes in lower direction dimension, contrary
to induction.  A full span shortens the complete family without loss.  It
cannot persist through \(K=s\), so a proper span eventually occurs.

The unsafe post-near load is
\[
 B_*-134944+1=274980728111260144>L_{10},
\]
with difference \(26274328769971774\).  Hence the rank-eleven branch is
paid.
\end{proof}

\begin{theorem}[Dense-core pair-type bound]
\label{thm:mca-dense-core-pair-type}
Put \(R=1048576\) and \(d=67472\).  On a complete shortened row
\((n,K,m)=(R+K,K,d+K)\), fix raw margins
\(\widetilde\theta_\gamma\) as in
\cref{thm:mca-raw-low-heavy-core-shortening}, and let \(1\le T\le d\).
Let the raw-low records be those with
\(\widetilde\theta_\gamma\le T\), and suppose their selected pair types
have common cores \(H_e\).
Put
\[
 h=d+K-T,\qquad \lambda=K-1.
\]
If \(h^2>\lambda n\), the number \(t\) of represented pair types is at most
\[
 Q(K,T)=\max\left\{
 \left\lceil\frac n{2h}\right\rceil-1,
 \left\lfloor\frac{n(h-\lambda)}{h^2-\lambda n}\right\rfloor
 \right\}.
 \tag{DC}\label{eq:mca-dense-core-pair-types}
\]
For an affine explanation family of dimension \(1\le s\le K\), define
\[
 C_s(K)=\left\lfloor\max\left\{
 \frac{(R+K)^{\underline{s+1}}}
      {(d+K)(d+1)^{\overline{s-1}}},
 ...
 \right\}\right\rfloor.
\]
Then the complete selected family \(Z_s\) satisfies
\[
 |Z_s|\le
 D_s(K,T):=\left\lfloor\frac{C_s(K)}{T+1}\right\rfloor
       +981105\,Q(K,T).
 \tag{DCB}\label{eq:mca-dense-core-bound}
\]
\end{theorem}

\begin{proof}
Distinct pair cores intersect in at most \(K-1\) coordinates, since a common
coordinate is a common root of two nonzero degree-\(<K\) pair differences.
Writing \(r_x\) for the number of cores through \(x\) and
\(S=\sum_xr_x\ge th\), we have
\[
 \sum_xr_x^2=S+2\sum_{e<f}|H_e\cap H_f|
 \le S+\lambda t(t-1).
\]
Cauchy gives \(S^2\le n\sum_xr_x^2\).  If
\(t<\lceil n/(2h)\rceil\), the first term of
\eqref{eq:mca-dense-core-pair-types} applies.  Otherwise \(th\ge n/2\),
and \(z^2/n-z\) is increasing for \(z\ge n/2\); substituting \(S\ge th\)
and solving the resulting quadratic inequality yields the second term.

Since \(T\le d\), truncation is inactive on the raw-low records, and the
nonuniform margin resource bounds the remaining records by
\(\lfloor C_s(K)/(T+1)\rfloor\).  For one fixed pair, put
\(\delta=\max\{1,m-|H_e|\}\).  Its distinct-slope exception sets outside
\(H_e\) are disjoint and each has at least \(\delta\) points, so it owns at
most
\[
 \left\lfloor\frac{n-|H_e|}{\delta}\right\rfloor\le R-d+1=981105
\]
slopes.  Adding the disjoint high and low parts proves
\eqref{eq:mca-dense-core-bound}.
\end{proof}

\begin{proposition}[Single-threshold wall at affine error rank twelve]
\label{prop:mca-rank-twelve-single-threshold-wall}
Retain the hypotheses of \cref{cor:mca-rank-eleven-repaired}.  At the initial
KoalaBear row \(K=R=1048576\), no single raw-low cutoff in
\cref{thm:mca-raw-low-heavy-core-shortening} can force the rank-ten load
\(248706399341288370\) from the available unsafe load.  The exact best
requirement is
\[
 \min_{1\le T\le d}
 \Lambda_{11}(R;248706399341288370,T)
 =546519697764383119,
 \tag{R12W}\label{eq:mca-rank-twelve-single-threshold-wall}
\]
uniquely at \(T=d=67472\).  This exceeds the available
\(274980728111260144\) slopes by \(271538969653122975\).
\end{proposition}

\begin{proof}
For each of the \(67472\) legal cutoffs, the verifier evaluates the exact
integer expression \eqref{eq:mca-rank-eleven-raw-low-threshold} at \(s=11\)
and \(K=R\).  The displayed endpoint is the unique minimum; the adjacent
cutoff \(T=67471\) requires \(546519951206547786\) slopes.  Therefore the
raw-low heavy-core theorem that pays rank eleven cannot even force its first
rank drop at rank twelve.  This is a sharp method wall, not a lower bound on
the true rank-twelve family and not a payment.
\end{proof}

\begin{remark}[Scope after the repaired rank-eleven payment]
\label{rem:mca-rank-eleven-repair-scope}
This direct theorem removes affine error rank eleven from the post-near
residual, but it does not regenerate the active-v4 first-match chronology or
move a deployed ledger atom.  Affine error rank twelve remains open already
at the initial row by
\cref{prop:mca-rank-twelve-single-threshold-wall}.  Its next theorem must
couple several raw-margin levels, use cross-pair/core structure, or construct
a chronology-correct owner; another single cutoff cannot suffice.
\end{remark}

\begin{proposition}[All-level scalar-resource barrier at rank twelve]
\label{prop:mca-rank-twelve-scalar-resource-barrier}
Retain the initial KoalaBear row and the post-near unsafe load
\[
 N=B_*-2d+1=274980728111260144.
\]
Put \(n=2097152\), \(m=1116048\), and
\[
 C=C_{11}(1048576)=3313389801746721900417.
\]
Consider the relaxation in which every selected record has raw margin
\(r_\gamma\ge1\), truncated margin
\(\theta_\gamma=\min\{d+1,r_\gamma\}\),
\(\sum_\gamma\theta_\gamma\le C\), and guaranteed pair-core incidence
\(m-r_\gamma\).  These hypotheses permit a histogram with total guaranteed
core incidence exactly
\[
 252089545421228709377370.
 \tag{R12S}\label{eq:mca-rank-twelve-scalar-core-incidence}
\]
Consequently they cannot force any one coordinate through more than
\[
 120205662451376300
\]
records.  This is short of the rank-ten child target
\(248706399341288370\) by \(128500736889912070\).

Even the stronger fantasy premise that every selected support contributes
all \(m\) core incidences forces by averaging only
\[
 \left\lceil\frac{Nm}{n}\right\rceil
 =146337362121160346,
\]
still short by \(102369037220128024\).  Thus no one-coordinate argument
based only on averaging the selected-support core incidences, no matter how
it layers the proved scalar margin resource, can produce the existing
rank-ten child target.
\end{proposition}

\begin{proof}
Give every record its mandatory unit of margin resource.  Promoting one
record from \(\theta=1\) to the capped class \(\theta=d+1\) costs \(d\)
additional units and may erase all \(m-1\) guaranteed incidences.  Spending
one additional unit in the uncapped class erases only one incidence.  Since
\((m-1)/d>1\), the incidence-minimizing histogram first promotes as many
records as possible.  Exact division gives
\[
 \left\lfloor\frac{C-N}{d}\right\rfloor
 =49103551414195675,
 \qquad
 C-N-49103551414195675d=56673.
\]
Hence the minimizer has that many capped records of raw margin \(m\), one
record of margin \(1+56673\), and all remaining records of margin one.  Its
incidence is
\[
 (N-49103551414195675)(m-1)-56673,
\]
which is \eqref{eq:mca-rank-twelve-scalar-core-incidence}.  Dividing by
\(n\) and rounding up gives the displayed coordinate load.  The final
full-support-incidence assertion is the same averaging calculation with
total incidence \(Nm\).

The coordinate load is attained within the declared relaxation.  Enumerate
the required incidence units record by record and assign a record of size
\(s_i\) the next \(s_i\) distinct residues in a cyclic traversal of the
\(n\) coordinates.  Since \(s_i\le n\), no record repeats a coordinate, and
every coordinate receives the floor or ceiling of the total incidence
divided by \(n\).  The release verifier checks the exact integers and hostile
unit mutations.
\end{proof}

\begin{remark}[Pair-count information does not repair the scalar barrier]
\label{rem:mca-rank-twelve-scalar-pair-barrier}
For the \(N-49103551414195675-1=225877176697064468\) margin-one records, a
fixed minimizing pair owns at most
\(n-(m-1)=981105\) slopes.  The histogram in
\cref{prop:mca-rank-twelve-scalar-resource-barrier} therefore needs only
\(230227321946\) pair types, whereas the proved sub-square interleaving cap
is \(Q_{11}(1)=12761830235484\), with
\(Q_{11}(1)^2<(2130706433)^6\).  Thus the current total resource,
first core moment, fixed-pair multiplicity, and pair-type cap still admit
the abstract packing.  This is a certificate-class no-go, not a claim that
the histogram is Reed--Solomon realizable.
\end{remark}

\begin{theorem}[Actual-line supported-dual gluing-defect compiler]
\label{thm:mca-actual-line-core-gluing-defect}
Let \(\varnothing\ne\Gamma\subseteq Z_{\rm bad}\setminus N\) be distinct
finite slopes on one actual received line \(r_0+\gamma r_1\).  For every
\(\gamma\in\Gamma\), retain one exact pair-noncontained size-\(m\) support
\(S_\gamma\) and its degree-\((<K)\) explanation \(h_\gamma\).  Assume all
\(h_\gamma\) lie in one affine translate \(h_*+C'\), and choose
\(b_\gamma\in C'\) minimizing
\[
 \widetilde\theta_\gamma
 =|\{x\in S_\gamma:r_1(x)\ne b_\gamma(x)\}|.
\]
...
\[
 ...
 \qquad U=\bigcup_{\gamma\in\Gamma}H_\gamma.
\]
For \(H\subseteq U\), let \(A_H\) be the annihilator of the restricted
degree-\((<K)\) Reed--Solomon code, extended by zero from \(H\) to \(U\),
and put
\[
 A=\sum_{\gamma\in\Gamma}A_{H_\gamma},
 \qquad W=A_U.
\]
Let \(L\) be the space of functions on \(U\) whose restriction to every
\(H_\gamma\) is represented by a degree-\((<K)\) polynomial.  Then
\[
 A^\perp=L,
 \qquad W^\perp=C_K(U),
 \qquad
 \dim W-\dim A=\dim L-\dim C_K(U).
 \tag{GL}\label{eq:mca-actual-line-core-gluing-defect}
\]
Moreover \(r_0|_U,r_1|_U\in L\), so their images in \(L/C_K(U)\) span a
space of dimension \(c\in\{0,1,2\}\).  On the initial KoalaBear rank-twelve
row, assume \(|U|\ge m\) and \(\dim C'=11\).  Then the \(c=0\) family
contains at most
\[
 981104+
 \left\lfloor
 ...
 \right\rfloor
 =49106899082787469
 \tag{G0}\label{eq:mca-rank-twelve-gluing-rank-zero}
\]
distinct slopes.  This is below the rank-ten child target by
\(199599500258500901\).

If, in addition, every \(|H_\gamma|\ge K\) and the cores admit an ordering
\(H_1,\ldots,H_t\) satisfying
\[
 |H_i\cap(H_1\cup\cdots\cup H_{i-1})|\ge K
 \qquad(2\le i\le t),
 \tag{GK}\label{eq:mca-core-incremental-k-overlap}
\]
then \(A=W\).  Consequently \eqref{eq:mca-core-incremental-k-overlap}
routes the family to the paid \(c=0\) terminal on the initial rank-twelve
row.
\end{theorem}

\begin{proof}
Orthogonality to the zero-extended space \(A_H\) is exactly the condition
that the restriction to \(H\) is a degree-\((<K)\) evaluation word.
Intersecting these orthogonal complements proves \(A^\perp=L\).  Taking
\(H=U\) proves \(W^\perp=C_K(U)\), and the dimension identity follows.
The defining equalities on \(H_\gamma\) show that both received
restrictions lie in \(L\).

Their quotient image has dimension at most two.  Suppose first that it has
dimension zero.  Then there are degree-\((<K)\) polynomials \(a,b\) agreeing
with \(r_0,r_1\) on all of \(U\).  Split the records by raw support margin
\(\widetilde\theta_\gamma\).  If
\(\widetilde\theta_\gamma\le d\), then its selected support meets its
complete pair core in at least \(m-d=K\) coordinates.  On those coordinates
both the selected pair \((a_\gamma,b_\gamma)\) and \((a,b)\) equal the
received pair.  Reed--Solomon uniqueness gives
\((a_\gamma,b_\gamma)=(a,b)\).

For these raw-low records, pair noncontainment supplies at least one support
coordinate outside the complete common core
\(G=\{x:r_0(x)=a(x),\ r_1(x)=b(x)\}\).  Such an exception coordinate
determines \(\gamma\) uniquely from
\((r_0-a)+\gamma(r_1-b)=0\); hence distinct slopes use disjoint nonempty
exception sets.  Since \(U\subseteq G\) and \(|U|\ge m\), the raw-low count
is at most \(n-|G|\le n-m=981104\).

If the raw-low count is \(L_0\), every remaining record contributes
\(d+1=67473\) to the nonuniform support-margin resource.  Therefore the
total count is at most
\[
 L_0+
 \left\lfloor\frac{C_{11}(1048576)-L_0}{67473}\right\rfloor.
\]
This expression is nondecreasing for integer
\(0\le L_0\le981104\), so its endpoint is exactly
\eqref{eq:mca-rank-twelve-gluing-rank-zero}.  This proves the stated
rank-zero payment.  Quotient image dimensions one and two remain the two
unpaid actual-line terminals.

Finally, under \eqref{eq:mca-core-incremental-k-overlap}, a local section's
degree-\((<K)\) representatives glue inductively: at each step the new and
old representatives agree on at least \(K\) evaluation points, hence are
identical.  Thus \(L=C_K(U)\), equivalently \(A=W\), which would make the
received quotient image zero and invokes the paid terminal just proved.
\end{proof}

\begin{remark}[Exact rank-twelve realization target]
\label{rem:mca-rank-twelve-gluing-target}
The compiler moves no active-v4 atom and does not pay affine error rank
twelve, but it pays the complete quotient-rank-zero subfamily in the exact
initial-row units.  It replaces the vague missing ``cross-pair
compatibility'' with two exact residual actual-line terminals: quotient
image rank one, where exactly one
projective scalar combination is globally polynomial on \(U\), and quotient
image rank two, where none is.  Outside the guarded post-near domain,
the same rank-zero formula is not claimed; a common pair core alone does not
eliminate support-wise MCA-bad witnesses.  The next theorem must count
or route these source-realizable rank-one and rank-two defect families while
preserving the identical line, slopes, supports, explanations, field, and
first-match chronology.  Abstract higher-order-MDS generation is
insufficient: a three-core \((n,K)=(6,3)\) control has nominal full excess
but one-dimensional gluing defect.
\end{remark}
