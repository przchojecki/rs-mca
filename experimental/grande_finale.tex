...
\begin{proposition}[Weighted exclusion of the rank-nine fixed target]
...
$10\le K'\le1048576$.  Before deduplication, one fixed nine-subset carries
...
\]
At the first dimension not excluded by the common-core size, $K'=67473$,
the two integer bounds are
\[
...
The $2578110>1434405$ distinct records also force
$|J|\ge134944$.  If $K'\le67472$, then $m'\le134944$, contradicting
$J\subset C_p$ and $|C_p|<m'$.  It remains to take $K'\ge67473$.
Exact cross-multiplication at $K'=67473$ gives the displayed strict gap
$6701539979372921064$.  Finally, after cancelling
...
\]
where $z$ is the number of global zero normals.  For $d\ge2$ the currently
proved uniform envelope is
\[
...
proves the uniform cap $M_1=8147918$.
\end{proposition}
...
\begin{corollary}[Conditional projective-basis kernel-capacity cut]
\label{cor:kb-rank11-kernel-projective-basis-capacity}
Assume that every official corank-two shortened chart satisfies the uniform
record cap $M_2\le84416263$.
...
\begin{corollary}[Conditional projective-frame kernel-capacity cut]
\label{cor:kb-rank11-kernel-projective-frame-capacity}
Assume that every official corank-two and corank-three shortened chart
satisfies the uniform record caps $M_2\le84416263$ and
$M_3\le983902549$, respectively.
...
the rank-nine alternative. Proposition~\ref{prop:kb-rank11-kernel-basis-capacity}
...
shows that its uniform promotion is not currently proved; conditional on
that promotion,
...
complete-chart corank-three cap.  Conditional on uniform promotion of both
the corank-two and corank-three caps,
...
explicit fence. Thus the remaining intervals are rank eight only on
$10\le K'\le22525$, dense-owner chronology only on
$22526\le K'\le37995$, no component target on
$37996\le K'\le377673$, and kernel only on
$377674\le K'\le1048576$.  The two conditional capacity replays describe
what the same ledger would prove if their stated uniform-cap premises were
established; they do not shrink this unconditional kernel interval.
...
Remark~\ref{rem:mca-dense-core-owner-acceptance-contract}, pay either the
large-owner, fixed-kernel, rank-eight, or kernel-plane branch, move an
