...
- dense-root high-coefficient saturation of the ten-dimensional correction
  space;
- an exact `198` isolated-point Bezout bound and a
...
- a plane-level dichotomy: a low-common-core rank-nine plane has at most
  ...
  received coordinate pairs across every owner;
- an overlap-correct nine-subset selector placing at least `2578110`
...
- a fixed-nine-subset marked-extension floor of `5868470021012020` at the
  shortest residual endpoint;
- the rank-nine marked-load cap `981105*(m'-10)*n'` in the same incidence
  unit;
...
- a codimension-three sparse-circuit dichotomy: three completions of one
  independent deletion force every support-at-most-five circuit into one
  carrier of size at most seven; otherwise every deletion has at most two
  completions, giving per-record cap
  `99254447944649683780146155758753837527116020`;
- the `K'=13` complete component payment: both nonzero kernel-corank terms,
  high-circuit rank-nine shadows, and the sparse completion cap total
...
- a rank-stratified isolated-incidence theorem: on a fixed eleven-set,
  evaluation rank at most nine puts every compatible record on a kernel line,
  while rank ten leaves at most one isolated retained slope after
  ten-coordinate interpolation.  Thus actual retained records contribute at
  most one isolated incidence per eleven-set, replacing the generic
  multiplicity allowance `198` by `1` for this payment;
- the resulting sharp `K'=41` payment.  Exact component demand
  `R*C(m',11)-C(n',11)` exceeds unchanged complete capacity by
...
- a mixed-support collision theorem.  A nonempty exact source carrier at
  support `c<=5` bounds every target deletion carrier by `s+d-1` outside
  the source whenever `c+d<=11`; exact outside-count incidence gives
  target-support caps through support nine;
- the resulting cross-support payment.  Exact low-support defects and all
  higher-support branches close `K'=60..70`; the minimum gap is
...
- exact exclusion of kernel-lane domination on all 4,589 shortenings
  `10<=K'<=4598`, with the inequality reversing at the honest method wall
  `K'=4599`;
- a loopless-matroid basis-decoration correction giving every corank-`d`
  eleven-set at least `d+2` rank bases;
...
- an ambient/record hybrid cut excluding kernel-lane domination through
  ...
  `K'=11773`;
- a shared nine-shadow resource: every loopless corank-`d` eleven-set has
  at least `C(d+2,2)` spanning nine-subsets, each with at most
  `C(K'-d-9,2)` same-rank extensions;
- exact fractional-knapsack exclusion of the kernel lane through `K'=15445`,
  with the one-shadow method reversing at `K'=15446`;
- a full-containment coupling which counts all `55` nine-subsets and charges
  corank one at `52+3*C(m'-9,2)/C(K'-10,2)`;
- exact two-resource LP exclusion of the kernel lane through `K'=15670`,
  with only coranks one and two active and first reversal at `K'=15671`;
- a rank-two-contraction deficit for every rank-eight nine-shadow: at least
  `C(67474,2)=2276336601` support pairs raise its rank to ten;
...
- all 28 multistep shadow inequalities coupling corank `d` to corank `d-t`
  for every `2<=t<d<=9`;
...
- the shortening-scope fence
  ...
  uniform and gives the generic values `M_2=253238254` and
  `M_3=3935391907` at `K'=377674`;
...
- a loopless rank-three matroid theorem
  `2b(M)>=(m-1)(m-1-a)` for maximum parallel-class size `a`;
- its shortening-aware application, which proves the corank-two cap
  `84416263` uniformly over all official charts;
- the resulting unconditional strengthened two-root hierarchy replay through
...
- a corank-three projective-normal basis count: the `67475` incident normals
  are distinct, no three are collinear, and they span `PG(3)`, forcing at
...
and tests the one-unit repair independently.
...
`kb-mca-rank11-full-carrier-atlas-k72-k73-v1` imports four later proved
source nodes and closes the adjacent rank-nine rows `K'=72,73`. Accordingly,
