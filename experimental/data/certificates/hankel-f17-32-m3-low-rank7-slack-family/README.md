# F17^32 M3 Rank-7 Low-Rank Finite-Slack Family

This directory contains a deterministic all-window finite-slack certificate for
the synthetic rank-7 low-rank family in the M3 regular window `385 <= A <= 426`.

Run:

```sh
python3 experimental/scripts/verify_f17_32_m3_low_rank7_slack_family.py \
  --write experimental/data/certificates/hankel-f17-32-m3-low-rank7-slack-family/f17_32_n512_k256_m3_low_rank7_slack_family_certificate.json
...
Rank `7` is beyond the v4 low-rank degree envelope: degree-only finite
accounting gives `7 > 6`, and degree-only projective accounting gives `8 > 6`.
