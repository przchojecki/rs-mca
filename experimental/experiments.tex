...
zero-root parity matches \(a\).  If this power gate fails, the standard
one-dimensional Kummer-Weil bound on \(\mathbb P^1\) has support contained in
the four discriminant roots plus \(0\) and \(\infty\), so the term has
depth-independent conductor and is bounded by \(4\sqrt p\).  Thus any
no-cancellation boundary quartic is an explicit monomial-square discriminant
branch, not a hidden growing-conductor family.
