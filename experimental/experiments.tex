...
The remaining one-root ledger is explicit.  Let \(\mathrm{W1Large}(A_0)\) be
the set of active large-node pairs \((S,O)\) with \(q_S>s_S+2\),
\(|O|=s_S+1\), and \(\ell_{D_S^{can}\setminus O}\in V_S\).  Put
\(Z_{S,O}=D_S^{can}\setminus O\),
\(a_S=\lfloor(q_S-2)/2\rfloor\), and
\[
  M^{\mathrm{r1}}_{S,O,x}
  =
  \sum_{f=0}^{a_S-1}
    \binom{|\{y\in Z_{S,O}:y>x\}|}{f}.
\]
Then
\[
  FR^{\mathrm{r1}}_1(A_0)
  ...
  \sum_{x\in Z_{S,O}} M^{\mathrm{r1}}_{S,O,x}
\]
is exactly the large-node width-one flag contribution.
Equivalently, if
\[
  b(q)=\sum_{e=1}^{\lfloor(q-2)/2\rfloor}\binom{q-1}{e},
\]
then any surviving subledger \(U\subseteq \mathrm{W1Large}(A_0)\) satisfies
\[
  FR^{\mathrm{r1}}_1(U)
  =
  \sum_{(S,O)\in U} b(q_S).
...
For \(q\ge4\),
\[
  \frac{2^{q-1}}{2q}\le b(q)\le 2^{q-1},
\]
so, with \(q_*(U)=\max\{q_S:(S,O)\in U,\ q_S\ge4\}\),
\[
  FR^{\mathrm{r1}}_1(U)
  \le C_\sigma Q^{\sigma+1}2^{q_*(U)-1}.
\]
Hence a logarithmic rank-test exclusion \(q_*(U)\le K\log_2 Q+O_\sigma(1)\)
gives a polynomial fixed-surplus bound for the width-one subledger.
Conversely, any bound \(FR^{\mathrm{r1}}_1(U)\le C Q^B\) forces every
surviving witness to satisfy
\[
  q_S\le B\log_2 Q+\log_2(2Cq_S)+1.
\]
Thus the remaining width-one target is a concrete rank-test exclusion problem:
after the standard charges, rule out bounded complements
\(\ell_{D_S^{can}\setminus O}\in V_S\) with \(q_S/\log Q\to\infty\).
There is also a fixed-surplus tail compression.  At a large node \(q>s+2\),
let
\[
  \operatorname{Tail}_s
  \bigl(X^{q-1}+c_1X^{q-2}+\cdots+c_{s+1}X^{q-s-2}+\cdots\bigr)
  =(c_1,\ldots,c_{s+1}).
\]
For \(L_O=\ell_{D\setminus O}\), \(Z=D\setminus O\), and
\(u_i=e_i(Z)=(-1)^ic_i\), the identities \(D=Z\sqcup O\) give the triangular
recursion
\[
  e_i(O)=e_i(D)-\sum_{j=1}^i u_j e_{i-j}(O),
  \qquad 1\le i\le s+1.
\]
Thus \(\operatorname{Tail}_s(L_O)\) recovers \(\ell_O\), hence \(O\), and the
bounded-complement tail map is injective.  If the leading coefficient is
nonzero on \(V\), the monic slice of \(V\) is \(M+\lambda N\); every
rank-test witness must satisfy
\[
  \operatorname{Tail}_s(L_O)
  \in \operatorname{Tail}_s(M)+F\,\operatorname{Tail}_s(N).
\]
So any superlogarithmic counterexample is already visible as a point of a
fixed-dimensional bounded-complement tail cloud lying on one affine line,
before the remaining full coefficient equations are checked.
Equivalently, write
\(\operatorname{Tail}_s(M)=(\alpha_i)\) and
\(\operatorname{Tail}_s(N)=(\beta_i)\).  For each \(\lambda\), define
\[
  u_i(\lambda)=(-1)^i(\alpha_i+\lambda\beta_i),\qquad
...
\[
  E_\lambda(X)
  =
  X^{s+1}-o_1(\lambda)X^s+\cdots+(-1)^{s+1}o_{s+1}(\lambda).
\]
Then \(\lambda\) gives a width-one rank-test complement if and only if
\[
  \ell_D(X)=E_\lambda(X)\bigl(M(X)+\lambda N(X)\bigr).
\]
So the bounded-complement rank test is a one-parameter factorization identity:
the fixed-degree complement factor is recovered from the tail, and the
remaining equations are exactly the full product identity.
Equivalently, with
\[
  R_\lambda(X)
  =
  \ell_D(X)-E_\lambda(X)\bigl(M(X)+\lambda N(X)\bigr),
\]
the leading coefficient and next \(s+1\) coefficients vanish identically, so
\(\deg_X R_\lambda\le q-2\).  Each remaining coefficient is a polynomial in
\(\lambda\) of degree at most \(s+2\).  If \(N\ne0\), these residual equations
cannot all vanish identically, since that would give infinitely many distinct
monic degree-\(q-1\) divisors of the fixed squarefree polynomial \(\ell_D\).
Thus any surviving witness is a common root of a long vector of fixed-degree
univariate residual equations.  This is the sharpened algebraic form of the
logarithmic rank-test exclusion.
The first residual equation is explicit.  If
\[
  M+\lambda N
  =X^{q-1}+c_1(\lambda)X^{q-2}+\cdots+c_{s+2}(\lambda)X^{q-s-3}+\cdots
\]
and \(u_i(\lambda)=(-1)^ic_i(\lambda)\), then after the tail recursion the
next coefficient is
\[
  G_{s+2}(\lambda)
  =
  e_{s+2}(D)-u_{s+2}(\lambda)
  -\sum_{j=1}^{s+1}u_j(\lambda)o_{s+2-j}(\lambda).
\]
The equation \(G_{s+2}(\lambda)=0\) is exactly the coefficient of
\(X^{q-2}\) in
\(\ell_D-E_\lambda(M+\lambda N)\), up to the sign \((-1)^{s+2}\), and
\(\deg_\lambda G_{s+2}\le s+2\).  Thus the first post-tail obstruction is a
single fixed-degree polynomial gate; if it is nonzero, there are at most
\(s+2\) scalar candidates at the node before later residual equations are
checked.
More generally, with \(u_0=o_0=1\), \(u_i=(-1)^ic_i(\lambda)\), and
\(u_i=0\) for \(i>q-1\), define
\[
  G_r(\lambda)
  =
  e_r(D)-\sum_{j=0}^{s+1}o_j(\lambda)u_{r-j}(\lambda).
\]
The tail recursion is \(G_r\equiv0\) for \(0\le r\le s+1\), while the full
factorization test is
\[
  G_r(\lambda)=0,\qquad s+2\le r\le q+s.
\]
The coefficient of \(X^{q+s-r}\) in
\(\ell_D-E_\lambda(M+\lambda N)\) is \((-1)^rG_r(\lambda)\), and every gate
has degree at most \(s+2\).  Since \(N\ne0\) prevents the whole chain from
vanishing identically, the scalar candidate set is the root set of the single
nonzero node gcd \(\gcd(G_{s+2},\ldots,G_{q+s})\), again of degree at most
\(s+2\).
The split roots of this gcd obey a sharper base-free packing filter.  If
\(\lambda\ne\mu\) both give factorizations
\(\ell_D=E_\lambda(M+\lambda N)=E_\mu(M+\mu N)\), then the two monic
degree-\(q-1\) divisors \(M+\lambda N\) and \(M+\mu N\) have disjoint root sets
in \(D\); a common root would force \(M\) and \(N\) both to vanish there.
Therefore the genuine split node-gcd roots satisfy
\[
  \#\Lambda_{\rm split}\le \left\lfloor {q+s\over q-1}\right\rfloor .
\]

For the slope-image count itself, width one is already polynomial in fixed
surplus.  At a base-free node, two distinct projective width-one directions
have disjoint root sets in \(D_S\); otherwise both basis directions vanish at a
common domain point.  Hence two such directions force
\[
  2(q_S-1)\le q_S+s_S,
\]
so on \(q_S>s_S+2\) there is at most one width-one projective slope.  On
\(q_S\le s_S+2\), fixed surplus gives only \(O_\sigma(1)\) possible root sets
per node.  The active-tree bound therefore gives
\[
  \#\{\hbox{width-one projective slopes}\}=O_\sigma(Q^{\sigma+1}).
\]
Thus width-one shadows are not a super-polynomial slope-packing source; the
large cube below is a recursive flag-ledger issue.
