...
The complete outer primitive-route compiler treats all transverse types,
not only inner degree twelve.  Starting from the \(26\) source-pencil types
and applying the two preceding \(m=12\) deletions, exact primitive
subdegrees and every proper outer right factor give
\[
  24
  =18\ {\rm forced\ strict\ decompositions}
   +1\ {\rm new\ contradiction}
   +5\ {\rm primitive\ compatible\ types}.
\]
The new empty type is
\[
  (m,n,r,\delta)=(10,6,4,10).
\]
Indeed a right factor of degree two creates the forbidden source profile
\(m'=20\), while a right factor of degree three gives either the impossible
same-fiber inequality \(4\le2\), or
\(\epsilon=3\cdot4=12>3^2\).

The only apparent recursive loop is \(m=6\to30\to6\).  A degree-five right
factor there would be totally ramified at the two rational order-five
poles, hence geometrically \(c z^5\).  Each of its split five-point fibers
among the rational simple outer zeros would give nontrivial fifth roots of
unity in \(K\), contrary to \(\gcd(5,p^6-1)=1\).  Thus the normalized
strict-decomposition graph is acyclic.

One further exact branch-cycle cut removes the primitive realization of
\((m,n,r,\delta)=(4,15,8,2)\).  In the degree-fifteen two-subset actions of
\(A_6,S_6\), the \(A_6\) branch-index ledger is empty.  The unique
\(S_6\) class multiset is two natural \(5\)-cycles and one transposition,
whose product sign is odd.  The row itself is retained through its strict
decomposition route.

Exactly five primitive-compatible types remain:
\[
 (6,10,3,8),\ (6,10,6,4),\ (10,6,5,8),\
 (12,5,2,24),\ (12,5,4,12).
\]
Riemann--Hurwitz and exhaustive GAP class/tuple enumeration compile the
first three into \(16\) three-branch Nielsen passport rows with \(18\)
simultaneous-conjugacy orbits; explicit product-one generating triples are
bound in the exact certificate.  The last two are the six geometric
quintic families above.  These finite targets still require the actual
source-producer divisibility, challenge-field descent, and same-record
owner semantics.  No primitive target is deleted or paid, and no ledger
quantity moves.
