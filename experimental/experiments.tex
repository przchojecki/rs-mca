...
degree-\(\le s\) denominator certificates for one syndrome series have the
...
If the primitive denominator has degree \(\delta\), then every degree-\(<h\)
...
For a degree-\(d\) denominator \(D\), write
\[
...
\]
one has \(h\le t+2\).  Hence every degree-\(<h\) fixed-kernel certificate has
degree \(e\le t+2\), and every degree-\(<h\) first-difference certificate has
degree \(e\le h-1\le t+1\).  Thus all short certificates at the row-count
...
Finally, each fixed denominator degree has a finite-landing alternative.  In
...
degree-\(d\) landing layer is zero-dimensional, projective Bezout bounds it by
\(2^d\) points.  After endpoint collision charges, bad parameters inject into
primitive denominator classes, so finite degree layers contribute at most
\[
