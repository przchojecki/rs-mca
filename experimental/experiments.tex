...
\((q^{h-\delta}-1)/(q-1)\) ambient primitive denominator classes before the
deeper truncation equations are imposed, independent of the number of
half-window depths.  Summed over the four residual families, the family-labelled
tail budget is
\[
...
paired tails also have controlled overlap: any denominator class appearing in
both is already an endpoint-pair certificate at the first half-window rung, so
after the endpoint-pair residual is charged their intersection contributes no
new tail budget.  Scalar-paired tail intersections are likewise explicit: they
are one-sided cutoff endpoint residuals, with one endpoint in the longer
\(t+r\) window and the other in the paired \(t+r-1\) window.  After these
one-sided residuals are charged, scalar-paired intersections also contribute no
new unlabelled tail denominator classes.  Finally, the two scalar tails
...
family-disjoint: every uncharged primitive denominator class has a unique
residual-family label, so the depth-independent family sum is an ordinary
disjoint denominator ledger.  In refined mixed-ladder form, after the
