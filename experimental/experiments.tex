...
input is used.
In the quadratic-palette case \(e=2\) this improves to a full collapse:
then \(h=1\), so every nonempty selected support has the full palette,
\[
        K_{\rm ap}=m_{\rm ap},\qquad C_{\rm ap}=0,\qquad
        \alpha_{\rm ap}=1 .
\]
After endpoint-star pruning, if \(m_{\rm ap}>D\), the full-palette coverage
formula gives \(B_{\rm ap}=q+1\).  Hence for every \(R<q+1\) the optional
sparse residual certificate branch is empty; the local output is only
endpoint-star charges, \(D\)-small residuals, or large support \(B_{\rm ap}>R\).
The first nontrivial case \(e=4\) is also exact.  Let \(P_{\rm ap}\) be the
number of residual supports carrying just one of the two packet classes.  Then
\[
        K_{\rm ap}=2m_{\rm ap}-P_{\rm ap},\qquad
        C_{\rm ap}=P_{\rm ap},\qquad
        \alpha_{\rm ap}=1-{P_{\rm ap}\over2m_{\rm ap}} .
\]
Thus any sparse certificate must satisfy
\[
        (q-1)^2(2m_{\rm ap}-P_{\rm ap})^2
        \le 4R\bigl((q-3)m_{\rm ap}^2+2m_{\rm ap}D\bigr),
\]
and
\[
        2(m_{\rm ap}-D)(q+1-R)\le(q-1)P_{\rm ap}.
\]
If \(m_{\rm ap}\ge LD\), the missing-class side alone rules out the
far-from-star quartic sparse branch whenever
\[
        R<q+1-{L(q-1)\over2(L-1)} .
\]
Below this half-line budget, the \(e=4\) sparse branch is forced into the
\(L\)-near-star/template ledger without any additional global density input.
Conversely, for a desired integer budget \(R\), the exact scalar target is
\[
        A_{\rm target}(q,R,L)=
        \min\left\{
          {\sqrt{R(q-3+2/L)}\over q-1},
        ...
        \right\}.
\]
A global theorem proving \(\alpha_{\rm ap}>A_{\rm target}(q,R,L)\) empties
the far-from-star sparse branch at support budget \(R\); the selected-side
candidate can be compared exactly by squaring.
For fixed \(q\) and \(L\), this target is nondecreasing in \(R\), since the
selected-side square is linear in \(R\) and the missing-side candidate is
linear increasing in \(R\).  Thus closing a budget \(R_2\) automatically closes
all smaller budgets \(R_1\le R_2\).
