...
In refined mixed-ladder form, after the
pre-half-window residuals, the cutoff overlap multiplier ledgers, and the
cutoff family multiplier ledgers are charged, the only remaining finite frontier
charge is the bottom finite parameter set
...
families, the current consumable upper ledger is
\[
  \mathrm{OverlapSep}_{hw}+\mathrm{FamilyBudget}_{hw}+2h,
  \qquad
  \mathrm{OverlapSep}_{hw}
  =\min\{\mathrm{SideHybridOverlap}_{hw},\mathrm{UnionRankOverlap}_{hw}\}.
\]
Here \(\mathrm{SideHybridOverlap}_{hw}\) is the four one-sided hybrid overlap
sum: the endpoint-pair residual is omitted as a separate hybrid summand because
it is contained in the one-sided overlap union.
This is an upper ledger rather than a sharp disjoint count: the family budget
is paid after the overlap charges and may shrink in a finer accounting.
For raw unlabelled denominator-class counting, however, the half-window tail is
bounded more sharply by the divisor arrangement of the active cutoff primitive
denominators:
\[
...
\]
The overlap-separation term is used to remove cross-family intersections before
applying the family-disjoint closure; it is not an extra raw tail-count addend.
