...
\[
  (2^h-1)+\frac{h(h-1)}2,
\]
before the explicit multiplier ledger attached to each primitive denominator.
This packages the geometry behind the endpoint-short route; it does not
improve the direct \(|\Lambda_{K,>b}|,|\Theta_{D,>b}|\le h\) closure bound
that already follows from short injectivity.
