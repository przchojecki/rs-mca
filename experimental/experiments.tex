...
core is a ruled determinant branch.  After fixed-slope root slices are charged,
any core with three or more residual one-exchange anchors is therefore ruled
but not same-slope; non-ruled cores contribute at most one unordered
one-exchange edge.

The ruled determinant branch also has a local dichotomy.  If
\[
...
\]
After the fixed-slope cases are removed, \(\alpha\) and \(\beta\) are not
proportional, and the active finite-slope map
\[
  y\longmapsto -\alpha(y)/\beta(y)
\]
is injective on the anchors with \(\beta(y)\ne0\).  Hence a ruled core that
survives fixed-slope charging is not a same-slope multiplicity source: it is a
rank-one moving-slope branch with at most one anchor per finite slope.
...
After ruled cores are charged or removed, residual one-exchange triangles are
therefore top-packet triangles.  After only fixed-slope charging, residual
stars can occur only in the rank-one moving-slope branch, with distinct finite
slopes on the three anchors.

Consequently, after fixed-slope root slices and ruled determinant cores are
removed, the residual \(t=2\) one-exchange graph has maximum degree at most
\(j\): each locator has only \(j\) possible \((j-1)\)-cores, and each non-ruled
core supplies at most one neighbor.  In particular the unordered residual edge
count is at most \(j|A|/2\), and also at most \(\binom{|D|}{j-1}\) by counting
cores.
...
the line-field size as \(Q\), gives for the non-ruled residual family \(A_{\rm
nr}\), \(M=|A_{\rm nr}|\),
\[
  B_2^{\max}(A_{\rm nr})
  \le
...
obstruction must come from small residual family size, root-slice or ruled-core
charges, or higher packet/two-exchange structure; this is not yet a worst-case
packing theorem.
