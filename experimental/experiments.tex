...
rigid: Paper D v12 gives a prize-facing upper cap on the MCA threshold through a
...
The main new additions over the old \texttt{experiments.tex} are one direct
Lane-L construction and two compact bridge lemmas.  The direct construction
gives an exact ordinary Reed--Solomon list lower bound beyond the Johnson radius
on every declared rate-half multiplicative-coset family.  For ordinary
column-distance interleaving, interleaved lists inject into common feasible
supports, so a uniform base locator-fiber theorem gives the same polynomial
exponent for all protocol arities; there is no automatic \(n^{\mu B}\) product
loss.  Finally, a list bound plus an MCA bound implies a concrete line-decoding
bound with loss \(M+nL+1\).  The older BCHKS provenance notes,
quotient-locator dictionaries, normal forms, arithmetic gates, and
finite-threshold infrastructure are retained below whenever they still look
useful.

The July 24--29 integration pass adds a status-preserving ledger for PRs
\#1087--\#1120.  It records exact local list and locator-fiber results, the
current Mersenne rank-seven terminal, KoalaBear equality-wall reductions,
finite-family list brackets, and several route cuts.  It does not import any
new deployed-row payment, and it rejects accounting that conflicts with the
live four-row compiler.
\end{abstract}
...
result is Paper D v12's universal cap.  Through the self-contained deep-point
...
\item \textbf{Paper D conversion audit.}  Paper D v12's headline MCA cap is no
...
\textsc{Conditional} until audited.  Paper D v12's MCA cap should instead be
tracked as \textsc{Proved} when the divisor, binomial, and subfield hypotheses
are verified.
\end{enumerate}

\section{Direct Lane L Result: a Rate-Half Cyclic List Floor}
\label{sec:rate-half-cyclic-quotient-rotation-floor}

\textbf{Status: \textsc{Proved} / \textsc{Direct-list} / lower
construction.}
This section integrates the mathematical content of AllenGrahamHart's
PR~\#1051 into this ledger.  It is a theorem about ordinary Reed--Solomon
codewords in one Hamming ball.  It is not an MCA or CA statement, a list upper
bound, an asymptotic theorem, or an adjacent deployed-row certificate.

\subsection{Exact Lane L contract}

\begin{description}
\item[Row.]
  \((\F_q,D,k=2^{40},n=2^{41},\rho=1/2)\), where
  \(2^{41}\mid(q-1)\) and \(D\) is a multiplicative coset of size \(2^{41}\).
\item[Object.] Ordinary \(\List\), not MCA.
\item[Radius/agreement.]
  \[
    a=2^{40}+2^{34}-1=1116691496959,
    \qquad
    \delta=\frac{2^{40}-2^{34}+1}{2^{41}}.
  \]
\item[Johnson comparison.]
  The radius is strictly beyond the exact Reed--Solomon Johnson radius:
  \[
    a^2<2^{41}(2^{40}-1).
  \]
\item[Bound.]
  \[
  L_C(\delta)\ge
  \left\lceil\frac{\binom{255}{129}}{256}\right\rceil
  =
  11092230961998080258863221315535829014398723445840079610908300691051869570.
  \]
\item[Route.] \textsc{Direct-list}; no CA-to-list or MCA-to-list conversion.
\item[CA or MCA input.] None.
\item[Code shift.] \(C=\RS[\F_q,D,2^{40}]\); no \(C^+\) shift.
\item[Status and impact.] \textsc{Proved} finite-family lower construction;
  it refutes every smaller uniform list cap on the declared family.
\end{description}

\subsection{General quotient-rotation theorem}

\begin{theorem}[cyclic quotient-rotation list floor]
\label{thm:rate-half-cyclic-quotient-rotation-floor}
Let \(\F_q\) be a finite field, let \(D\subseteq\F_q^\times\) be a
multiplicative coset of even size \(n\), and put
\[
  C=\RS[\F_q,D,n/2].
\]
Suppose that \(c\mid n/2\), put \(N=n/c\), and choose integers
\[
  1\le d\le N/2-1,
  \qquad
  m=N/2+d,
...
\begin{equation}
\label{eq:rate-half-list-floor}
  \left\lceil
    \frac{\binom{N-1}{m}}{Nq^{d-1}}
  \right\rceil
\end{equation}
distinct codewords agreeing with it at exactly
\begin{equation}
\label{eq:rate-half-exact-agreement}
  \frac n2+dc+s
\end{equation}
positions.
\end{theorem}
...
\begin{proof}
Write \(D=\gamma H\), where \(H\) is a multiplicative subgroup of order
\(n\), and let
\[
  Q=D^c=\{x^c:x\in D\}.
\]
The power map \(D\to Q\) has fibers of size \(c\), so \(|Q|=N\).  There is a
fixed nonzero \(\Delta\in\F_q\) such that \(y^N=\Delta\) for every \(y\in Q\).
Fix one point \(b_0\in Q\), choose an \(s\)-subset of its \(c\)-point fiber,
and let \(L_0(X)\) be the monic polynomial vanishing on those \(s\) points.

For every \(m\)-subset \(A\subseteq Q\setminus\{b_0\}\), write
\[
  P_A(Y)=\prod_{b\in A}(Y-b)
        =\sum_{j=0}^{m}a_j(A)Y^j
\]
and cyclically rotate its coefficient arc modulo \(Y^N-\Delta\):
\begin{equation}
\label{eq:rate-half-cyclic-rotation}
\begin{split}
  R_A(Y)
  &=
  \operatorname{rem}_{Y^N-\Delta}
    \bigl(Y^{N-d}P_A(Y)\bigr)\\
  &=
...
\[
  L_A(X)=L_0(X)R_A(X^c).
\]
Because \(s<c\), the coefficient blocks created by multiplication by \(L_0\)
do not overlap.  Every coefficient of \(L_A\) in degrees at least
\(k=n/2\) is determined by \(a_0(A),\ldots,a_{d-1}(A)\), together with the
fixed monic coefficient \(a_m(A)=1\).  Indeed, the terms with \(j<d\) lie
strictly above the \(k\)-boundary, the term \(j=m\) begins at degree \(k\),
and every term \(d\le j<m\) ends below degree \(k\).

The constant coefficient \(a_0(A)\), up to its fixed sign, is the product of
the \(m\) elements of \(A\).  Since \(Q\) is a multiplicative coset of a
group of order \(N\), this product takes at most \(N\) values.  Each of
\(a_1(A),\ldots,a_{d-1}(A)\) takes at most \(q\) values.  Hence the
\(\binom{N-1}{m}\) choices of \(A\) occupy at most \(Nq^{d-1}\) high-degree
prefixes.  One prefix fiber therefore has size at
least~\eqref{eq:rate-half-list-floor}.

Fix such a prefix fiber and let \(U\) be its common degree-\(\ge k\) part.
For every member write \(L_A=U+E_A\), where \(\deg E_A<k\).  Then
\(-E_A\in C\), and
\[
  U-(-E_A)=L_A.
\]
On \(D\), equation~\eqref{eq:rate-half-cyclic-rotation} gives
\[
  R_A(x^c)=(x^c)^{N-d}P_A(x^c).
\]
The factor \((x^c)^{N-d}\) is nonzero.  Thus \(L_A\) vanishes precisely on
the \(mc\) points lying above \(A\) and the chosen \(s\) points above
\(b_0\).  This proves the exact
agreement~\eqref{eq:rate-half-exact-agreement}.  Distinct subsets \(A\)
give distinct root sets, hence distinct locators \(L_A\) and distinct
codewords \(-E_A\).
\end{proof}

\subsection{The declared post-Johnson specialization}
...
\[
  ...
  s=c-1.
\]
Theorem~\ref{thm:rate-half-cyclic-quotient-rotation-floor} gives the exact
agreement and list lower bound in the Lane L contract.  The closed-ball radius
is strictly beyond Johnson because the Johnson agreement is
\(\sqrt{n(k-1)}\) and
\[
...
The concrete prime
\[
  q_0=6597069766657=3\cdot2^{41}+1
\]
supplies one fully declared base field; a Pocklington certificate uses base
\(5\), the complete factorization \(q_0-1=3\cdot2^{41}\), and the exact
certificate conditions
\[
  5^{q_0-1}\equiv1\pmod {q_0},
  \qquad
  \gcd\!\left(5^{(q_0-1)/2}-1,q_0\right)=1,
  \qquad
  \gcd\!\left(5^{(q_0-1)/3}-1,q_0\right)=1.
\]
More generally, the theorem applies over every field satisfying
\(2^{41}\mid(q-1)\) and every order-\(2^{41}\) multiplicative coset.

The lower bound has \(243\) bits and exceeds \(2^{238}\).  Consequently, for
every \(q<2^{256}\), it also exceeds \(q/2^{128}\).  This comparison records
the Proximity Prize scale only; it does not convert the ordinary list into an
MCA lower bound.

\paragraph{Source and verification boundary.}
The section is a one-file consolidation of the theorem and proof at source
commit \texttt{0b339ebb0c92c1e7703c1e765f0ad9f819c3b9f7}.  The PR's two
Python replay scripts and its separate per-PR log fragment are deliberately
not imported.  No Python, Sage, Lean, or \TeX\ build is part of this
consolidation.
...
Crites--Stewart list-to-agreement conversion.  Paper D v12 no longer uses this
...
PR \#137 sharpens the remaining one-root ledger into an explicit
bounded-complement algebraic test.  Let \(\mathrm{W1Large}(A_0)\) be the set of
active large-node pairs \((S,O)\) with \(q_S>s_S+2\), \(|O|=s_S+1\), and
\(\ell_{D_S^{can}\setminus O}\in V_S\).  For
\(Z_{S,O}=D_S^{can}\setminus O\) and \(a_S=\lfloor(q_S-2)/2\rfloor\), the
large-node flag contribution is exactly
\[
  FR^{\mathrm{r1}}_1(A_0)
  =
  \sum_{(S,O)\in \mathrm{W1Large}(A_0)}
  \sum_{x\in Z_{S,O}}
  \sum_{f=0}^{a_S-1}\binom{|\{y\in Z_{S,O}:y>x\}|}{f}.
\]
Equivalently, with
\[
  b(q)=\sum_{e=1}^{\lfloor(q-2)/2\rfloor}\binom{q-1}{e},
\]
any surviving subledger \(U\subseteq \mathrm{W1Large}(A_0)\) has weight
\[
  FR^{\mathrm{r1}}_1(U)=\sum_{(S,O)\in U} b(q_S),
  \qquad
  {2^{q-1}\over 2q}\le b(q)\le2^{q-1}\quad(q\ge4).
\]
Thus polynomiality of the width-one one-root ledger is equivalent, up to the
already-polynomial active-tree factor, to excluding surviving rank-test
complements with \(q_S/\log Q\to\infty\).

The same update gives a fixed-surplus tail readout.  If
\[
  L_O=\ell_{D\setminus O}
  =
  X^{q-1}+c_1X^{q-2}+\cdots+c_{s+1}X^{q-s-2}+\cdots ,
\]
then the top \(s+1\) coefficients determine \(O\) by the triangular identities
coming from \(D=(D\setminus O)\sqcup O\).  After choosing a monic pencil
\[
  M+\lambda N\in V,
\]
the full bounded-complement rank test is equivalent to the one-parameter
factorization
\[
  \ell_D(X)=E_\lambda(X)(M(X)+\lambda N(X)),
\]
where \(E_\lambda\) is the recovered degree-\(s+1\) complement factor.  The
residual coefficients form a vector of univariate equations in \(\lambda\),
each of degree at most \(s+2\).  Unless \(N=0\), this residual vector is not
identically zero, so the scalar candidates are the roots of a fixed-degree node
gcd; genuine split roots additionally have disjoint degree-\(q-1\) root shadows.
This is still a local closure target, not a full M1 theorem.
...
\[
  WO_1(A_0)
  \le FR^{\mathrm{r1}}_1(A_0)+O_\sigma(Q^{\sigma+1}),
\]
where \(Q\) is the initial quotient width.  This closes the width-one
critical-tail branch only after a polynomial bound for the displayed explicit
one-root fixed-root/root-slice subledger is proved or imported.  It is not the
final all-line M1 theorem and does not add a leaderboard row.
...
current Paper D v12 and public-site state.  The integrated source files are the
...
\section{June 30 PR Batch: Mixed-Radix Counts, Formal Ledgers, and Audit Packets}
\label{sec:jun30-pr-packets}

This section records the parts of PRs \#137--\#149 that are useful for later
theory or verification.  The largest proof-program dumps were not merged into
this manuscript wholesale: the same-slope packet of PR \#138 is too broad for a
single promoted theorem, and the \(a=327\) RIM packet of PR \#145 is explicitly
a route cut rather than a new interleaved-list row.  Both are summarized in the
triage note rather than upgraded to a proof of M1 or to the leaderboard.

\subsection{Paper B: exact \(\{2,3\}\)-smooth canonical slope count}

PR \#149, due to Holm Buar, supplies the missing mixed-radix analogue of the
2-power count in Paper B.  Let \(N'=2^a3^b\) with \(a\ge1\), and let
\[
  A_{2,3}(N',\ell')
  =
  \#\{e_1(B):B\subseteq\mu_{N'},\ |B|=\ell'\}
\]
be the characteristic-zero canonical slope count.  Conditional on the same
vanishing-sum import used by Paper B's \(\{2,3\}\)-rigidity theorem, the count
is reduced to independent cells.  For \(b\ge1\), put
\[
  n_c=2^{a-1}3^{b-1}.
\]
Each \(2\times3\) cell has a fixed alphabet of \(19\) difference types, with
size sets
\[
  6\cdot\{3\},\qquad
  6\cdot\{2,4\},\qquad
  6\cdot\{1,2,3,4,5\},\qquad
  1\cdot\{0,2,3,4,6\}.
\]
Then
\[
  A_{2,3}(N',\ell')
  =
  \#\{(d_1,\ldots,d_{n_c})\in T^{n_c}:
      \ell'\in \mathrm{Sizes}(d_1)\oplus\cdots\oplus
      \mathrm{Sizes}(d_{n_c})\}.
\]
For \(b=0\), the cell is an antipodal pair and the formula collapses to the
already proved 2-power closed form.  At half rate the new exponent is
\[
  \beta_{2,3}(1/2)=\frac{\log_2 19}{6}\approx0.707983,
\]
below the 2-power value \(\beta_2(1/2)\approx0.792481\).  This is a useful
Paper B combinatorial input: it fixes the characteristic-zero class count for
\(\{2,3\}\)-smooth canonical lines, but it does not by itself prove the finite
field transfer, the M1 local limit, or an extension-line theorem.

\subsection{F1: fixed-root star slope collapse}

The F1 note from PR \#139 sharpens a branch that had already been moved to the
non-aperiodic side of the ledger.  In the global monic-rank-one branch of the
syndrome-pencil normal form, the first \(j+1\) syndrome entries are a rational
normal point at \(\alpha\).  For every star complement
\[
  T=\{\alpha\}\cup U,\qquad |U|=j-1,
\]
the first row of the Hankel landing vector vanishes and the second row is
independent of \(U\):
\[
  a_0=b_0=0,\qquad
  a_1=u_{j+1}-a\alpha^{j+1},\qquad
  b_1=v_{j+1}-b\alpha^{j+1}.
\]
Therefore the whole fixed-root star contributes at most one noncontained
finite slope, and the entire global rank-one branch contributes at most two
slopes after including the scalar-zero slope.  This replaces the older
exponential star bound \(1+\binom{|D|-1}{j-1}\) inside this degenerate branch.
It is a sharpening of a disposed branch, not progress on the genuinely
aperiodic F1 obstruction.

\subsection{High-agreement compiler and Lean ledger packets}

PRs \#139 and \#147 add verification infrastructure around the already promoted
high-agreement tangent staircase.  The certificate package records the exact
row
\[
...
where
\[
  \left\lfloor {17^{32}\over2^{128}}\right\rfloor=6,\qquad
  6\cdot2^{128}<17^{32}<7\cdot2^{128}.
\]
In the exact tangent range this pins the finite-slope support-wise grid:
agreement \(507\) is safe with numerator \(6\), while agreement \(506\) is
unsafe with numerator \(7\).  The row-independent compiler condition is
\[
  1\le B_Q=\lfloor Q/2^{128}\rfloor
     \le \left\lfloor {n-k\over3}\right\rfloor .
\]
For a rate \(1/d\) power-of-two field \(Q=2^\lambda\), this is equivalently
\[
  k\ge
  \left\lceil {3\cdot2^{\lambda-128}\over d-1}\right\rceil .
\]
At the maximum official dimension \(k=2^{40}\), the pinned power-of-two bit
ranges end at \(\lambda=166,168,169,170\) for rates
\(1/2,1/4,1/8,1/16\), respectively.  Larger field sizes remain lower-agreement
M1/L1/F1 work, not high-agreement consequences.

The Lean packet is deliberately smaller than the papers: it formalizes integer
ledger arithmetic, quotient-overlap divisibility gates, the width-one
first-root identity, and the \(17^{32}\) threshold arithmetic.  It does not
formalize Reed--Solomon finite-field decoding or the M1 local limit.  Local
Lean build verification was not run here because the toolchain download was
blocked by the sandbox; the files were checked for the absence of
\texttt{sorry}, \texttt{admit}, \texttt{unsafe}, and executable IO hooks before
integration.

\subsection{Finite scans and source-conditioned audits}

The remaining integrated PRs are audit or experimental support:
\begin{itemize}
\item Jose Brox's PR \#140 fixes stale paths in the L3 quotient-profile
  documentation.
\item Holm Buar's PR \#141 fixes repository-root handling in audit scripts and
  adds a Paper A finite-verification crosswalk.
\item Holm Buar's PR \#142 adds an exact \(p=257\) quotient-periodic locator
  matrix for finite toy cases.
\item Holm Buar's PRs \#143 and \#144 extend the M1 two-coordinate wall and
  beta-pushforward conductor scans.  They are finite numerical evidence only:
  they adjust empirical constants and find no tested violation of the stated
  \(4p\) or bounded-conductor targets.
\item AllenGrahamHart's PR \#148 adds an independent algebra-checker audit for
  the \(\F_{17^{32}}\) row.
\item Holm Buar's PR \#146 adds a compact self-contained \(40\)-slope witness
  audit for the Cycle120 row.  It is retained as a finite witness/audit for
  \(\varepsilon_{\rm mca}(\RS[\F_{17^{32}},H,256],125/256)>2^{-128}\), not as a
  prize solve and not as an exact threshold theorem.
\end{itemize}

The same-slope local-reduction packet in PR \#138 and the \(a=327\) RIM
route-cut packet in PR \#145 remain follow-up material.  They are useful search
directions, but their current form is too large and too conditional to promote
as a clean theorem, and neither moves the public leaderboard.

\section{July 24--29 PR Integration: Local Advances and Route Cuts}
\label{sec:july-29-pr-integration}

This section is the single-file integration ledger for PRs
\#1087--\#1120 (there is no PR \#1100 in the batch).  The patches were reviewed
statically, one by one.  No Python, Sage, Lean, or document build was run during
integration.  Consequently the statements below retain the status supported by
their written arguments and printed certificates; reported replay success is
provenance from the contributing packet, not a fresh execution claim.

\subsection{One-by-one disposition}

\begin{description}
\item[PR \#1087.] Retained as a pinned-profile support counterexample:
  a non-\(T_{16}\)-aligned pair of \(479\)-supports has deficiency \(192\),
  equal locator prefix through coefficient \(39\), and unequal coefficient
  \(40\).  It does not project to a received word or list bound.
\item[PR \#1088.] Retained as the first stage of the cumulative M31 rank-seven
  chain, through deficit head \(Q=29{,}554\); its later status is governed by
  PRs \#1092, \#1105, and \#1113.
\item[PR \#1089.] Retained as the exact adjacent fixed-\(G\) Hahn relaxation
  and route cut.  The relaxation gives an unconditional cap
  \(20{,}737{,}821\), still above the target \(16{,}777{,}214\).
\item[PR \#1090.] Retained as an audit only.  Its checked integers and status
  labels have no detected defect, but external circulation still has an
  \texttt{OPEN GAP}: the load-bearing \texttt{ChoComp26} source is not
  auditable.
\item[PR \#1091.] Retained as a KoalaBear K3 derivation-direction route cut:
  the circulating pencil-chart counts were solved backwards from the closure
  threshold and are not independent census evidence.
\item[PR \#1092.] Retained as the second rank-seven stage: the cumulative local
  head advances through \(Q=147{,}594\), leaving one \(Q=147{,}595\),
  \(k=4{,}981\) residual.
\item[PR \#1093.] Not retained as a true conjectural input.  Its proposed
  uniform rooted-shell cap \(1{,}233\) is recorded only as the conjecture later
  refuted by PR \#1102.
\item[PR \#1094.] Retained as a support-level signed-\(T_8\) census reduction.
  The exact ternary search proves selector support at least \(8\); the known
  selector has support \(24\).
\item[PR \#1095.] Retained as a scoped tension audit: the tested complete-block
  closure produces no \(1{,}234\)th in-band neighbour, but also exposes the
  separate out-of-band obligation.  It does not survive PR \#1102 as evidence
  for the cap \(1{,}233\).
\item[PR \#1096.] Retained for its three boundary falsifiers: restoring a
  puncture, observing only \(31\) coefficients, or pooling exact deficiencies
  invalidates weakened forms of the flatness interface.
\item[PR \#1097.] Retained as an improved post-Johnson rate-half list lower
  construction, but superseded as the strongest declared-field scale by
  PR \#1101.
\item[PR \#1098.] Retained as a scalar-shortening route cut and a conditional
  interface.  PR \#1103 later shows that its selected-support gap hypothesis is
  target-equivalent rather than an independent bridge.
\item[PR \#1099.] Retained for the deterministic interpolation-packing upper
  bound and the resulting ordinary-list bracket.  Its lower endpoint is
  superseded by PR \#1101.
\item[PR \#1101.] Retained as the current declared-field rate-half lower
  champion: the zero-remainder boundary of the quotient-rotation construction.
\item[PR \#1102.] Retained as the explicit refutation of the cap \(1{,}233\):
  one pinned target and anchor have \(d_{192}(A)\ge1{,}237\).
\item[PR \#1103.] Retained as the exact Hahn complementarity identity and route
  cut showing that optimization over selected supports cannot independently
  close the adjacent fixed-\(G\) rows.
\item[PR \#1104.] Retained as the canonical-remainder barrier
  \(\lvert F_\eta\rvert\le1716r(\eta)+5577\), hence an unsafe pinned target
  needs at least \(9{,}774\) represented remainders.
\item[PR \#1105.] Retained as the third rank-seven stage: positive common-zero
  and large projective-line branches close locally, leaving a
  high-master-locator-overlap terminal.  PR \#1113 sharpens that terminal.
\item[PR \#1106.] Retained only as a per-fixed-union KoalaBear bound:
  column-far charts with nullity at most \(10\) lie below the full row budget;
  nullity \(11\) is the first failure.  It is not called a payment because no
  union census or add-back theorem is supplied.
\item[PR \#1107.] Retained as a literal admissible corridor-field pin and exact
  rerun contract.  It strengthens no corridor bound.
\item[PR \#1108.] Retained as an independent Proth-row arithmetic audit, with
  no endpoint or theorem movement.
\item[PR \#1109.] Retained as five finite WCL supporting theorems.  It makes
  the closed-cell register self-contained but closes none of its ten open
  cells.
\item[PR \#1110.] Retained as the tool-relative E1 route boundary: the fixed
  cubic-Hermite variance majorant is already insufficient at \(V=64\) and
  excludes nothing for even \(V\le48\).
\item[PR \#1111.] Retained as an exact local family theorem: the
  \(m=4,h=3\) split-pencil stratum is empty on the four declared Mersenne rows.
  The coincident length \(2^{21}\) does not import this theorem to KoalaBear.
\item[PR \#1112.] Retained as concordance and a route cut, not novelty:
  the rate-half quadratic range is strictly contained in the existing official
  range, leaving the stated three-value seam open.
\item[PR \#1113.] Retained as the terminal rank-seven reduction in this batch.
  A dangerous \(Q=147{,}595\) class is forced into a varying proper-\(G\),
  zero-excess cross-cofactor incidence problem; the needed aggregate bound is
  still open.
\item[PR \#1114.] Retained as provenance for the equality-wall stack, not as a
  new row payment: it packages the \(22\) local prerequisite families.
\item[PR \#1115.] Retained as a local equality-wall normalization: the
  \(q=1\) lower branch and splitting degrees \(2,\ldots,11\) are excluded;
  degrees \(12,\ldots,16\) survive.
\item[PR \#1116.] Retained as the local reduction of those survivors to the
  normalized \(Q=6,u=2\) plane/conic/quartic split.  Its synthetic model search
  is finite evidence, not a deployed-field theorem.
\item[PR \#1117.] Retained as the exclusion of all \(60\) labelled
  \(P_3\sqcup C_3\) common-signature graphs in the reduced conic universe.
  The \(405\) other labelled cases remain.
\item[PR \#1118.] Retained only for the already-known odd-part divisibility
  reduction \(p\mid R\Longleftrightarrow p\mid R_{\rm odd}\) for odd \(p\).
  Its census-dependent application is not banked because the census itself is
  not exported in the packet.
\item[PR \#1119.] Not integrated as an accounting result.  Its abstract
  finite-set compiler is conditional on candidate-supplied predicates, while
  its assertion that a large KoalaBear \(U_{\rm paid}\) is already banked
  conflicts with the live four-row compiler, where \(U_{\rm paid}\) is
  \texttt{null}.  The useful inherited equality-wall reductions are already
  represented by PRs \#1114--\#1117.
\item[PR \#1120.] Retained for the written local algebra: the cubic
  \(3+3\) HNF profile is empty on the four declared \(n=8(p+1)\) rows, and the
  \(2+2+2\) and \(3+2+1\) profiles reduce to explicit low-variable endpoints.
  Unrun Modal, Python, Sage, and gcd requests are not treated as evidence.
\end{description}

\subsection{Pinned M31 quotient-fiber consequences}

The support-level picture is now more sharply delimited.  PR \#1087 gives a
ragged depth-\(32\) collision outside the aligned \(T_{16}\) census:
\[
  |A\setminus B|=|B\setminus A|=192,\qquad
  \operatorname{pref}_{39}(A)=\operatorname{pref}_{39}(B),
  \qquad
  \operatorname{pref}_{40}(A)\ne\operatorname{pref}_{40}(B).
\]
Inside one intact \(T_{16}\) class the two differences are opposite \(T_8\)
halves.  Thus an aligned two-equation census cannot be advertised as
exhaustive.

PR \#1094 identifies the signed opposite-half sector with a ternary
two-congruence lattice of determinant \(p^2\).  Its exact finite census gives
\[
  8\le m_{\min}\le24,
  \qquad
  64\le e_{\min}=8m_{\min}\le192.
\]
These are selector/support statements only.

The proposed pointwise shell cap \(d_e(A)\le1233\) for
\(33\le e\le213\) is false.  PR \#1102 supplies one pinned target and anchor
with
\[
  d_{192}(A)\ge1225+12=1237.
\]
The boundary audits remain useful because they identify load-bearing features
of any replacement theorem: both punctures, all \(32\) coefficients, and the
exact-shell quantifier matter.  Independently, PR \#1104 proves
\[
  |F_\eta|\le1716\,r(\eta)+5577.
\]
Consequently \(|F_\eta|>16{,}777{,}215\) requires
\[
  r(\eta)\ge9774.
\]
This is a necessary condition on an unsafe support fiber, not a uniform list
upper bound and not a deployed \(U_Q\).

\subsection{M31 rank-seven frontier}

The cumulative rank-seven chain advances through four PRs without becoming a
row atom.  PR \#1088 proves
\[
  N_{\delta\le29554}\le15{,}775{,}891.
\]
PR \#1092 advances this to
\[
  N_{\delta\le147594}\le15{,}775{,}917
  <15{,}775{,}932,
\]
and localizes the adjacent failure to one \(Q=147{,}595\),
\(k=4{,}981\) class whose recurrence cap needs a \(45\)-unit improvement.

PR \#1105 pays the positive-common-zero and sufficiently large
projective-line alternatives and forces a high-overlap source branch.
PR \#1113 sharpens any remaining class large enough to violate the local
closing cap: it has at least \(2{,}157{,}929\) zero-excess proper-\(G\)
members, occupies at least \(19\) proper locators, and contains an actual
source-basis pair satisfying
\[
 \deg\gcd(G_i,G_j)\ge228{,}652,\qquad
 \deg\gcd(H_i,H_j)\ge282{,}544,\qquad
 q_i+q_j\le131{,}300.
\]
The missing theorem is an aggregate upper bound \(2{,}157{,}928\) across the
varying proper-\(G\) slices.  Thus \(Q=147{,}595\), higher ranks, v4 ownership,
and the M31 list row remain open.

\subsection{Ordinary-list results beyond Johnson}

At the adjacent fixed-\(G\) boundary pair, PR \#1089 solves the complete
ordinary Johnson-scheme Hahn relaxation:
\[
 H=
 \frac{24044092640301071703360149280}
      {1159431963847722545269}
 =20737821.0968\ldots .
\]
Hence \(L\le20{,}737{,}821\), still \(3{,}960{,}607\) above the required
\(16{,}777{,}214\).  Matching primal and dual certificates make this a route
cut for arguments using only the pairwise Johnson distribution.  PR \#1103
adds the complementarity identity
\[
 H=L+E_{\rm Hahn}+S_{\rm shell},
 \qquad E_{\rm Hahn},S_{\rm shell}\ge0,
\]
and shows that the earlier selected-support gap hypothesis is equivalent to
the desired adjacent cap itself.  Selection optimization therefore supplies
no independent bridge.

For the declared rate-half family put
\[
 q_0=3\cdot2^{41}+1,\quad n=2^{41},\quad k=2^{40},
 \quad T=2^{34},\quad a=k+T-1.
\]
The zero-remainder boundary in PR \#1101 gives, for every multiplicative coset
\(D\) of size \(n\), an ordinary-list lower bound
\[
 L_1=
 \left\lceil
   \frac{\binom{n}{a}}{nq_0^{\,T-2}}
 \right\rceil
 \le L_{\max}(a),
\]
with certified bit interval
\[
 1{,}466{,}604{,}010{,}422
 \le \operatorname{bits}(L_1)
 \le1{,}467{,}447{,}159{,}516.
\]
The deterministic interpolation-packing argument retained from PR \#1099 is
\[
 L_{\max}(a)
 \le
 \left\lfloor
   \frac{\binom{n}{k}}{\binom{a}{k}}
 \right\rfloor,
 \qquad
 \operatorname{bits}(L_{\max}(a))
 \le2{,}095{,}944{,}040{,}454.
\]
The agreement is strictly beyond the exact finite-field Johnson radius.  This
is a genuine finite-family Lane-L bracket for ordinary codewords, not an MCA
claim and not a deployed M31-row result.

\subsection{KoalaBear K3 reductions}

PR \#1091 proves that the two circulating pencil-chart thresholds are
allowances derived backwards from the target, not source-derived chart
censuses.  First-match disjointness and a two-slope chart cap alone permit
arbitrarily many abstract charts, so a closing rank-or-pencil argument still
needs a new geometry-derived census.

PR \#1106 supplies a different local fact.  On one column-far fixed union, the
agreement-weighted transverse-secant bound is below \(B^*\) for nullity
\(\nu\le10\), with
\[
  |Z|_{\nu=10}\le78{,}289{,}526{,}705{,}722{,}101,
\]
while \(\nu=11\) is already above the full row budget.  This is not additive
over all fixed unions and hence is not \(U_{\rm BC}\), \(U_Q\), or a row
payment.

PRs \#1114--\#1117 form one source-scoped equality-wall chain.  After importing
the declared prerequisites, the chain excludes the \(q=1\) lower branch and
splitting degrees \(2,\ldots,11\), reduces degrees \(12,\ldots,16\) to a
normalized \(Q=6,u=2\) plane/conic/quartic split, and excludes all \(60\)
labelled \(P_3\sqcup C_3\) graphs.  The labelled conic universe moves from
\(465\) to \(405\) survivors.  The \(P_6\) and \(P_2\sqcup C_4\) signatures,
quartic/repeated branches, global owner chronology, and chart aggregation
remain open.

An exact deployed-field packet further shows that the reciprocal \(P_6\)
source-facet branch is not empty under the currently extracted local gates.
Over
\[
  \mathbf F_{p^2}\subset\mathbf F_{p^6},
  \qquad p=2{,}130{,}706{,}433,
\]
it constructs twelve distinct source labels and six reconstructed quartic
rows satisfying all twelve weighted-GRS parity equations, the exact \(P_6\)
gcd graph, split and reduced pole quadratics, both endpoint deck fibres, and
the reciprocal common-decic involution.  Thus those local equations alone
cannot eliminate reciprocal \(P_6\).  The construction is not an active
received-line or bad-slope record, does not prove survival of the first-match
partition or failure of earlier owners, and books no charge.  A closing K3
argument must add the active source producer and either delete the component
there or route the same line, slope, and graph record to a reviewed owner.

The exact active-producer follow-up deletes this particular witness.  If
\(H(T,X)\) is an actual outgoing component of
\[
  M(T,X)=\sum_{i=1}^{12}\kappa_iL_i(T)\frac{B(X)}{z_i(X)},
\]
then specialization at every source gives
\(H(\alpha_i,X)\mid B(X)\).  For the committed \(H\), the exact gcd-degree
histogram over the twelve source labels is
\[
  \{0:6,\ 4:6\}.
\]
The five common invariant sources and the extra invariant source
\(\eta=0\) have gcd degree zero, so this \(H\) cannot be an actual endpoint
component.  This is a witness-specific source-semantic deletion, not an
elimination of all reciprocal \(P_6\) components, and it moves no ledger
quantity.

The complete-source saturation argument upgrades this to a uniform conic
exclusion.  Put \(q_i(X)=H(\alpha_i,X)\).  Every actual component satisfies
\(q_i\mid B\).  At a pole \(x\), the nonzero quadratic \(H(T,x)\) vanishes
at at most two of the twelve source labels, so
\[
  \sum_i\operatorname{ord}_xq_i
  \le2\operatorname{ord}_xB.
\]
Both sides have total degree \(48\); hence
\[
  \sum_{i=1}^{12}\operatorname{div}q_i
  =2\operatorname{div}B.
\]
In the conic branch every \(q_i\) is a pullback through the degree-two
conic quotient, so the complete divisor \(B\) is invariant under its
involution \(\iota\), and an \(\iota\)-fixed root of \(B\) cannot be simple.
This contradicts each retained reduced profile.  In the reciprocal profile,
invariance of the complete twelve-source set forces the second fixed source
value, whose unramified fiber is a simple \(\iota\)-fixed pair.  For
\(D_4\), the two rotation-fixed common poles are already simple, and the
simple/double support sizes \(24-2r,r\), \(0\le r\le2\), cannot be unions
of the remaining four-orbits.  For \(D_5\), the same sizes require too many
fixed points for \(r=0,1\), while \(r=2\) forces \(\iota=b\).

Thus reciprocal, \(D_4\), and \(D_5\) are all empty.  Together with the
retained ramified-common exclusions this closes the complete
\(Q=6,s=6,u=2\) conic-image branch, independently of pole partition and
signature.  The birational-quartic \(u=2\) image, \(u=3\), the active
same-record owner bridge, and the KoalaBear row remain open; no ledger
quantity moves.

The all-source saturation identity also sharpens the residual quartic
branch.  At a root \(x\) of \(B\) of multiplicity \(m_x\), exactly two
distinct source rows vanish, each to order \(m_x\).  Hence every pole maps
to a vertex of the complete twelve-line source star.  If \(w_v\) is the
total pole multiplicity over a star vertex \(v\), then a rational plane
quartic has
\[
  \sum_v\binom{w_v}{2}\le3.
\]
Thus at least \(21\) distinct vertices occur; the only repeated profiles
have at most three weight-two vertices or one weight-three vertex.

There is a second, global route cut.  After the proved line/conic
exclusions, every residual actual birational-quartic \(u=2\) component
descends to a non-diagonal bidegree-\((4,4)\) component of the degree-\(60\)
endpoint self-correspondence, and therefore produces geometric monodromy
subdegree four.  The exhaustive primitive degree-\(60\) catalogue contains
nine groups, with complete subdegree rows
\[
\begin{gathered}
  (1,12,12,15,20),\quad
  (1,15,20,24),\quad
...
\end{gathered}
\]
None has subdegree four.  Consequently every residual actual \(u=2\)
survivor forces a geometric functional decomposition \(f=F\circ h\).  The
order-five pole divisor and Riemann--Hurwitz restrict \(\deg h\) to
\[
  2,3,4,5,6,10,12,\text{ or }30.
\]
This removes the primitive quartic-atlas route but does not yet pay the
decomposable branch: each row still needs a source-bound proof that all
witness data descend through a domain-compatible fold, or an
actual-producer deletion.  The result moves no ledger quantity.

The source-bound decomposition adapter makes this finite branch sharper.
For \(m=\deg h\) and \(n=60/m\), every outer zero is simple and its
pullback is a complete unramified \(m\)-point active fiber.  If \(a\) outer
poles have order five and \(b\) are simple, then
\[
  5a+b=n,\qquad
  \frac{4bm}{5}\le2m-2,
\]
and the source divisor consists of \(am\) complete-fiber points plus
\(bm/5\) index-five exceptional points.  Exact enumeration gives
\[
\begin{array}{c|rrrrrrrr}
m&2&3&4&5&6&10&12&30\\ \hline
\text{complete source points}&12&12&12&10&12&10&12&0\\
\text{exceptional source points}&0&0&0&2&0&2&0&12.
\end{array}
\]
The endpoint parameter line is not the deployed carrier domain.  If a
separate same-record theorem transports \(h\) to a complete \(m\)-fold map
on the carrier, then \(m\mid2^{21}\), so only \(m=2,4\) pass that
conditional necessary gate.  The \(m=5\) row is empty for a different
reason.  Riemann--Hurwitz forces two split total branch points, and two
split active fibers descend a target transform of \(h\) to
\(K=\mathbf F_{p^6}\).  After \(K\)-rational source and target
normalization, \(h=cz^5\).  But
\(\gcd(5,p^6-1)=1\), so fifth powering is bijective on \(K\) and cannot
have a reduced five-point \(K\)-rational active fiber.  In the \(m=30\)
row the two exceptional pole fibers give \(h=p_5\circ r\) with
\(\deg r=6\), so that row routes to \(m=6\).  The source conditions are
equivalent to membership in an exact binary degree-\(m\) pencil; for
\(m=12\) this yields one canonical pencil and one explicit membership
test.  The remaining live degrees are \(2,3,4,6,10,12\).  No
parameter-to-carrier transport, data descent, slope payment, or ledger
movement follows from this divisor adapter alone.

The source-pencil compiler makes the six surviving rows deterministic for
each supplied endpoint record.  Canonical unordered source partitions give
\[
  10395+15400+5775+462+66+1=32099
\]
raw templates in inner degrees \(2,3,4,6,10,12\).  For
\(m=2,3,4,6,10\), the forced source locators must have coefficient rank
two, after which
\[
  V_{\rm act}\in
  \operatorname{span}\{H_0^{n-j}H_1^j:0\le j\le n\}
\]
is one exact \(61\times(n+1)\) membership test.  In degree twelve, the
canonical residue \(N_0^5\equiv V_{\rm act}\pmod A\) reduces the test to a
rank-five \(49\times5\) matrix, hence \(44\) independent syndromes.  The
number \(32099\) is a per-record template count, not a finite census of all
endpoint records.

There is also a uniform same-fiber route cut.  First replace a decomposable
inner map by a proper right factor, iterating strictly downward.  If the
inherited irreducible bidegree-\((4,4)\) component were then contained in
\[
  H_0(T)H_1(W)-H_1(T)H_0(W)=0,
\]
the primitive monodromy of the terminal inner map would have subdegree four.
The complete primitive catalogues in degrees \(2,3,4,6,10,12\) have no such
subdegree; degree five, where it can occur, is already empty.  Thus every
actual terminal survivor maps to a non-diagonal outer self-correspondence
\(C\).  If \(\operatorname{bideg}C=(r,r)\) and
\(\delta=\deg(\Gamma\to C)\), then exact projection degrees give
\[
  \delta r=4m,\qquad \delta\le m^2.
\]
This is an explicit unpaid transverse-outer terminal, not a quotient owner.

Exact divisor-interface controls over the deployed field show that this
stopping point is necessary.  Power maps over
\(\mathbf F_{p^2}\subset\mathbf F_{p^6}\) realize the source and active
rank gates in degrees \(2,3,4,6,12\); the degree-two and degree-three
controls are indecomposable.  A separate split degree-ten control uses
\[
  h(z)=(z+2/z)^5+(z+2/z)^2+(z+2/z)
\]
and routes through its degree-two right component.  These controls satisfy
only the divisor interface, not the inherited actual-component,
source-star, received-data, or slope semantics.  Consequently they prove
that another source-only rank calculation cannot close the prime-degree
rows, while moving no ledger quantity.

The inner-degree-twelve outer row admits a further exact geometric cut.  In
this row the outer map \(F\) has degree five, one \(K\)-rational pole of
order five, and five distinct simple \(K\)-rational zeros.  The transverse
ledger initially gives
\[
  (r,\delta)=(1,48),(2,24),(3,16),(4,12).
\]
Because degree five is prime, the geometric monodromy of \(F\) is
primitive.  Its complete subdegree rows are
\((1,1,1,1,1)\), \((1,2,2)\), and \((1,4)\), with repetitions among the five
primitive groups, so no geometric component has subdegree three.  If a
non-diagonal subdegree-one component existed, it would be the graph of a
nontrivial deck transformation.  The degree-five cover would then be tame
cyclic, with exactly two total branch points.  The unique rational pole
makes the second point rational as well, and a rational domain normalization
gives
\[
  F(x)=a x^5+b,\qquad a\ne0.
\]
But \(\gcd(5,p^6-1)=1\), so fifth powering permutes \(K\); this form has at
most one distinct \(K\)-rational zero, contrary to the five imported outer
zeros.  Hence the \(r=1\) and \(r=3\) types are empty, and only
\((r,\delta)=(2,24),(4,12)\) remain at \(m=12\).  The global transverse
frontier falls from \(26\) to \(24\) types.  Neither survivor is paid or
deleted, and no ledger quantity moves.

The two inner-degree-twelve survivors admit a complete geometric outer
normal-form compiler.  The actual component is birational to a
bidegree-\((2,4)\) curve, so its normalization has genus at most three.
For the \(r=4,\delta=12\) row, Riemann--Hurwitz forces the outer component
to have genus at most one.  Exact enumeration of all tame degree-five branch
tuples leaves five profiles:
\[
\begin{array}{c|c|c}
G_{\rm geom}&\text{finite cycle types}&g(C)\\ \hline
A_5&(3),(2,2)&0\\
A_5&(3),(3)&1\\
S_5&(2),(3,2)&0\\
S_5&(2),(4)&0\\
...
\end{array}
\]
No tame polynomial tuple has geometric group
\(\operatorname{AGL}(1,5)\); the remaining \(S_5\) profiles have genus two
or three and are excluded.

The \(r=2\) row has dihedral monodromy and is geometrically affine-equivalent
to the Dickson quintic
\[
  x^5-5ax^3+5a^2x.
\]
Its divided difference splits into the two explicit bidegree-\((2,2)\)
components over \(\sqrt5\).  Critical-value integration gives the five
\(r=4\) normal forms
\[
\begin{gathered}
x^3(12x^2-15(1+\tau)x+20\tau),\quad
  3\tau^2+4\tau+3=0,\\
x^3(6x^2-15x+10),\qquad x^3(x-1)^2,\qquad
x^4(5-4x),\\
x^2(x-1)^2(2x-5\tau),
\end{gathered}
\]
where the last parameter lies on the open locus with the printed branch
profile.  Thus arbitrary outer-quintic search is replaced by five rigid
affine classes and one one-parameter family.  The normalizations are
geometric: no challenge-field coefficient classifier, family deletion,
owner, payment, or ledger movement is claimed.

\paragraph{The inner-degree-12 row is empty.}
The later diagonal-socle packet supersedes the six-family search as a live
frontier.  Let \(N\) be the kernel of the degree-\(60\) monodromy action on
the five inner blocks of size \(12\).  The complete primitive degree-\(12\)
catalogue and the outer point-stabilizer bound show that
\(D=[N,N]\) projects onto the nonabelian simple socle in every block.
Scott's subdirect-product lemma and primitivity of the degree-\(5\) block
action leave only the independent product and one full diagonal strip.  The
actual size-four suborbit meets another block, where the independent product
would give an orbit of size \(12\), so \(D\) is diagonal.

The socle point stabilizer has orbits \(1,11\) in every equivalent action.
For the only exceptional cross-action, between the two degree-\(12\)
actions of \(M_{12}\), an exact paired-standard-generator replay constructs
the group of order \(95{,}040\), its point stabilizer of order \(7{,}920\),
and a transitive orbit of length \(12\) on the opposite action.  The actual
suborbit therefore contributes one point per block, forcing \(r=4\) and
equivalent socle actions on all five blocks.  The centralizer of this
faithful two-transitive socle in \(S_{12}\) is trivial.  Hence every element
of the full monodromy has the form
\[
  (x,i)\longmapsto(n_g(x),\pi_g(i))
\]
after equivariant block identifications.  It preserves the twelve columns
of size five.  The monodromy/intermediate-field correspondence and Luroth's
theorem now force a second geometric decomposition of the same endpoint map
with inner degree \(5\), contradicting the universal source-fiber adapter's
challenge-field fifth-power exclusion.  Thus the complete \(m=12\) row is
empty, the transverse frontier falls from \(24\) to \(22\) types, and the
live inner degrees are \(2,3,4,6,10\).  No \(u=2\), K3, row, owner, or
ledger closure follows.

\paragraph{Inner degree 10 routes strictly to lower degrees.}
For the six original blocks of size \(10\), the complete primitive
degree-\(10\) catalogue has simple socle \(A_5\), \(A_6\), or \(A_{10}\).
If the block kernel is trivial, the outer point-stabilizer order bound
\(120\) leaves only the inner groups \(A_5\) and \(S_5\).  The global
degree-\(60\) action is then the \(A_6\) or \(S_6\) action on flags
\[
  (i,A),\qquad i\in\{1,\ldots,6\},\quad
  A\subseteq\{1,\ldots,6\}\setminus\{i\},\quad |A|=2.
\]
An exact permutation replay gives point-stabilizer subdegrees
\[
  1,2,3,3,3,\underbrace{6,\ldots,6}_{8}
  \quad\hbox{for }A_6
\]
and
\[
  1,2,3,3,3,\underbrace{6,\ldots,6}_{6},12
  \quad\hbox{for }S_6.
\]
Neither contains the actual transverse subdegree four.

Otherwise \(D=[N,N]\) is subdirect in six copies of one of the three simple
socles.  Scott's lemma and outer primitivity make its strip supports a
uniform invariant partition, with support size \(t\in\{1,2,3,6\}\).
For \(t=1\), an independent factor gives an orbit of size \(10\) in another
block, contradicting the actual size-four suborbit.  For \(t>1\), all socle
automorphisms are realized in the catalogue's degree-\(10\) actions and the
socle centralizer is trivial.  Untwisting the strips therefore produces
preserved columns of size \(t\).  Monodromy/intermediate-field
correspondence and Luroth's theorem give a second decomposition of inner
degree \(t\in\{2,3,6\}\).  Thus \(m=10\) has no independent terminal
producer: its four types route strictly to lower live rows.  The transverse
frontier falls from \(22\) to \(18\) types in inner degrees \(2,3,4,6\).
No destination row is deleted or paid, and no \(u=2\), K3, KoalaBear-row,
owner, or ledger closure follows.

\paragraph{Inner degree 6 routes to degree 2 or the excluded degree 5.}
For the ten original blocks of size \(6\), a trivial block kernel makes the
global group one of the \(45\) transitive degree-\(10\) groups.  Since its
point stabilizer has a primitive degree-\(6\) quotient, \(600\mid |G|\).
Only catalogue entries \(40,\ldots,45\) survive.  The \(A_{10}\) and
\(S_{10}\) point stabilizers \(A_9,S_9\) have no transitive degree-\(6\)
quotient.  In each of the four remaining wreath groups, exact subgroup
reconstruction gives endpoint and intermediate indices \(6\) and \(5\).
The resulting inner degree five is already excluded.

For nontrivial kernel, \(D=[N,N]\) is subdirect in ten copies of \(A_5\)
or \(A_6\).  Refine its Scott strips by twists realized on the six-point
actions.  A compatible coordinate has stabilizer orbits \(1,5\), while an
incompatible coordinate or another strip contributes a transitive
six-point orbit.  The actual four-point suborbit therefore consists of
synchronized fixed counterparts in a uniform class of size \(5\) or
\(10\).  Size five is excluded.  At size ten, the four points lie in one
column fiber, but the primitive degree-\(10\) catalogue has no subdegree
four.  The column map factors to inner degree \(2\) or \(5\), so only
degree two survives.  Thus \(m=6\) has no independent producer and the
frontier falls from \(18\) to \(12\) types in degrees \(2,3,4\).  Degree
two is not deleted or paid, and no \(u=2\), K3, row, owner, or ledger
closure follows.

\paragraph{Inner degree 4 cuts to one outer \(A_6/S_6\) type.}
The four incoming source-rank rows are
\[
  (r,\delta)=(1,16),(2,8),(4,4),(8,2).
\]
The complete primitive degree-\(15\) catalogue consists of \(A_7\),
\(A_6\), \(S_6\), \(\operatorname{PSL}(4,2)\), \(A_{15}\), and
\(S_{15}\).  Its nontrivial point-stabilizer subdegrees are \(14\), except
for the two-subset actions of \(A_6,S_6\), whose subdegrees are \(6,8\).
Thus the rows with \(r=1,2,4\) force a proper right factor of the outer
degree-\(15\) map.  Such a factor has degree \(3\) or \(5\), so composition
with the degree-\(4\) inner map routes the endpoint to inner degree \(12\)
or \(20\).  Degree \(12\) is already closed.  Degree \(20\) is impossible:
the three exceptional pole fibers each have profile \(5^4\), contributing
\(3\cdot4\cdot(5-1)=48>2\cdot20-2=38\) to Riemann--Hurwitz.

Only \((r,\delta)=(8,2)\) can remain, and its primitive outer monodromy is
\(A_6\) or \(S_6\) on the fifteen two-subsets of six letters.  Exact action
reconstruction gives subdegrees \(1,6,8\); a five-cycle fixing one letter
acts with cycle profile \(5^3\), so the prescribed outer pole fiber is
compatible and the survivor is not closed.  The transverse frontier falls
from \(12\) to \(9\) types in inner degrees \(2,3,4\).  No destination row,
\(u=2\), K3, KoalaBear row, owner, or ledger payment follows.

\paragraph{The surviving inner-degree-4 type has four geometric passports.}
For a degree-\(15\) rational outer map, Riemann--Hurwitz gives total
branch-cycle index \(28\).  The prescribed \(5^3\) pole cycle has index
\(12\), leaving residual index \(16\).  Reconstructing all eleven
conjugacy classes of \(S_6\) and their actions on two-subsets leaves nine
parity-compatible class multisets.  Exact product-one enumeration derives
the last branch cycle in each row and computes the generated subgroup.
Five rows generate only a proper subgroup of order \(60\) or \(120\).
Exactly four passports remain, in letter-cycle notation:
\[
\begin{array}{c|l}
A_6 & 5.1,\ 2.2.1.1,\ 4.2\\
S_6 & 5.1,\ 2.1.1.1.1,\ 2.2.1.1,\ 2.2.2\\
S_6 & 5.1,\ 2.1.1.1.1,\ 6\\
S_6 & 5.1,\ 2.2.2,\ 3.2.1
\end{array}
\]
Both split \(A_6\) classes of five-cycles give the same result.  Three rows
are three-point covers and one is a four-point cover.  This is the complete
geometric genus-zero frontier by the Riemann existence theorem.  It does not
supply challenge-field descent, split zero or pole fibers, the quartic
source-star incidence, an owner or charge, or an inner-degree-\(4\), K3, or
KoalaBear-row close.

\paragraph{One rigid inner-degree-4 cover is explicit and its poles descend.}
For the retained passport
\(S_6:(5.1,2.1.1.1.1,6)\), the unordered-pair resolvent of BelyiDB's pinned
rational degree-six \(S_6[6,5,2]\) companion is a rational curve.  Projecting
its pair-invariant quintic from the rational triple point leaves the conic
\[
  w^2=3025m^2-2816m+1024.
\]
An exact rational parametrization gives
\[
 T(u)=\frac{-9566429400000(u+44)^6(u+55)^3}
 {(u+143)Q_4(u)^2Q_6(u)},
\]
where
\[
 Q_4=u^4+176u^3+14520u^2+660176u+12576619
\]
and
\[
 Q_6=u^6-330u^5+22143u^4+3380740u^3
      -372423117u^2-39333485730u-870224422859.
\]
Direct subtraction factors as
\[
 T(u)-1=
 \frac{-(u+77)^5(u^2-44u-4961)^5}
 {(u+143)Q_4(u)^2Q_6(u)}.
\]
The fibers over \(0,1,\infty\) therefore have profiles
\((6,6,3)\), \((5,5,5)\), and
\((2,2,2,2,1,1,1,1,1,1,1)\).  The three order-five points are
\(-77\) and \(22\pm33\sqrt5\).  Since every base-field unit is a square in
the even extension \(K=\mathbb F_{2130706433^6}\), all three poles split
over the challenge field.  This proves one rigid normal form and pole
descent only; it does not provide the fixed active fiber, quartic source-star
incidence, an owner or charge, or an inner-degree-\(4\), K3, or KoalaBear-row
close.

\paragraph{A second rigid inner-degree-4 cover is explicit.}
For the passport \(S_6:(5.1,2.2.2,3.2.1)\), a cubic-adjoint normalization of
the unordered-pair quintic for BelyiDB's rational \(S_6[5,6,2]\) companion
gives
\[
 T(u)=\frac{177147(188u-287)^5
 (25444u^2-50922u+15129)^5}{C_3(u)C_6(u)^2},
\]
where
\[
 C_3=14658356u^3-31403007u^2-8441982u+33495606
\]
and
\[
\begin{split}
 C_6={}&915512069923328u^6+6554290056691968u^5
 -83250949083482880u^4\\
 &+290661295480797960u^3-474965645409866205u^2\\
 &+379227334439635443u-119893424310248247.
\end{split}
\]
The three branch profiles are \((5,5,5)\), \((6,3,3,2,1)\), and
\((2,2,2,2,2,2,1,1,1)\).  The order-five points are \(287/188\) and
\[
  \frac{25461\pm7257\sqrt5}{25444},
\]
so they split over \(\mathbb F_{2130706433^6}\).  This is the second rigid
normal form and pole descent only; the active fiber, quartic source-star
incidence, owner, charge, K3, and KoalaBear-row close remain open.

The abstract compiler proposed in PR \#1119 proves only the taut finite-set
fact that least-owner selection partitions candidate-supplied records and the
arithmetic distinction between \(68\) and \(69\) declared source-map classes.
It does not validate those declarations.  In particular, its printed
\(U_{\rm paid}=4{,}200{,}515{,}150{,}819{,}207\) is not imported: the live
four-row authority still records the active KoalaBear \(U_{\rm paid}\) as
\texttt{null}.

\subsection{Other finite certificates and local algebra}

The remaining accepted packets have deliberately narrower scope.
\begin{itemize}
\item PR \#1107 pins one literal \(256\)-bit corridor prime with the stated
  denominator and repeats the existing comparisons; PR \#1108 independently
  checks the four Proth-row arithmetic contracts.  Neither changes a bound.
\item PR \#1109 records the finite WCL exclusions at \((2,5)\) and \((2,6)\),
  the order-\(512\) weight-three and weight-four exclusions, and the Newton
  short-window floor.  The ten registered open cells remain open.
\item PR \#1110 locates the exact failure range of one cubic-Hermite E1
  majorant.  PR \#1118 contributes only the transferred elementary reduction
  \[
    p\mid R\quad\Longleftrightarrow\quad p\mid R_{\rm odd}
    \qquad(p\ \hbox{odd}).
  \]
  The latter packet's unexported census is not used to claim a closed level.
\item PR \#1111 proves emptiness of the local \(m=4,h=3\) split-pencil stratum
  on the four declared \(n=4(p+1)\) Mersenne rows.  PR \#1120 proves emptiness
  of the local \(m=8,h=7\) cubic \(3+3\) profile on the four declared
  \(n=8(p+1)\) rows and reduces the \(2+2+2\) and \(3+2+1\) profiles to
  explicit algebraic endpoints.  Neither family theorem has the live
  first-match owner bridge.
\item PR \#1112 verifies that the proposed rate-half quadratic range is
  contained in the already published official range.  Its high-field
  statement remains a bracket, and its four-slope \(\mathbb F_5\) example is
  an MDS-only route cut rather than a deployed counterexample.
\end{itemize}

\subsection{Status after this batch}

No deployed adjacent row closes.  The live exact-completion compiler remains
the authority: every deployed \(U_Q\) is still \texttt{null}, no active
KoalaBear \(U_{\rm paid}\) is instantiated, exhaustive MCA slope projection
and list-interior coverage remain missing, and no chronology-correct final
add-back is supplied.  The mathematical gains are exact local theorems,
counterexamples to overstrong intermediate claims, sharper terminals, and
finite-family Lane-L results.
