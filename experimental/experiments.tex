...
In refined mixed-ladder form, after the pre-half-window residuals, the cutoff
overlap multiplier ledgers, and the raw half-window tail divisor-arrangement
ledger are charged, the only remaining finite frontier charge is the bottom
finite parameter set \(E_{r_0,>b}\cup\Theta_{r_0,>b}\), of size at most \(2h\)
on the remaining ledger.  Equivalently, apart from the pre-half-window
longer-Pade residual families, the current consumable upper ledger is
\[
  \mathrm{OverlapSep}_{hw}+\mathrm{ArrBudget}_{hw}+2h,
  \qquad
...
\]
This is an upper ledger rather than a sharp disjoint count:
\(\mathrm{ArrBudget}_{hw}\) bounds the whole raw unlabelled tail union, so it
may still include overlap classes already paid by
\(\mathrm{OverlapSep}_{hw}\).  The overlap-separation term is used to remove
cross-family intersections before applying the family-disjoint closure; it is
not an extra raw tail-count addend.
