...
**Companion script:** `scripts/restricted_sum_dp.py`
...
```bash
python3 scripts/restricted_sum_dp.py --p 17 --subgroup-order 8 --r 3 --expect-size 16
```

Paper A style subgroup check:

```bash
python3 scripts/restricted_sum_dp.py --p 257 --subgroup-order 16 --r 9 --expect-size 256
```

JSON output:

```bash
python3 scripts/restricted_sum_dp.py --p 17 --subgroup-order 8 --r 3 --format json
```
