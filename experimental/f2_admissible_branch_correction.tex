...
full-cube mass, including cross-weight collisions.  Finally, an exact
generated-field descent shows that the seven non-generating degree/order
types have the same final block census and kernel mass as three generating
types.
...
\[
 ...
 \qquad(b\in\mathcal G).                            \tag{4.6}
\]
Then
\[
 Z(A)\le \frac{3T_{\mathcal G}^2}{2^m}
       +3L\left(m+1+\frac{2^m}{Q}\right).           \tag{4.7}
\]
...
contributes at most \(B_b\), while (4.6) bounds a good weight by
\[
 \sqrt{M_bB_b}
...
all cross-weight collisions.  Corollary~\ref{cor:distance} controls
