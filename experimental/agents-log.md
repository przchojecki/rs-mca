...
- **What is being added:** Strengthens the integrated M1 average baseline with
  ...
  strict high-overlap profile exact variance identity and max-codegree
  certificate, the slack-one support-family case has an exact
  pairwise-independence phase diagram, entropy-reserve interpretation, and
...
- **How it is useful:** Turns the first-moment random-line estimate into an
  ...
  the entropy-reserve transition after zero-slope, tangent-floor, and
  quotient-periodic families are separated while keeping `q_line` distinct from
  `q_gen`.
