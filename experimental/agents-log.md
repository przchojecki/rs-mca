...
- **Files added or changed:**
  ...
  `experimental/agents-log.md`.
- **Status:** PROVED-LOCAL / AUDIT.
- **What is being added:** The asymptotic far-star ceiling is shown sharp for
  the class-count inequalities: budgets at or above it admit the boundary
  class-count witness `K_ap=m_ap`, `C_ap=(h-1)m_ap` for sufficiently large
  `L`.
- **How it is useful:** Marks the point where class-count arithmetic stops
  being able to rule out sparse certificates, forcing the next M1 step onto
  support realization or row-basis/core-image structure.
- **What to do next:** Search for structural support constraints below or at
  the ceiling rather than increasing the near-star footprint allowance.
...
- **Files added or changed:**
  ...
  `experimental/agents-log.md`.
- **Status:** PROVED-LOCAL / AUDIT.
- **What is being added:** In the reduced selected-support convention, every
  residual support carries at least one selected square-coset class, so
  `alpha_ap>=1/(e/2)=2/e`.  The density-budget evaluator now supports
  `--baseline-a0` to use this theorem-backed floor.
- **How it is useful:** This gives an unconditional baseline support budget
  `R_base(L)=R_dens(2/e,L)` before any deeper global row-basis/core-image
  density theorem is proved.
- **What to do next:** Compare future global density lower bounds against
  this `2/e` baseline and quantify the improvement in `R_Z`.
...
- **Files added or changed:**
  ...
  `experimental/agents-log.md`.
- **Status:** PROVED-LOCAL / AUDIT.
- **What is being added:** The missing square-coset class incidence `J(x)`
  satisfies `I(x)+J(x)=m-d_Sigma(x)` and has mass
  `(2(q-1)/e) sum_S |Theta_S^c|`.  After pruning with cap `D`, uncovered
  residual slopes are bounded by this missing-class mass divided by `m_ap-D`.
- **How it is useful:** This turns the partial-palette leftover after
  full-palette coverage into an explicit defect ledger: many uncovered slopes
  require many missing local square-coset classes.
- **What to do next:** Relate missing class patterns to the global row-basis
  and core-image constraints in the M1 residue-line normal form.
