...
ONE_SPIKE_WINDOW_FULL_HANKEL_REF = (
    "experimental/data/certificates/"
...
def validate_inputs(
    ...
    require(
        ...
        == 238
        and low_rank6_11_known_ledger_table["aggregate"][
            "finite_regular_root_full_hankel_witness_upper_sum"
        ]
        == 0
        and low_rank6_11_known_ledger_table["aggregate"][
            "projective_infinity_contribution_sum"
    ...
    require(
        representative_shifted_minor_exclusion["schema_version"]
        == "f17-32-m3-representative-shifted-minor-exclusion-v1",
        "representative shifted-minor exclusion schema mismatch",
    )
    require(
        representative_shifted_minor_exclusion["packet_count"] == 6
        and representative_shifted_minor_exclusion["row_shift_tested"] == 1
        and representative_shifted_minor_exclusion["aggregate"][
            "representative_ranks"
        ]
        == [6, 7, 8, 9, 10, 11]
        and representative_shifted_minor_exclusion["aggregate"][
            "first_minor_root_total"
        ]
        == 18
        and representative_shifted_minor_exclusion["aggregate"][
            "cleared_root_total"
        ...
        == 18
        and representative_shifted_minor_exclusion["aggregate"][
            "all_representative_roots_excluded_as_support_witnesses"
        ]
        is True
        and representative_shifted_minor_exclusion["aggregate"][
    ...
    require(
        low_rank6_11_shifted_minor_exclusion["agreement_range"]
        == [AGREEMENT_MIN, AGREEMENT_MAX]
        and low_rank6_11_shifted_minor_exclusion["ranks"]
        == [6, 7, 8, 9, 10, 11]
        and low_rank6_11_shifted_minor_exclusion["row_shift_tested"] == 1
        and low_rank6_11_shifted_minor_exclusion["aggregate"][
            "source_record_count"
        ]
        == 252
        and low_rank6_11_shifted_minor_exclusion["aggregate"][
            "root_bearing_record_count"
        ]
        == 158
        and low_rank6_11_shifted_minor_exclusion["aggregate"][
            "finite_root_total"
    ...
    require(
        one_spike_window_full_hankel["schema_version"]
        == "f17-32-m3-one-spike-window-full-hankel-v1",
        "one-spike window full-Hankel schema mismatch",
    )
    require(
        one_spike_window_full_hankel["agreement_range"]
        == [AGREEMENT_MIN, AGREEMENT_MAX]
        and one_spike_window_full_hankel["aggregate"]["record_count"] == 42
        and one_spike_window_full_hankel["deterministic_records"][
            "record_count"
        ]
        == 42
        and one_spike_window_full_hankel["aggregate"][
            "finite_regular_root_count_sum"
        ]
        == 42
        and one_spike_window_full_hankel["aggregate"][
        ...
        == 0
        and one_spike_window_full_hankel["aggregate"][
            "projective_endpoint_contribution_sum"
        ]
        == 42
        and one_spike_window_full_hankel["aggregate"][
            "endpoint_quotient_image_witness_sum"
        ]
        == 42
        and one_spike_window_full_hankel["aggregate"][
            "max_regular_projective_upper_per_record"
        ...
        == {"0": 42}
        and one_spike_window_full_hankel["aggregate"][
            "all_finite_roots_excluded_by_shifted_minor"
        ]
        is True
        and one_spike_window_full_hankel["aggregate"][
            "all_projective_endpoints_have_quotient_image_witness"
        ...
        is True,
        "one-spike window full-Hankel aggregate mismatch",
    )
    one_spike_items = one_spike_window_projective_packet["exact_agreements"]
    require(
        one_spike_window_projective_packet["schema_version"]
        == "aperiodic-hankel-eliminant-v1",
        "one-spike window projective packet schema mismatch",
    )
    require(
        one_spike_window_projective_packet["status"] == "PROVED / AUDIT"
        and one_spike_window_projective_packet["sampler"] == "projective_line"
        and one_spike_window_projective_packet["agreement_threshold"]
        == AGREEMENT_MIN
        and len(one_spike_items) == AGREEMENT_MAX - AGREEMENT_MIN + 1
        and [item["A"] for item in one_spike_items]
        == list(range(AGREEMENT_MIN, AGREEMENT_MAX + 1))
        and all(item["status"] == "regular_minor" for item in one_spike_items)
        and all(item["regular_minor"]["degree"] == 1 for item in one_spike_items)
        and all(
            item["projective_infinity"]["status"] == "nonempty"
            and item["projective_infinity"]["contribution"] == 1
            for item in one_spike_items
        )
        and len(one_spike_window_projective_packet["root_union"]) == 42
        and one_spike_window_projective_packet["declared_aperiodic_numerator"]
        == 43
        and one_spike_window_projective_packet["aggregate"][
            "finite_root_union_size"
        ]
        == 42
        and one_spike_window_projective_packet["aggregate"][
            "projective_endpoint_union_contribution"
        ]
        == 1
        and one_spike_window_projective_packet["aggregate"][
...
def build_status() -> dict[str, Any]:
    ...
    one_spike_window_full_hankel = load_json(ONE_SPIKE_WINDOW_FULL_HANKEL_REF)
    ...
    artifacts = [
        ...
        artifact_record(
            "synthetic_one_spike_window_full_hankel",
            ONE_SPIKE_WINDOW_FULL_HANKEL_REF,
            "f17-32-m3-one-spike-window-full-hankel-v1",
        ),
        artifact_record(
    ...
    return {
        ...
        "summary": {
            ...
            "synthetic_representative_shifted_minor_exclusion_status": (
                "proved that the 18 listed finite roots in the representative "
                "rank-6..11 projective-line packets are first-minor artifacts: "
                "the row-shift-1 square minor is nonzero at every listed root"
            ),
            "synthetic_representative_shifted_minor_exclusion_roots_cleared": (
                representative_shifted_minor_exclusion["aggregate"][
                    "cleared_root_total"
                ]
            ),
            "synthetic_representative_shifted_minor_exclusion_nonclaim": (
            ...
            "synthetic_low_rank6_11_shifted_minor_exclusion_status": (
                ...
                "row-shift-1 artifacts, hence not actual full-Hankel "
                "exact-support witnesses"
            ),
            "synthetic_low_rank6_11_shifted_minor_exclusion_roots_cleared": (
                low_rank6_11_shifted_minor_exclusion["aggregate"][
                    "cleared_root_total"
                ]
            ),
            "synthetic_low_rank6_11_shifted_minor_exclusion_root_bearing_records": (
                low_rank6_11_shifted_minor_exclusion["aggregate"][
                    "root_bearing_record_count"
                ]
            ),
            "synthetic_low_rank6_11_shifted_minor_exclusion_nonclaim": (
            ...
            "synthetic_one_spike_window_full_hankel_status": (
                "proved a non-proportional one-spike branch across the whole "
                "M3 regular window: every A=385..426 has one finite "
                "first-minor root, the row-shift-1 minor excludes that root "
                "from the full-Hankel witness column, and the remaining "
                "[0:1] endpoint is charged to quotient-image"
            ),
            "synthetic_one_spike_window_finite_roots": (
                one_spike_window_full_hankel["aggregate"][
                    "finite_regular_root_count_sum"
                ]
            ),
            "synthetic_one_spike_window_finite_roots_excluded_by_shifted_minor": (
                one_spike_window_full_hankel["aggregate"][
                    "finite_regular_roots_excluded_by_shifted_minor_sum"
                ]
            ),
            "synthetic_one_spike_window_max_regular_projective_upper": (
                one_spike_window_full_hankel["aggregate"][
                    "max_regular_projective_upper_per_record"
                ]
            ),
            "synthetic_one_spike_window_max_full_hankel_before_endpoint_image": (
                one_spike_window_full_hankel["aggregate"][
                    "max_full_hankel_projective_upper_before_endpoint_image_per_record"
                ]
            ),
            "synthetic_one_spike_window_max_aperiodic_residual": (
                one_spike_window_full_hankel["aggregate"][
                    "max_aperiodic_full_hankel_projective_upper_per_record"
                ]
            ),
            "synthetic_one_spike_window_nonclaim": (
                "synthetic one-spike branch only; not arbitrary M3 row data "
                "and not a finite-root quotient-image/support audit"
            ),
            "synthetic_one_spike_window_projective_packet_status": (
                "v9 projective-line packet covers the non-proportional "
                "one-spike branch for every A=385..426: each row has one "
                "finite affine root and the shared [0:1] projective endpoint"
            ),
            "synthetic_one_spike_window_projective_packet_numerator": (
                one_spike_window_projective_packet["declared_aperiodic_numerator"]
            ),
            "synthetic_one_spike_window_projective_packet_finite_union": (
                one_spike_window_projective_packet["aggregate"][
                    "finite_root_union_size"
                ]
            ),
            "synthetic_one_spike_window_projective_packet_endpoint_union": (
                one_spike_window_projective_packet["aggregate"][
                    "projective_endpoint_union_contribution"
                ]
            ),
            "synthetic_one_spike_window_projective_packet_nonclaim": (
                "synthetic v9 packet only; the projective-line numerator is "
...
def print_summary(status: dict[str, Any]) -> None:
    ...
    print(
        "one-spike window full-Hankel: "
        f"{summary['synthetic_one_spike_window_full_hankel_status']}"
    )
    print(
