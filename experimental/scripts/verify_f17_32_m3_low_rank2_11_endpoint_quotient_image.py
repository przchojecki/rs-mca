#!/usr/bin/env python3
r"""Verify a quotient-image witness for M3 low-rank projective endpoints.

The projective endpoint [0:1] in the synthetic low-rank ladder is witnessed on
``D \ Y``.  That particular support is not a quotient-remainder support, but
quotient-image accounting only asks whether the same projective parameter has
some quotient-remainder witness support.  This verifier constructs one.

For every rank/agreement row at ranks 2..11, it builds an agreement-size
quotient-remainder support using c=2 fibers that avoids the update block Y.
The co-support T therefore contains Y, so the endpoint syndrome v lies in W_T.
The base syndrome u does not lie in W_T by Vandermonde independence, because
|X|=j+1, |T|=j, and |X union T|<=255<=n-k.
"""
...
ENDPOINT_QUOTIENT_SUPPORT_CERT = REPO_ROOT / (
...
def validate_inputs(
    ...
    require(
        ...
        and endpoint["aggregate"]["all_endpoint_noncontainment_checks_pass"]
        is True,
        "endpoint aggregate mismatch",
    )
    require(
        quotient_support["schema_version"]
        == "f17-32-m3-low-rank2-11-endpoint-quotient-support-v1",
        "endpoint quotient-support schema mismatch",
    )
    require(
        quotient_support["aggregate"][
...
def quotient_remainder_support_avoiding_y(
    ...
    require(support.isdisjoint(update_exponents), "support hits Y")
    return {
            ...
            records.append(
                {
                    ...
                    "endpoint_image_audit": {
                        "v_explained_on_quotient_support": True,
                        "reason_v": (
                            "the quotient co-support contains Y, and Syn(g)=v "
                            "lies in the parity-column span W_Y"
                        ),
                        "u_explained_on_quotient_support": False,
                        "reason_u": (
                            "Syn(f)=u lies in W_X with |X|=j+1, while the "
                            "quotient co-support has size j; Vandermonde "
                            "independence on X union T rules out containment"
                        ),
...
def build_certificate() -> dict[str, Any]:
    ...
    quotient_support = load_json(ENDPOINT_QUOTIENT_SUPPORT_CERT)
    validate_inputs(descriptor, endpoint, quotient_support)
    ...
    return {
        ...
        "source_artifacts": {
            ...
            "endpoint_quotient_support_exclusion": {
                "ref": str(ENDPOINT_QUOTIENT_SUPPORT_CERT.relative_to(REPO_ROOT)),
        ...
        "method": {
            ...
            "image_map": (
                ...
                "explained there by v; because |X|=j+1 and |T|=j with "
                "|X union T|<=n-k, u is not explained there"
            ),
            "relation_to_support_exclusion": (
                "the minimal endpoint support D minus Y is not itself a "
                "quotient-remainder support, but the same projective parameter "
                "has a different quotient-remainder witness support"
            ),
        },
        "aggregate": aggregate,
        ...
        "nonclaims": [
            ...
            "does not claim the minimal support D minus Y is quotient-remainder",
