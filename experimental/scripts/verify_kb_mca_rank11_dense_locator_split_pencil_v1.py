...
def cross_support_terminal_caps(
    quotient: int,
    mprime: int,
    source: int,
    defect: int,
    ...
    branch = dict(inherited)
    branch[source] = min(
        branch[source],
        completion_deletion_cap(mprime, source, quotient - defect),
    )
    for target in range(2, 10):
...
def descending_support_leaf_caps(kprime: int) -> dict[str, dict[int, int]]:
    quotient = kprime - 10
    mprime = 67472 + kprime
    inherited = {
        support: (
    ...
    for source in (5, 4, 3, 2):
        for defect in range(10 - source):
            ...
            leaves[label] = cross_support_terminal_caps(
                quotient, mprime, source, defect, inherited
            )
        inherited = dict(inherited)
        inherited[source] = min(
            inherited[source],
            completion_deletion_cap(mprime, source, quotient - (10 - source)),
        )
...
def completion_branch_premiums(kprime: int, refine_support6: bool) -> tuple[int, dict[str, int]]:
    quotient = kprime - 10
    mprime = 67472 + kprime
    weights = {support: comb(11 - support, 2) for support in range(2, 10)}
    parents = descending_support_leaf_caps(kprime)
    leaves: dict[str, dict[int, int]] = {}
    for label, inherited in parents.items():
        ...
        for defect in range(4):
            leaves[f"{label}__c6_defect_{defect}"] = cross_support_terminal_caps(
                quotient, mprime, 6, defect, inherited
            )
        fallback = dict(inherited)
        fallback[6] = min(
            fallback[6], completion_deletion_cap(mprime, 6, quotient - 4)
        )
        leaves[f"{label}__c6_fallback"] = fallback
    branches = {
        label: sum(weights[support] * caps[support] for support in range(2, 10))
        for label, caps in leaves.items()
    }
    uncoupled, _ = cross_support_branch_premiums(kprime)
    return uncoupled, branches


def completion_branch_payment_row(
    ...
    marks = comb(nprime, 9) * chart
    kernel = refined_kernel_capacity(kprime)
    uncoupled, branches = completion_branch_premiums(kprime, refine_support6)
...
def support4_external_support_count(
    ...
    return comb(carrier, 4) + sum(
        comb(carrier, 4 - external)
        * comb(outside, external - 1)
        * (quotient_defect + 4 - external)
        // external
        for external in range(1, 5)
    )


def support4_external_incidence_cap(
    ...
    for quotient_defect in range(min(support4_defect, support5_defect) + 1):
        for zero_dimension in range(4, 7):
            support_count = support4_external_support_count(
                kprime, mprime, zero_dimension, quotient_defect
            )
            candidates.append(
                (
                    support_count * comb(mprime - 4, 7),
                    zero_dimension,
                    quotient_defect,
                    support_count,
                )
...
def full_completion_source_options(
    ...
    fallback[support] = min(
        fallback[support],
        completion_deletion_cap(mprime, support, quotient - (10 - support)),
    )
...
@cache
def full_completion_branch_summary(kprime: int) -> dict[str, Any]:
    quotient = kprime - 10
    mprime = 67472 + kprime
    ...
    option_table = {
        support: full_completion_source_options(kprime, support, baseline)
        for support in supports
    }
    ...
    for choices in itertools.product(*(option_table[support] for support in supports)):
        ...
        if joint:
            ...
            require(
                ...
                "support-four/support-five overlap condition",
            )
            joint_cap = support4_external_incidence_cap(
...
@cache
def full_completion_payment_row(kprime: int, records: int) -> dict[str, Any]:
    nprime = 1048576 + kprime
    mprime = 67472 + kprime
    core_rows = {
        core: integral_core_offset_row(kprime, core)["chart"]
    ...
    marks = comb(nprime, 9) * chart
    kernel = refined_kernel_capacity(kprime)
    summary = full_completion_branch_summary(kprime)
...
def deep_exact_source_caps(
    ...
    if defect <= 9 - support:
        return cross_support_terminal_caps(
            quotient, mprime, support, defect, baseline
        )
    caps = dict(baseline)
    caps[support] = min(
        caps[support],
        completion_deletion_cap(mprime, support, quotient - defect),
    )
    return caps


def deep_other_branch_vectors(
    ...
    for choices in itertools.product(
        *(full_completion_source_options(kprime, support, baseline)
        ...
        for support, (label, _defect, local_caps) in zip(
            ...
            for target in supports:
                caps[target] = min(caps[target], local_caps[target])
        vectors[tuple(caps[target] for target in supports)] = "/".join(labels)
    return vectors


def deep_maximal_other_vectors(
    kprime: int, baseline: dict[int, int]
) -> tuple[dict[tuple[int, ...], str], list[tuple[str, tuple[int, ...]]]]:
    vectors = deep_other_branch_vectors(kprime, baseline)
    maximal = []
    for vector in vectors:
        ...
        if not dominated:
            maximal.append((vectors[vector], vector))
    maximal.sort()
    return vectors, maximal


def deep_full_label(other_label: str, support4_defect: int, support5_defect: int) -> str:
...
@cache
def deep_completion_branch_summary(kprime: int) -> dict[str, Any]:
    quotient = kprime - 10
    mprime = 67472 + kprime
    supports = tuple(range(2, 10))
    other_supports = (2, 3, 6, 7, 8, 9)
    ...
    all_other, maximal_other = deep_maximal_other_vectors(kprime, baseline)
    ...
    for support4_defect in range(quotient + 1):
        ...
        for support5_defect in range(quotient + 1):
            ...
            if joint:
                ...
                if joint_cap < pair_caps[2]:
                    tightened_pairs += 1
                pair_caps[2] = min(pair_caps[2], joint_cap)
            ...
            for other_label, other_caps in maximal_other:
                ...
                premium = sum(
                    weights[target] * final_caps[index]
                    for index, target in enumerate(supports)
                )
                if premium > pair_maximum[0]:
                    pair_maximum = (premium, other_label, final_caps)
            label = deep_full_label(
                pair_maximum[1], support4_defect, support5_defect
            )
            pair_digest.update(
                f"{support4_defect},{support5_defect}:"
                f"{pair_maximum[0]}:{pair_maximum[1]}\n".encode()
            )
            pair_sum += pair_maximum[0]
            if pair_maximum[0] > maximum[0]:
                maximum = (
                    pair_maximum[0],
                    label,
                    support4_defect,
            ...
            if joint and pair_maximum[0] > maximum_joint[0]:
                maximum_joint = (pair_maximum[0], label)
            if not joint and pair_maximum[0] > maximum_nonjoint[0]:
                maximum_nonjoint = (pair_maximum[0], label)

    exact_pairs = (quotient + 1) ** 2
    raw_other = prod(
        len(full_completion_source_options(kprime, support, baseline))
        for support in other_supports
    )
    return {
        "exact_pair_count": exact_pairs,
        "joint_pair_count": joint_pairs,
        "nonjoint_pair_count": exact_pairs - joint_pairs,
        "joint_tightened_pair_count": tightened_pairs,
        "other_raw_branch_count": raw_other,
        ...
        "raw_leaf_count": exact_pairs * raw_other,
...
@cache
def deep_completion_payment_row(kprime: int, records: int) -> dict[str, Any]:
    nprime = 1048576 + kprime
    mprime = 67472 + kprime
    core_rows = {
        core: integral_core_offset_row(kprime, core)["chart"]
...
def cross_support_collision_count(
    kprime: int,
    mprime: int,
    source_support: int,
    target_support: int,
    ...
    quotient = kprime - 10
    require(2 <= source_support <= 5, "cross-collision source")
    require(
        2 <= target_support <= 9 and source_support + target_support <= 11,
        "cross-collision target",
    )
    require(0 <= defect < quotient, "cross-collision nonempty defect")
    carrier = quotient + source_support - 1 - defect
    if defect == 0:
        return comb(carrier, target_support)
    outside = mprime - carrier
    return comb(carrier, target_support) + sum(
        comb(carrier, target_support - external)
        * comb(outside, external - 1)
        * (defect + target_support - external)
        // external
        for external in range(1, target_support + 1)
    )


def cross_support_collision_cap(
    kprime: int,
    mprime: int,
    source_support: int,
    target_support: int,
    ...
    return cross_support_collision_count(
        kprime,
        mprime,
        source_support,
        target_support,
...
def cross_collision_group(
    ...
    rows: dict[tuple[int, ...], str] = {}
    for left_defect in range(quotient + 1):
        left_caps = exact_cross_collision_caps(
            kprime, left_support, left_defect, baseline
        )
        for right_defect in range(quotient + 1):
            right_caps = exact_cross_collision_caps(
                kprime, right_support, right_defect, baseline
            )
            vector = [
                min(baseline[target], left_caps[target], right_caps[target])
...
@cache
def cross_collision_branch_summary(kprime: int) -> dict[str, Any]:
    quotient = kprime - 10
    mprime = 67472 + kprime
    supports = tuple(range(2, 10))
    weights = {support: comb(11 - support, 2) for support in supports}
    ...
    raw23, maximal23 = cross_collision_group(kprime, baseline, 2, 3)
    raw45, maximal45 = cross_collision_group(kprime, baseline, 4, 5)
    raw69, maximal69 = high_support_group(kprime, baseline)
    maximum = (-1, "", ())
    premium_sum = 0
    stream = hashlib.sha256()
...
@cache
def cross_collision_payment_row(kprime: int, records: int) -> dict[str, Any]:
    nprime = 1048576 + kprime
    mprime = 67472 + kprime
    core_rows = {
        core: integral_core_offset_row(kprime, core)["chart"]
    ...
    marks = comb(nprime, 9) * chart
    kernel = refined_kernel_capacity(kprime)
    summary = cross_collision_branch_summary(kprime)
    premium = summary["completion_premium"]
    full_rank = (marks + records * premium) // 55
    total = kernel + full_rank
    demand = records * comb(mprime, 11) - comb(nprime, 11)
...
def expected() -> dict[str, Any]:
    ...
    descending_support_payment_rows = {
        str(kprime): completion_branch_payment_row(
            kprime, refined_record_cap, False
        )
        for kprime in (43, 44)
    }
    branch_lattice_payment_rows = {
        str(kprime): completion_branch_payment_row(
            kprime, refined_record_cap, True
        )
        for kprime in (44, 45)
    }
    full_completion_payment_rows = {
        str(kprime): full_completion_payment_row(kprime, refined_record_cap)
        for kprime in (45, 46)
    }
    deep_completion_payment_rows = {
    ...
    return {
        ...
        "rank11_k43_descending_support_ladder_payment": {
            ...
            "source_order": [5, 4, 3, 2],
            "branch_count": 27,
            **descending_support_payment_rows["43"],
            "K44_method_wall": {
                key: value
                for key, value in descending_support_payment_rows["44"].items()
                if key
                not in {
                    "isolated_global_cap",
                    "uncoupled_completion_premium",
            ...
            | {
                "capacity_excess": -descending_support_payment_rows["44"]["gap"]
            },
        ...
        "rank11_k44_branch_lattice_payment": {
            ...
            "parent_source_order": [5, 4, 3, 2],
            "refined_parent_leaves": ["c5_defect_2", "c5_defect_3"],
            "refinement_source_support": 6,
            "parent_branch_count": 27,
            "refined_branch_count": 35,
            **branch_lattice_payment_rows["44"],
            "K45_method_wall": {
                key: value
                for key, value in branch_lattice_payment_rows["45"].items()
                if key
                not in {
                    "isolated_global_cap",
                    "uncoupled_completion_premium",
            ...
            | {
                "capacity_excess": -branch_lattice_payment_rows["45"]["gap"]
            },
        ...
        "sparse_circuit_support4_external_charge": {
            ...
            "support": 4,
            "zero_closure_dimension_range": [4, 6],
            "quotient_defect_range": "0<=delta<=min(s_4,s_5)",
            "carrier_size": "b=K-t-delta",
            "outside_size": "N=m-b",
            "inside_support_count": "C(b,4)",
            "outside_stratum_count": (
                "floor(C(b,4-j)C(N,j-1)(delta+4-j)/j)"
            ),
            "incidence_multiplier": "C(m-4,7)",
            "K45_caps_by_minimum_defect": {
                str(defect): {
                    "t": support4_external_incidence_cap(
                        45, 67517, defect, defect
                    )[1],
                    "delta": support4_external_incidence_cap(
                        45, 67517, defect, defect
                    )[2],
                    "b": 45
                    - support4_external_incidence_cap(45, 67517, defect, defect)[1]
                    - support4_external_incidence_cap(45, 67517, defect, defect)[2],
                    "support_count": support4_external_incidence_cap(
                        45, 67517, defect, defect
                    )[3],
                    "incidence_cap": support4_external_incidence_cap(
                        45, 67517, defect, defect
                    )[0],
                }
                for defect in range(5)
        ...
        "rank11_k45_full_completion_product_payment": {
            ...
            "residual_record_floor": refined_record_cap,
            "source_support_order": list(range(2, 10)),
            "branch_stream_format": (
            ...
            "K45": full_completion_payment_rows["45"],
            "K46_method_wall": {
                key: value
                for key, value in full_completion_payment_rows["46"].items()
                if key != "gap"
            }
            | {"capacity_excess": -full_completion_payment_rows["46"]["gap"]},
        ...
        "sparse_circuit_support45_deep_defect_partition": {
            ...
            "pareto_rule": (
                "discard a only if another b satisfies a_c<=b_c "
                "for every support"
            ),
            "rows": {
                str(kprime): {
                    "q": kprime - 10,
                    "exact_pair_count": (kprime - 9) ** 2,
                    "joint_eligible_pair_count": (kprime - 10) * (kprime - 9) // 2,
                    "nonjoint_pair_count": (kprime - 9) * (kprime - 8) // 2,
                    "other_support_option_product": 8640,
                    "raw_leaf_count": (kprime - 9) ** 2 * 8640,
                }
                for kprime in range(46, 55)
            },
        },
        "rank11_k46_k53_deep_joint_completion_payment": {
            "closed_rows": list(range(46, 54)),
            "new_closed_prefix": [10, 53],
            "first_method_wall": 54,
            "residual_record_floor": refined_record_cap,
            "exact_support_defects": [4, 5],
            "other_support_order": [2, 3, 6, 7, 8, 9],
            "deficit_weights": {
                str(support): comb(11 - support, 2)
                for support in range(2, 10)
            },
            "rows": {
                **{
                    str(kprime): deep_completion_payment_rows[str(kprime)]
                    for kprime in range(46, 54)
                },
                "54": {
                    key: value
                    for key, value in deep_completion_payment_rows["54"].items()
                    if key != "gap"
                }
                | {
                    "capacity_excess": -deep_completion_payment_rows["54"]["gap"]
                },
            },
        ...
        "sparse_circuit_cross_support_collision_charge": {
            ...
            "K60_defect25_samples": {
                str(source): {
                    str(target): {
                        "intersection_dimension": 12 - source - target,
                        "source_carrier_size": 50 + source - 1 - 25,
                        "target_outside_budget": 25 + target - 1,
                        "target_support_count": cross_support_collision_count(
                            60, 67532, source, target, 25
                        ),
                        "target_incidence_cap": cross_support_collision_cap(
                            60, 67532, source, target, 25
                        ),
                    }
                    for target in range(2, 10)
        ...
        "rank11_k60_k70_cross_support_collision_payment": {
            "closed_rows": list(range(60, 71)),
            "new_closed_prefix": [10, 70],
            "first_method_wall": 71,
            "residual_record_floor": refined_record_cap,
            ...
            "rows": {
                **{
                    str(kprime): cross_collision_payment_rows[str(kprime)]
                    for kprime in range(60, 71)
                },
                "71": {
                    key: value
                    for key, value in cross_collision_payment_rows["71"].items()
                    if key != "gap"
                }
                | {
                    "capacity_excess": -cross_collision_payment_rows["71"]["gap"]
                },
            },
...
def validate(value: object, wanted: dict[str, Any] | None = None) -> dict[str, int]:
    ...
    descending_ladder = value["sparse_circuit_descending_support_completion_ladder"]
    k43_ladder = value["rank11_k43_descending_support_ladder_payment"]
    branch_lattice = value["sparse_circuit_completion_branch_lattice_refinement"]
    k44_branch = value["rank11_k44_branch_lattice_payment"]
    joint_zero_carrier = value["sparse_circuit_support45_joint_zero_carrier"]
    support4_external_charge = value["sparse_circuit_support4_external_charge"]
    ...
    require(
        ...
        "descending-support ladder constants",
    )
    row43_ladder = completion_branch_payment_row(
        43, k41_sharp["residual_record_floor"], False
    )
    row44_ladder = completion_branch_payment_row(
        44, k41_sharp["residual_record_floor"], False
    )
    require(
        ...
        and k43_ladder["branch_count"] == len(row43_ladder["branch_premiums"]) == 27
        and all(k43_ladder[key] == row43_ladder[key] for key in row43_ladder)
        and row43_ladder["active_branch"] == "c5_defect_2"
        and row43_ladder["completion_premium"]
        == 39510045591272162389536743615445318852720199164
        and row43_ladder["gap"]
        == 4456829341030748859349785682161828589177837723939653522311506
        and row43_ladder["record_coefficient_cross"] > 0
        and row43_ladder["floor_record_raw_cross"] > 0
        and all(
            k43_ladder["K44_method_wall"][key] == row44_ladder[key]
            for key in row44_ladder
            if key
            not in {
                "isolated_global_cap",
                "uncoupled_completion_premium",
        ...
        and k43_ladder["K44_method_wall"]["capacity_excess"]
        == -row44_ladder["gap"]
        == 1729575114830772639212937201922834766923716849296098844264209
        and row44_ladder["floor_record_raw_cross"] < 0
        and k43_ladder["remaining_rank9_interval"] == [44, 15528],
        "descending-support K'=43 payment and K'=44 wall",
    )
    require(
        branch_lattice
        == {
        ...
        "completion branch-lattice constants",
    )
    row44_branch = completion_branch_payment_row(
        44, k41_sharp["residual_record_floor"], True
    )
    row45_branch = completion_branch_payment_row(
        45, k41_sharp["residual_record_floor"], True
    )
    require(
        ...
        == len(row44_branch["branch_premiums"])
        == 35
        and all(k44_branch[key] == row44_branch[key] for key in row44_branch)
        and row44_branch["active_branch"] == "c5_defect_2__c6_defect_2"
        and row44_branch["completion_premium"]
        == 40318474413130846902399237147930487840413149400
        and row44_branch["gap"]
        == 535634409944931896502583174279429171539133692200964798538249
        and row44_branch["record_coefficient_cross"] > 0
        and row44_branch["floor_record_raw_cross"] > 0
        and all(
            k44_branch["K45_method_wall"][key] == row45_branch[key]
            for key in row45_branch
            if key
            not in {
                "isolated_global_cap",
                "uncoupled_completion_premium",
        ...
        and k44_branch["K45_method_wall"]["capacity_excess"]
        == -row45_branch["gap"]
        == 5651502053446174523626296867091469400380654135040887972894842
        and row45_branch["floor_record_raw_cross"] < 0
        and k44_branch["remaining_rank9_interval"] == [45, 15528],
        "branch-lattice K'=44 payment and K'=45 wall",
    )
    require(
        value["claims"]["rank9_k43_descending_support_ladder_closed_K_prime"] == 43
        and value["claims"]["rank9_k44_branch_lattice_closed_K_prime"] == 44,
        "branch-lattice rank-nine closure claims",
    )
    require(
        joint_zero_carrier
        == {
    ...
    for support4_defect in range(6):
        for support5_defect in range(5):
            require(
                35 > support4_defect + support5_defect,
                "K'=45 joint overlap condition",
            )
            source_overlap = (
            ...
            require(
                source_overlap == 35 - support4_defect - support5_defect > 0,
                "joint source-carrier overlap",
            )
            joint_dimension_checks += 1
    expected_external_caps = {}
    for defect in range(5):
        cap, zero_dimension, quotient_defect, support_count = (
            support4_external_incidence_cap(45, 67517, defect, defect)
        )
        expected_external_caps[str(defect)] = {
            "t": zero_dimension,
    ...
    require(
        ...
        == {
            ...
            "incidence_multiplier": "C(m-4,7)",
            "K45_caps_by_minimum_defect": expected_external_caps,
        },
        "support-four external charge",
    )
    row45_full = full_completion_payment_row(
        45, k41_sharp["residual_record_floor"]
    )
    row46_full = full_completion_payment_row(
        46, k41_sharp["residual_record_floor"]
    )
    row46_wall = dict(row46_full)
    row46_wall["capacity_excess"] = -row46_wall.pop("gap")
    require(
        k45_full_product["closed_row"] == 45
        and k45_full_product["new_closed_prefix"] == [10, 45]
        and k45_full_product["first_method_wall"] == 46
        and k45_full_product["residual_record_floor"]
        == k41_sharp["residual_record_floor"]
        and k45_full_product["source_support_order"] == list(range(2, 10))
        and k45_full_product["joint_cap_condition"]
        == "support4 and support5 both terminal"
        and k45_full_product["K45"] == row45_full
        and row45_full["leaf_count"] == 362880
        and row45_full["joint_branch_count"] == 259200
        and row45_full["joint_tightened_count"] == 48384
        and row45_full["active_branch"]
        == "c2F/c3F/c4F/c5F/c6F/c7F/c8F/c9F"
        and row45_full["completion_premium"]
        == 40126324034612056409620566967689123241580103372
        and row45_full["gap"]
        == 1616971801308361526826641488053709685917408248376428345137933
        and row45_full["premium_ceiling_margin"] > 0
        and row45_full["floor_record_raw_cross"] > 0
        and k45_full_product["K46_method_wall"] == row46_wall
        and row46_wall["active_branch"]
        == "c2F/c3F/c4F/c5F/c6F/c7F/c8F/c9F"
        and row46_wall["capacity_excess"]
        == 5057508862309072579343840146913199075599800084788396842011438
        and row46_wall["premium_ceiling_margin"] < 0
        and row46_wall["floor_record_raw_cross"] < 0
        and k45_full_product["remaining_rank9_interval"] == [46, 15528],
        "full completion-product K'=45 payment and K'=46 wall",
    )
    require(
        value["claims"]["rank9_k45_full_completion_product_closed_K_prime"]
        == 45,
        "full completion-product closure claim",
    )
    require(
        ...
        and deep_partition["exact_defect_range"] == "0<=s_c<=q"
        and deep_partition["exact_pair_count"] == "(q+1)^2"
        and deep_partition["joint_condition"] == "s_4+s_5<q"
        and deep_partition["joint_pair_count"] == "q(q+1)/2",
        "deep support-four/support-five partition",
    )
    deep_pair_checks = 0
    for kprime in range(46, 55):
        quotient = kprime - 10
        ...
        require(
            ...
            == {
                "q": quotient,
                "exact_pair_count": (quotient + 1) ** 2,
                "joint_eligible_pair_count": quotient * (quotient + 1) // 2,
                "nonjoint_pair_count": (quotient + 1) * (quotient + 2) // 2,
                "other_support_option_product": 8640,
                "raw_leaf_count": (quotient + 1) ** 2 * 8640,
            },
            f"deep partition row K'={kprime}",
        )
        deep_pair_checks += partition_row["exact_pair_count"]

    minimum_deep_gap = None
    deep_raw_leaves = 0
    for kprime in range(46, 54):
        row = deep_completion_payment_row(
            kprime, k41_sharp["residual_record_floor"]
        )
        manifest_row = deep_payment["rows"][str(kprime)]
        active_defect = (kprime - 11) // 2
        require(manifest_row == row, f"deep payment row K'={kprime}")
        require(
            row["active_s4"] == row["active_s5"] == active_defect
            and row["active_branch"].endswith("c6F/c7F/c8F/c9F")
            and row["other_raw_branch_count"] == 8640
            and row["other_unique_vector_count"] == 1182
            and row["other_maximal_vector_count"] == 9
            and row["gap"] > 0
            and row["premium_ceiling_margin"] > 0
            and row["record_coefficient_cross"] > 0
            and row["floor_record_raw_cross"] > 0,
            f"deep payment signs K'={kprime}",
        )
        minimum_deep_gap = (
            row["gap"]
            if minimum_deep_gap is None
            else min(minimum_deep_gap, row["gap"])
        )
        deep_raw_leaves += row["raw_leaf_count"]
    row54 = deep_completion_payment_row(
        54, k41_sharp["residual_record_floor"]
    )
    row54_wall = dict(row54)
    row54_wall["capacity_excess"] = -row54_wall.pop("gap")
    require(
        deep_payment["closed_rows"] == list(range(46, 54))
        and deep_payment["new_closed_prefix"] == [10, 53]
        and deep_payment["first_method_wall"] == 54
        and deep_payment["residual_record_floor"]
        == k41_sharp["residual_record_floor"]
        and deep_payment["exact_support_defects"] == [4, 5]
        and deep_payment["other_support_order"] == [2, 3, 6, 7, 8, 9]
        and deep_payment["rows"]["54"] == row54_wall
        and minimum_deep_gap
        == 2503373059664320603163477388007627909210651834842589498907998
        and row54_wall["active_s4"] == row54_wall["active_s5"] == 21
        and row54_wall["capacity_excess"]
        == 2477882110233058360154706764229180240778698202487636349407165
        and row54_wall["premium_ceiling_margin"]
        == -495611154275787830253977941644262122450512788
        and row54_wall["floor_record_raw_cross"] < 0
        and deep_payment["remaining_rank9_interval"] == [54, 15528],
        "deep completion K'=46..53 payment and K'=54 wall",
    )
    require(
        value["claims"]["rank9_k46_k53_deep_joint_completion_closed_K_prime"]
        == 53,
    ...
    require(
        cross_collision_charge["correction_dimension"] == 10
        and cross_collision_charge["component_size"] == 11
        and cross_collision_charge["source_support_range"] == [2, 5]
        and cross_collision_charge["target_support_range"] == [2, 9]
        and cross_collision_charge["support_condition"] == "c+d<=11"
        and cross_collision_charge["completion_maximum"] == "M_c=q-s>0"
        and cross_collision_charge["source_carrier_size"] == "b=q+c-1-s"
        and cross_collision_charge["intersection_dimension"] == "12-c-d"
        and cross_collision_charge["target_outside_budget"] == "s+d-1"
        and cross_collision_charge["incidence_multiplier"] == "C(m-d,11-d)"
    ...
    for source in range(2, 6):
        for target in range(2, 10):
            ...
            count = cross_support_collision_count(
                60, 67532, source, target, 25
            )
            require(
                ...
                and cross_support_collision_count(
                    60, 67532, source, target, 0
                ) == comb(50 + source - 1, target),
                f"cross-collision sample c={source},d={target}",
            )
    ...
    for kprime in range(60, 71):
        row = cross_collision_payment_row(
            kprime, k41_sharp["residual_record_floor"]
        )
        declared = cross_collision_payment["rows"][str(kprime)]
        active = (kprime - 10 + 1) // 2
        require(declared == row, f"cross-collision payment row K'={kprime}")
        require(
            row["active_small_defects"]
            == {str(support): active for support in range(2, 6)}
            and row["active_branch"].endswith("c6F/c7F/c8F/c9F")
            and row["group_maximal_vector_counts"]["69"] == 7
            and row["gap"] > 0
            and row["premium_ceiling_margin"] > 0
            and row["record_coefficient_cross"] > 0
            and row["floor_record_raw_cross"] > 0,
            f"cross-collision payment signs K'={kprime}",
        )
        cross_collision_gaps.append(row["gap"])
        cross_collision_leaves += row["represented_raw_leaf_count"]
    cross_collision71 = cross_collision_payment_row(
    ...
    require(
        ...
        and cross_collision_payment["residual_record_floor"]
        == k41_sharp["residual_record_floor"]
        and cross_collision_payment["exact_source_defects"] == [2, 3, 4, 5]
        and cross_collision_payment["cross_support_condition"]
        == "c+d<=11 and s_c<q"
        and cross_collision_payment["high_support_order"] == [6, 7, 8, 9]
        ...
        "cross-support collision K'=60..70 payment and K'=71 wall",
    )
    require(
        value["claims"]["rank9_k60_k70_cross_support_collision_closed_K_prime"]
        == 70,
    ...
    return {
        **dense,
        ...
        "k43_descending_gap": k43_ladder["gap"],
        "k44_descending_excess": k43_ladder["K44_method_wall"]["capacity_excess"],
        "k44_branch_gap": k44_branch["gap"],
        "k45_branch_excess": k44_branch["K45_method_wall"]["capacity_excess"],
        "joint_zero_carrier_checks": joint_dimension_checks,
        "k45_full_product_leaves": row45_full["leaf_count"],
        "k45_full_product_gap": row45_full["gap"],
        "k46_full_product_excess": row46_wall["capacity_excess"],
        "deep_pair_checks": deep_pair_checks,
        ...
        "cross_collision_checks": cross_collision_checks,
...
def tamper_selftest(reference: dict[str, Any]) -> int:
    ...
    mutations = (
        ...
        lambda item: item["sparse_circuit_descending_support_completion_ladder"].__setitem__("total_leaf_count", 26),
        lambda item: item["rank11_k43_descending_support_ladder_payment"]["branch_premiums"].__setitem__("c5_defect_2", 0),
        lambda item: item["rank11_k43_descending_support_ladder_payment"]["K44_method_wall"].__setitem__("capacity_excess", 0),
        lambda item: item["sparse_circuit_completion_branch_lattice_refinement"]["support6_specialization"].__setitem__("replacement_leaf_count", 4),
        lambda item: item["rank11_k44_branch_lattice_payment"]["branch_premiums"].__setitem__("c5_defect_2__c6_defect_2", 0),
        lambda item: item["rank11_k44_branch_lattice_payment"]["K45_method_wall"].__setitem__("capacity_excess", 0),
        lambda item: item["sparse_circuit_support45_joint_zero_carrier"].__setitem__("intersection_dimension_lower_bound", 3),
        lambda item: item["sparse_circuit_support4_external_charge"]["K45_caps_by_minimum_defect"]["2"].__setitem__("incidence_cap", 0),
        lambda item: item["rank11_k45_full_completion_product_payment"]["K45"].__setitem__("branch_digest_sha256", "0" * 64),
        lambda item: item["rank11_k45_full_completion_product_payment"]["K46_method_wall"].__setitem__("capacity_excess", 0),
        lambda item: item["sparse_circuit_support45_deep_defect_partition"].__setitem__("empty_stratum_convention", "M_c=1"),
        ...
        lambda item: item["sparse_circuit_cross_support_collision_charge"].__setitem__("support_condition", "c+d<=12"),
        ...
        lambda item: item["claims"].__setitem__("rank9_k43_descending_support_ladder_closed_K_prime", 42),
...
def main() -> None:
    ...
    print(
        ...
        f"k43_descending_gap={result['k43_descending_gap']} "
        f"k44_descending_excess={result['k44_descending_excess']} "
        f"k44_branch_gap={result['k44_branch_gap']} "
        f"k45_branch_excess={result['k45_branch_excess']} "
        f"joint_zero_carrier_checks={result['joint_zero_carrier_checks']} "
        f"k45_full_product_leaves={result['k45_full_product_leaves']} "
        f"k45_full_product_gap={result['k45_full_product_gap']} "
        f"k46_full_product_excess={result['k46_full_product_excess']} "
        f"deep_pair_checks={result['deep_pair_checks']} "
        ...
        f"cross_collision_checks={result['cross_collision_checks']} "
