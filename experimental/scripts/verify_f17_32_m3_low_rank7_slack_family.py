#!/usr/bin/env python3
"""Verify the F_17^32 M3 rank-7 low-rank finite-slack family.

For each 385 <= A <= 426, put r=j+1=513-A and use the first r
descriptor-domain nodes as X, with the next seven descriptor nodes as Y.  The
low-rank template gives

    Delta_r(Z)=det(H_X) det(I+ZK),
    K_ab=sum_i L_i(y_a)L_i(y_b).

Rank 7 is beyond the v4 low-rank degree envelope: degree-only accounting gives
7 finite roots plus one possible projective infinity point.  This verifier
supplies an exact finite-root slack certificate for this synthetic family by
computing gcd(Delta_r(Z), Z^q-Z) over F_17^32.  The exact finite-root maximum
is 4, so the corrected projective bound is 4+1=5 <= 6.
"""
...
SCHEMA_VERSION = "f17-32-m3-low-rank7-slack-family-v1"
N = 512
K = 256
AGREEMENT_MIN = 385
AGREEMENT_MAX = 426
...
def low_rank_sidecar_from_prefix(
    ...
    sidecar = {
        "kind": "square_base_lagrange_kernel_rank7_finite_slack",
...
def build_records(
    ...
    require(
        UPDATE_RANK == max(finite_safe_ranks) + 1
        and UPDATE_RANK not in finite_safe_ranks
        and UPDATE_RANK not in projective_safe_ranks,
        "rank-7 is not recorded as just beyond the v4 low-rank degree envelope",
    )
    records = []
    base_nodes: list[tuple[int, ...]] = []
    ...
    for size in range(1, N - AGREEMENT_MIN + 2):
        ...
        require(
            ...
            f"A={agreement}: rank-7 polynomial is not degree 7",
        )
        require(
        ...
        if listed_roots is not None:
            ...
            require(
                root_count_value > 2,
                f"A={agreement}: small-degree roots should be listed",
            )
        records.append(
            {
                "A": agreement,
                ...
                "projective_infinity": {
                    ...
                    "reason": (
                        ...
                        "direction has rank 7 < j+1, so det H(v)=0 and this "
                        "regular minor does not exclude [0:1]; the nonzero "
                        "compressed degree-7 leading coefficient only controls "
...
def build_certificate() -> dict[str, Any]:
    ...
    require(max_finite_roots == 4, "unexpected rank-7 finite-root maximum")
    ...
    return {
        ...
        "construction": {
            ...
            "update_nodes": "next seven descriptor-domain elements",
        ...
        "method": {
            ...
            "finite_slack": (
                "exact finite roots are at most 4, so rank 7 supplies "
                "finite-root slack beyond the v4 degree envelope"
            ),
        ...
        "aggregate": {
            ...
            "polynomial_degree_histogram": {"7": len(records)},
...
def check_certificate(certificate: dict[str, Any], path: Path) -> None:
    ...
    if actual != expected:
        raise AssertionError(f"rank-7 low-rank slack family mismatch: {path}")


def print_summary(certificate: dict[str, Any]) -> None:
    aggregate = certificate["aggregate"]
    print("F_17^32 M3 low-rank-7 finite-slack family")
