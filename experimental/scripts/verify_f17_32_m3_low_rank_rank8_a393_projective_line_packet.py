#!/usr/bin/env python3
"""Build the M3 rank-8, A=393 projective-line low-rank v9 packet.

This is the rank-8 hard-row companion to the rank-7 A=393 packet.  Rank 8 is
two steps beyond the degree-only M3 projective envelope: the regular minor has
degree 8 and the projective endpoint may contribute one more point.  For A=393
the Frobenius gcd has four linear factors, so exact root slack gives
projective numerator 4+1=5 <= 6.
"""
...
PACKET_SCHEMA = "f17-32-m3-low-rank-rank8-a393-projective-line-v1"
N = 512
K = 256
A = 393
J = N - A
...
INPUT_PATH = REPO_ROOT / (
    ...
    "f17_32_n512_k256_a393_low_rank8_projective_line_input.json"
)
...
def rank8_record(slack: dict[str, Any]) -> dict[str, Any]:
    records = [record for record in slack["records"] if record["A"] == A]
    require(len(records) == 1, "expected exactly one rank-8 A=393 record")
    record = records[0]
    require(record["j"] == J and record["t"] == T, "rank-8 record mismatch")
    require(record["root_count"] == 4, "rank-8 A=393 should have four roots")
...
def validate_sources(
    ...
    require(
        slack["schema_version"] == "f17-32-m3-low-rank8-slack-family-v1",
        "rank-8 slack schema mismatch",
    )
    require(slack["agreement_range"] == [385, 426], "rank-8 slack window mismatch")
    ...
    require(
        ...
        "rank-8 slack aggregate mismatch",
    )
    require(
    ...
    require(
        ...
        == N - UPDATE_RANK,
        "rank-8 endpoint support mismatch",
    )
    ...
    return {
        ...
        "row": {
            ...
            "domain_description": (
                ...
                "synthetic M3 rank-8 low-rank update syndrome uses the first "
                "120 elements and the next 8 descriptor-domain elements"
            ),
        },
        "claim_scope": {
            ...
            "note": (
                "Rank-8 low-rank update replay input for the A=393 "
        ...
        "line_syndrome": {
            "description": (
                "synthetic M3 rank-8 low-rank update witness: "
                "u_m=sum_{x in X}x^m for the first 120 descriptor-domain "
                "elements and v_m=sum_{y in Y}y^m for the next 8 elements"
            ),
...
def build_packet(
    ...
    return {
        ...
        "row": {
            ...
            "domain_description": (
                "order-512 subgroup from the pinned F_17^32 row descriptor; "
                "synthetic rank-8 low-rank update syndrome at A=393"
            ),
        },
        "agreement_threshold": A,
        "sampler": "projective_line",
        ...
        "claim_scope": {
            ...
            "note": (
                ...
                "rank-8 low-rank row. It is a v9 replay packet, not an "
        ...
        "source_artifacts": {
            ...
            "rank8_slack_family": {
                "ref": str(LOW_RANK8_SLACK.relative_to(REPO_ROOT)),
                "sha256": file_sha256(LOW_RANK8_SLACK),
        ...
        "exact_agreements": [
            {
                ...
                "projective_infinity": {
                    ...
                    "reason": (
                        ...
                        "the original degree j+1. Since the rank-8 compressed "
                        "determinant has degree 8 < j+1=120, the top "
                ...
                "extractor_audit": {
                    "certificate_mode": "low_rank_update_bound",
                    "row_set_source": "low_rank_update_prefix_rank8",
                    "tested_row_sets": 1,
...
def build_objects() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    slack = load_json(LOW_RANK8_SLACK)
...
def print_summary(packet: dict[str, Any]) -> None:
    ...
    print("F_17^32 M3 rank-8 A=393 projective-line v9 packet")
...
def main() -> None:
    ...
    if args.check:
        check_file(INPUT_PATH, input_object, "rank-8 projective-line packet input")
        check_file(PACKET_PATH, packet, "rank-8 projective-line packet")
    if args.json:
