#!/usr/bin/env python3
"""Verify the A=385 fixed two-core global-component slope dichotomy."""
...
SCHEMA_VERSION = "f17-32-m3-rank6-a385-two-core-global-component-slope-dichotomy-v1"
Q_LINE = 17**32
TARGET_BITS = 128
...
def build_certificate() -> dict[str, Any]:
    ...
    require(
        ...
        "A385 two-core component-cut schema mismatch",
    )
    require(
        component_cut["summary"]["fixed_two_core_component_cut_branch_projective_safe"],
        "A385 two-core component-cut criterion unavailable",
    )
    require(
    ...
    require(
        ...
        "A=385 Q-space should be projective dimension four before base-core reduction",
    )
    ...
    residual_cases = [
        {
            ...
            "meaning": (
                "a fixed two-core global component carries a nonconstant "
                "rational slope map; it needs split-locator, quotient, or "
                "moving-slope incidence analysis"
            ),
        },
        {
    ...
    return {
        ...
        "status": "PROVED / AUDIT",
        "object": "A=385 separated rank-6 fixed two-core global-component slope dichotomy",
        "row": {
        ...
        "source_artifacts": {
            ...
            "a385_two_core_component_cut_safety": {
        ...
        "setup": {
            "residual_Q_space": (
                "after the fixed two-point base core, write Q=E R with "
                "deg R<3 and [R] in P^2"
            ),
            "direction_pairs": (
                "For each direction node y, set N_y(R)=Omega_y E(y)R(y) "
                "and D_y(R)=b_y L_{E R}(y); both are linear forms on the "
                "residual Q-plane."
            ),
            "pairwise_consistency_conics": (
                "C_{y,y'}(R)=N_y(R)D_{y'}(R)-N_{y'}(R)D_y(R), one for each "
                "pair of direction nodes."
            ),
            "global_component_residual": (
                "an irreducible component G in the residual P^2 contained in "
                "all pairwise direction-consistency conics after the fixed "
                "two-core component-cut packet"
            ),
        },
        "theorem": {
            "slope_map": (
            ...
            "finite_roots_factor_through_slope_map": (
                ...
                "slope z=N_y(R)/D_y(R) for the induced map, before the "
            ...
            "nonconstant_residual": (
                ...
                "a fixed two-core moving-slope global-component residual and is "
                "not closed by Bezout root counting alone."
            ),
            "slope_free_residual": (
        ...
        "checks": [
            ...
            "component-cut packet narrows to fixed two-core global components",
            "direction numerator and denominator forms are linear on the residual Q-plane",
            "pairwise conics identify a slope map independent of direction node",
            "constant slope map contributes at most one finite parameter off its base locus",
...
def check_certificate(path: Path, certificate: dict[str, Any]) -> None:
    ...
    if actual != expected:
        raise AssertionError(f"A=385 two-core global-component mismatch: {path}")


def print_summary(certificate: dict[str, Any]) -> None:
    summary = certificate["summary"]
    print("F_17^32 M3 rank-6 A=385 fixed two-core global component")
    print(
        "constant finite<={constant_slope_finite_root_upper_bound_off_base_locus}, projective total<={constant_slope_projective_total_upper_bound_off_base_locus}".format(
            **summary
        )
    )
