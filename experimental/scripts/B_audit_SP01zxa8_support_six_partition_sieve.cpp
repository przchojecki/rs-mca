...
struct Vec6 {
    ...
    auto operator<=>(const Vec6&) const = default;
};

Vec6 fraction_free_null_normal(const std::array<Vec6, ROWS>& input, bool& independent) {
    ...
    std::array<int, SUPPORT> permutation{0, 1, 2, 3, 4, 5};
...
std::uint32_t dot(const Vec6& left, const Vec6& right) {
...
bool admissible(const Vec6& value) {
    std::uint32_t sum = 0U;
...
void batch_projectivize(std::vector<Vec6>& values) {
    if (values.empty()) {
        return;
    }
...
std::uint64_t choose5(int value) {
    return value < 5 ? 0U
        : static_cast<std::uint64_t>(value) * (value - 1) * (value - 2)
          * (value - 3) * (value - 4) / 120U;
}
...
Result process_support(
    ...
    std::vector<Vec6> normals;
    normals.reserve(136695U);
    for (int group = 0; group <= FULL_GROUPS; ++group) {
    ...
    if (result.observed >= result.forced) {
        throw std::runtime_error("support-six candidate survives partition sieve");
    }
    return result;
}
...
void write_report(const std::string& path, const std::vector<Result>& results) {
    ...
    output << "{\n"
           << "  \"schema\": \"sp01zxa8-independent-support-six-audit/v1\",\n"
           << "  \"claim_id\": \"SP01ZXA8_EXACT_SUPPORT_SIX_PARTITION_SIEVE\",\n"
           << "  \"status\": \"PASS_INDEPENDENT_EXACT_SUPPORT_SIX_EXCLUSION\",\n"
    ...
    output << "," << CORE_SIZE - FULL_GROUPS * GROUP_SIZE << "],\n"
           ...
           << "  \"dependent_within_group_five_row_histogram\": " << histogram_json(dependent) << ",\n"
           ...
           << "  \"independent_five_row_normals_generated\": " << normal_total << ",\n"
    ...
    output << "],\n"
           << "  \"new_split_locator_with_exact_coefficient_support_six\": false,\n"
           << "  \"scope_guard\": \"Exact support-six classification inside the affine hull of the sixteen known syndrome locators. Support seven or more and arbitrary syndrome-section split locators remain open.\"\n"
           << "}\n";
}
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        std::cout << "PASS independent SP01zxa8 support-six audit\n";
        return 0;
