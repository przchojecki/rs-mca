...
def two_root_line_partner(
    line_model: str,
    line_parameter: int,
    line_multiplier: int | None,
    root: int,
    p: int,
) -> int | None:
    if line_model == "product_mobius":
        if line_multiplier is None or line_multiplier == 0:
            raise AssertionError("product Mobius partner needs nonzero multiplier")
        if root == line_parameter:
            return None
        return (
            line_parameter
            + line_multiplier * inv_mod((root - line_parameter) % p, p)
        ) % p
    if line_model == "sum_mobius":
        return (line_parameter - root) % p
    raise AssertionError("partner is only defined for non-fixed root lines")


def two_root_line_fixed_or_pole_bound(line_model: str) -> int:
    if line_model == "product_mobius":
        return 3
    if line_model == "sum_mobius":
        return 1
    raise AssertionError("fixed/pole bound is only defined for non-fixed lines")
...
def two_exchange_quadratic_slice_profile(
    ...
    if t != 3:
        return {
            ...
            "two_exchange_det_proper_line_variable_nonfixed_involution_root_checks": 0,
            "two_exchange_det_proper_line_variable_nonfixed_domain_orbit_roots": 0,
            "two_exchange_det_proper_line_variable_nonfixed_contained_domain_roots": 0,
            "two_exchange_det_proper_line_variable_nonfixed_contained_domain_pairs": 0,
            "two_exchange_det_proper_line_variable_nonfixed_core_hit_roots": 0,
            "two_exchange_det_proper_line_variable_nonfixed_off_domain_roots": 0,
            ...
            "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots": 0,
            "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower": 0,
            "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs": 0,
            ...
            "two_exchange_det_proper_line_variable_nonfixed_active_domain_singletons": 0,
            "two_exchange_det_proper_line_variable_nonfixed_active_two_packets": 0,
            "two_exchange_det_proper_line_variable_nonfixed_active_edge_bound": 0,
            "two_exchange_det_proper_line_variable_nonfixed_active_sharp_edge_bound": 0,
            "two_exchange_det_proper_line_variable_nonfixed_active_core_slope_max": 0,
            "two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound_max": 0,
            ...
            "two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound": 0,
            ...
            "two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound_max": 0,
    ...
    det_proper_line_variable_nonfixed_involution_root_checks = 0
    det_proper_line_variable_nonfixed_domain_orbit_roots = 0
    det_proper_line_variable_nonfixed_contained_domain_roots = 0
    det_proper_line_variable_nonfixed_contained_domain_pairs = 0
    det_proper_line_variable_nonfixed_core_hit_roots = 0
    det_proper_line_variable_nonfixed_off_domain_roots = 0
    ...
    det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots = 0
    det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower = 0
    det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs = 0
    det_proper_line_variable_nonfixed_domain_singleton_off_domain_roots = 0
    det_proper_line_variable_nonfixed_domain_singleton_contained_boundary_edges = 0
    ...
    det_proper_line_variable_nonfixed_core_defect_edge_bound_max = 0
    ...
    for core in combinations(domain, j - 2):
        ...
        core_nonfixed_variable_domain_singletons = 0
        core_nonfixed_variable_quotient_defects = 0
        ...
        for line_key in all_affine_line_keys:
            ...
            line_aperiodic_complements: list[tuple[int, ...]] = []
            line_noncontained = 0
            line_aperiodic = 0
            line_domain_pairs = 0
            for root_sum, root_product in line_points:
                ...
                line_aperiodic_complements.append(complement)
            ...
            if len(line_slopes) <= 1:
                ...
                if line_model != "fixed_root":
                    ...
                    core_set = set(core_tuple)
                    line_domain_orbit_roots = 0
                    line_contained_domain_roots = 0
                    ...
                    for root in available:
                        ...
                        if partner is None or partner == root:
                            line_fixed_or_pole_roots += 1
                            continue
                        if partner in available_set:
                            root_sum = (root + partner) % p
                            root_product = root * partner % p
                            b_vec = tuple(
                                (
                                    b_square_shift[idx]
                                    - root_sum * b_shift[idx]
                            ...
                            if all(value == 0 for value in b_vec):
                                line_contained_domain_roots += 1
                                contained_complement = tuple(
                                    sorted(core_tuple + (root, partner))
                                )
                                if contained_complement in row_map:
                                    raise AssertionError(
                                        "contained line pair was also aperiodic"
                                    )
                                line_contained_domain_pair_keys.add(
                                    contained_complement
                                )
                            else:
                                line_domain_orbit_roots += 1
                            continue
                        if partner in core_set:
                    ...
                    if (
                        ...
                        raise AssertionError("non-fixed involution root census changed")
                    if line_domain_orbit_roots != 2 * line_domain_pairs:
                        raise AssertionError("non-fixed involution packet orbit count changed")
                    ...
                    det_proper_line_variable_nonfixed_domain_orbit_roots += (
                        line_domain_orbit_roots
                    )
                    det_proper_line_variable_nonfixed_contained_domain_roots += (
                        line_contained_domain_roots
                    )
                    det_proper_line_variable_nonfixed_contained_domain_pairs += (
                        line_contained_domain_pairs
                    )
                    det_proper_line_variable_nonfixed_core_hit_roots += (
                        line_core_hit_roots
                    )
                    det_proper_line_variable_nonfixed_off_domain_roots += (
                        line_off_domain_roots
                    )
                    ...
                    if line_core_hit_roots > len(core_tuple):
                        raise AssertionError("non-fixed line had too many core-hit roots")
                    fixed_or_pole_bound = two_root_line_fixed_or_pole_bound(line_model)
                    if line_fixed_or_pole_roots > fixed_or_pole_bound:
                        raise AssertionError("non-fixed line exceeded its fixed/pole bound")
                    line_free_escape_roots = (
                        line_contained_domain_roots + line_off_domain_roots
                    )
                    line_free_escape_lower = max(
                        0,
                        line_escape_roots - len(core_tuple) - fixed_or_pole_bound,
                    )
                    if line_free_escape_roots < line_free_escape_lower:
                    ...
                    if line_domain_pairs == 1:
                        ...
                        singleton_free_escape_lower = max(
                            0,
                            len(available)
                            - 2
                            - len(core_tuple)
                        ...
                        if line_free_escape_lower != singleton_free_escape_lower:
                            raise AssertionError(
                                "domain singleton free escape lower bound changed"
                            )
                        if line_aperiodic == 1:
                            active_complement = line_aperiodic_complements[0]
                            active_set = set(active_complement)
                            for contained_complement in line_contained_domain_pair_keys:
                                contained_set = set(contained_complement)
                                if (
                                    len(active_set - contained_set) != 2
                                    or len(contained_set - active_set) != 2
                                ):
                                    raise AssertionError(
                                        "contained boundary edge was not two-exchange"
                                    )
                                edge_key = tuple(
                                    sorted((active_complement, contained_complement))
                                )
                                if edge_key in line_contained_boundary_edge_keys:
                                    raise AssertionError(
                                        "contained boundary edge repeated on one line"
                                    )
                                line_contained_boundary_edge_keys.add(edge_key)
                                if (
                                    edge_key
                                    in det_proper_line_variable_nonfixed_domain_singleton_contained_boundary_edge_keys
                                ):
                                    raise AssertionError(
                                        "contained boundary edge was charged twice"
                                    )
                        ...
                        if line_contained_boundary_edges != (
                            ...
                            raise AssertionError(
                                "contained boundary edge count changed"
                            )
                        if line_contained_boundary_edges != len(
                        ...
                        det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots += (
                            line_free_escape_roots
                        )
                        det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower += (
                            line_free_escape_lower
                        )
                        det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs += (
                            line_contained_domain_pairs
                        )
                        det_proper_line_variable_nonfixed_domain_singleton_off_domain_roots += (
                            line_off_domain_roots
                        )
                        ...
                        if line_model == "product_mobius":
                            ...
                            det_proper_line_variable_nonfixed_domain_singleton_sum += 1
                        else:
                            raise AssertionError("domain singleton had unknown line model")
                    packet_pairs = 0
                    for left_complement, right_complement in combinations(
                        line_aperiodic_complements, 2
                    ):
                        left_set = set(left_complement)
                        right_set = set(right_complement)
                        if (
                            len(left_set - right_set) != 2
                            or len(right_set - left_set) != 2
                        ):
                            raise AssertionError(
                                "non-fixed variable line packet was not two-exchange"
                            )
                        left_slope = row_map[left_complement]
                        right_slope = row_map[right_complement]
                    ...
                    if packet_pairs != line_aperiodic * (line_aperiodic - 1) // 2:
                        raise AssertionError(
                            "non-fixed variable line packet pair count changed"
                        )
                    det_proper_line_variable_nonfixed_packet_pair_max = max(
                        det_proper_line_variable_nonfixed_packet_pair_max,
                        packet_pairs,
                    )
                    ...
                    det_proper_line_variable_nonfixed_packet_records.append(
                        (
                            core_tuple,
                            line_model,
                            line_domain_pairs,
                    ...
                    quotient_defects = line_domain_pairs - line_aperiodic
                    if quotient_defects < 0:
                        raise AssertionError("non-fixed variable line had negative quotient defect")
                    det_proper_line_variable_nonfixed_quotient_defects += (
                        quotient_defects
                    )
                    core_nonfixed_variable_quotient_defects += quotient_defects
                    if line_aperiodic == 1:
                        det_proper_line_variable_nonfixed_singletons += 1
                        core_nonfixed_variable_singletons += 1
                        if line_domain_pairs == 1:
                            det_proper_line_variable_nonfixed_domain_singletons += 1
                            core_nonfixed_variable_domain_singletons += 1
        ...
        core_nonfixed_defect_edge_bound = (
            core_nonfixed_variable_domain_singletons
            + core_nonfixed_variable_quotient_defects
            + 2 * core_nonfixed_variable_packet_pairs
        )
        ...
        if core_nonfixed_variable_aperiodic > core_nonfixed_defect_edge_bound:
            raise AssertionError("non-fixed variable line core exceeded defect-edge bound")
        ...
        det_proper_line_variable_nonfixed_core_defect_edge_bound_max = max(
            det_proper_line_variable_nonfixed_core_defect_edge_bound_max,
            core_nonfixed_defect_edge_bound,
        )
        if core_full_det_plane:
    ...
    det_proper_line_variable_nonfixed_defect_edge_bound = (
        det_proper_line_variable_nonfixed_domain_singletons
        + det_proper_line_variable_nonfixed_quotient_defects
        + 2 * det_proper_line_variable_nonfixed_packet_pair_checks
    )
    if (
    ...
    if (
        ...
        raise AssertionError("non-fixed variable line packets exceeded defect-edge bound")
    ...
    det_proper_line_variable_nonfixed_new_slope_set: set[int] = set()
    det_proper_line_variable_nonfixed_new_slope_checks = 0
    det_proper_line_variable_nonfixed_active_singletons = 0
    det_proper_line_variable_nonfixed_active_domain_singletons = 0
    det_proper_line_variable_nonfixed_active_two_packets = 0
    active_core_rows: dict[tuple[int, ...], dict[str, int]] = {}
    for (
        core_tuple,
        line_model,
        line_domain_pairs,
        ...
        quotient_defects = line_domain_pairs - line_aperiodic
        if quotient_defects < 0:
            raise AssertionError("non-fixed active packet had negative quotient defect")
        det_proper_line_variable_nonfixed_new_slope_set.update(active_new_slopes)
        det_proper_line_variable_nonfixed_new_slope_checks += active_new_count
        if line_aperiodic == 1 and active_new_count == 1:
            ...
            if line_domain_pairs == 1:
                ...
                for edge_key in line_contained_boundary_edge_keys:
                    if (
                        edge_key
                        in det_proper_line_variable_nonfixed_active_domain_singleton_contained_boundary_edge_keys
                    ):
                        raise AssertionError(
                            "active contained boundary edge was charged twice"
                        )
                ...
                if line_model == "product_mobius":
                    ...
                    det_proper_line_variable_nonfixed_active_domain_singleton_sum += 1
                else:
                    raise AssertionError("active domain singleton had unknown line model")
        if line_aperiodic == 2 and active_new_count == 2:
            det_proper_line_variable_nonfixed_active_two_packets += 1

        core_row = active_core_rows.setdefault(
            core_tuple,
            {
                "new": 0,
                "active_singletons": 0,
                "active_domain_singletons": 0,
                "active_two_packets": 0,
                "quotient_defects": 0,
                "packet_pairs": 0,
            },
        )
        core_row["new"] += active_new_count
        core_row["quotient_defects"] += quotient_defects
        core_row["packet_pairs"] += packet_pairs
        if line_aperiodic == 1 and active_new_count == 1:
            core_row["active_singletons"] += 1
            if line_domain_pairs == 1:
                core_row["active_domain_singletons"] += 1
        if line_aperiodic == 2 and active_new_count == 2:
            core_row["active_two_packets"] += 1

    det_proper_line_variable_nonfixed_active_edge_bound = (
        det_proper_line_variable_nonfixed_active_singletons
        + 2 * det_proper_line_variable_nonfixed_packet_pair_checks
    )
    det_proper_line_variable_nonfixed_active_sharp_edge_bound = (
    ...
    if (
        ...
        raise AssertionError("active two-packets exceeded packet-edge energy")
    det_proper_line_variable_nonfixed_active_target_edge_bound = (
        det_proper_line_variable_nonfixed_active_domain_singleton_zero_free_escape_lower
        + det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_full_target_budget
        + det_proper_line_variable_nonfixed_quotient_defects
        + 2 * det_proper_line_variable_nonfixed_packet_pair_checks
    ...
    if (
        ...
        raise AssertionError("non-fixed variable line target edge bound failed")
    if (
        det_proper_line_variable_nonfixed_new_slope_checks
        > det_proper_line_variable_nonfixed_active_edge_bound
    ):
        raise AssertionError("non-fixed active variable-line packets exceeded edge bound")
    if (
        det_proper_line_variable_nonfixed_new_slope_checks
        > det_proper_line_variable_nonfixed_active_sharp_edge_bound
    ):
        raise AssertionError("non-fixed active variable-line packets exceeded sharp edge bound")
    det_proper_line_variable_nonfixed_active_core_slope_max = 0
    det_proper_line_variable_nonfixed_active_core_sharp_bound_max = 0
    for core_row in active_core_rows.values():
        core_active_edge_bound = (
            core_row["active_singletons"] + 2 * core_row["packet_pairs"]
        )
        core_active_sharp_edge_bound = (
            core_row["active_domain_singletons"]
            + core_row["quotient_defects"]
            + core_row["active_two_packets"]
            + core_row["packet_pairs"]
        )
        if core_row["new"] > core_active_edge_bound:
            raise AssertionError("non-fixed active core exceeded edge bound")
        if core_row["new"] > core_active_sharp_edge_bound:
            raise AssertionError("non-fixed active core exceeded sharp edge bound")
        det_proper_line_variable_nonfixed_active_core_slope_max = max(
            det_proper_line_variable_nonfixed_active_core_slope_max,
            core_row["new"],
        )
        det_proper_line_variable_nonfixed_active_core_sharp_bound_max = max(
            det_proper_line_variable_nonfixed_active_core_sharp_bound_max,
            core_active_sharp_edge_bound,
        )
    ...
    return {
        ...
        "two_exchange_det_proper_line_variable_nonfixed_packet_pair_max": (
            det_proper_line_variable_nonfixed_packet_pair_max
        ),
        ...
        "two_exchange_det_proper_line_variable_nonfixed_involution_root_checks": (
            det_proper_line_variable_nonfixed_involution_root_checks
        ),
        "two_exchange_det_proper_line_variable_nonfixed_domain_orbit_roots": (
            det_proper_line_variable_nonfixed_domain_orbit_roots
        ),
        "two_exchange_det_proper_line_variable_nonfixed_contained_domain_roots": (
            det_proper_line_variable_nonfixed_contained_domain_roots
        ),
        "two_exchange_det_proper_line_variable_nonfixed_contained_domain_pairs": (
            det_proper_line_variable_nonfixed_contained_domain_pairs
        ),
        "two_exchange_det_proper_line_variable_nonfixed_core_hit_roots": (
            det_proper_line_variable_nonfixed_core_hit_roots
        ),
        "two_exchange_det_proper_line_variable_nonfixed_off_domain_roots": (
            det_proper_line_variable_nonfixed_off_domain_roots
        ),
        "two_exchange_det_proper_line_variable_nonfixed_fixed_or_pole_roots": (
            det_proper_line_variable_nonfixed_fixed_or_pole_roots
        ),
        "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_product": (
            det_proper_line_variable_nonfixed_domain_singleton_product
        ),
        ...
        "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots": (
            det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots
        ),
        "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower": (
            det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower
        ),
        "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs": (
            det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs
        ),
        "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_off_domain_roots": (
            det_proper_line_variable_nonfixed_domain_singleton_off_domain_roots
        ),
        "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_boundary_edges": (
            det_proper_line_variable_nonfixed_domain_singleton_contained_boundary_edges
        ),
        ...
        "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_roots": (
            det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_roots
        ),
        ...
        "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_off_domain_roots": (
            det_proper_line_variable_nonfixed_active_domain_singleton_off_domain_roots
        ),
        "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_contained_boundary_edges": (
            det_proper_line_variable_nonfixed_active_domain_singleton_contained_boundary_edges
        ),
        ...
        "two_exchange_det_proper_line_variable_nonfixed_new_slope_image": len(
            det_proper_line_variable_nonfixed_new_slope_set
        ),
        "two_exchange_det_proper_line_variable_nonfixed_active_singletons": (
            det_proper_line_variable_nonfixed_active_singletons
        ),
        "two_exchange_det_proper_line_variable_nonfixed_active_domain_singletons": (
            det_proper_line_variable_nonfixed_active_domain_singletons
        ),
        "two_exchange_det_proper_line_variable_nonfixed_active_two_packets": (
            det_proper_line_variable_nonfixed_active_two_packets
        ),
        "two_exchange_det_proper_line_variable_nonfixed_active_edge_bound": (
            det_proper_line_variable_nonfixed_active_edge_bound
        ),
        "two_exchange_det_proper_line_variable_nonfixed_active_sharp_edge_bound": (
            det_proper_line_variable_nonfixed_active_sharp_edge_bound
        ),
        "two_exchange_det_proper_line_variable_nonfixed_active_core_slope_max": (
            det_proper_line_variable_nonfixed_active_core_slope_max
        ),
        "two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound_max": (
            det_proper_line_variable_nonfixed_active_core_sharp_bound_max
        ),
        "two_exchange_det_proper_line_variable_nonfixed_singletons": (
            det_proper_line_variable_nonfixed_singletons
        ),
        "two_exchange_det_proper_line_variable_nonfixed_domain_singletons": (
            det_proper_line_variable_nonfixed_domain_singletons
        ),
        "two_exchange_det_proper_line_variable_nonfixed_quotient_defects": (
            det_proper_line_variable_nonfixed_quotient_defects
        ),
        ...
        "two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound": (
            det_proper_line_variable_nonfixed_defect_edge_bound
        ),
        ...
        "two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound_max": (
            det_proper_line_variable_nonfixed_core_defect_edge_bound_max
        ),
...
def verify_word_pair(
    ...
    return {
        ...
        "two_exchange_det_proper_line_variable_nonfixed_new_slope_image": (
            two_exchange_profile[
                "two_exchange_det_proper_line_variable_nonfixed_new_slope_image"
            ]
        ),
...
def verify_t3_same_slope_two_exchange_probe() -> dict[str, object]:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_packet_pair_max"] != 3:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_new_slope_checks"] != 0:
        raise AssertionError("same-slope probe gained active non-fixed new slopes")
    if row["two_exchange_det_proper_line_variable_nonfixed_new_slope_image"] != 0:
        raise AssertionError("same-slope probe gained a non-fixed new slope image")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_singletons"] != 0:
        raise AssertionError("same-slope probe gained an active singleton")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_domain_singletons"] != 0:
        raise AssertionError("same-slope probe gained an active domain singleton")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_two_packets"] != 0:
        raise AssertionError("same-slope probe gained an active two-packet")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_edge_bound"] != 12:
        raise AssertionError("same-slope probe active edge bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_sharp_edge_bound"] != 6:
        raise AssertionError("same-slope probe active sharp edge bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_core_slope_max"] != 0:
        raise AssertionError("same-slope probe active core slope max changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound_max"] != 3:
        raise AssertionError("same-slope probe active core sharp bound changed")
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_domain_singletons"] != 0:
        raise AssertionError("same-slope probe gained a domain-singleton packet")
    if row["two_exchange_det_proper_line_variable_nonfixed_quotient_defects"] != 0:
        raise AssertionError("same-slope probe gained a variable-line quotient defect")
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound"] != 12:
        raise AssertionError("same-slope probe non-fixed defect-edge bound changed")
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound_max"] != 6:
        raise AssertionError("same-slope probe non-fixed core defect-edge bound changed")
...
def verify_t3_variable_new_slope_probe() -> dict[str, object]:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_new_slope_checks"] != 1:
        raise AssertionError("variable new-slope probe active new slope count changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_new_slope_image"] != 1:
        raise AssertionError("variable new-slope probe active new slope image changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_singletons"] != 0:
        raise AssertionError("variable new-slope probe gained an active singleton")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_domain_singletons"] != 0:
        raise AssertionError("variable new-slope probe gained an active domain singleton")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_two_packets"] != 0:
        raise AssertionError("variable new-slope probe gained an active two-packet")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_edge_bound"] != 6:
        raise AssertionError("variable new-slope probe active edge bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_sharp_edge_bound"] != 3:
        raise AssertionError("variable new-slope probe active sharp edge bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_core_slope_max"] != 1:
        raise AssertionError("variable new-slope probe active core slope max changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound_max"] != 3:
        raise AssertionError("variable new-slope probe active core sharp bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_singletons"] != 0:
        raise AssertionError("variable new-slope probe gained a singleton packet")
    if row["two_exchange_det_proper_line_variable_nonfixed_domain_singletons"] != 0:
        raise AssertionError("variable new-slope probe gained a domain-singleton packet")
    if row["two_exchange_det_proper_line_variable_nonfixed_quotient_defects"] != 0:
        raise AssertionError("variable new-slope probe gained a quotient defect")
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound"] != 6:
        raise AssertionError("variable new-slope probe non-fixed defect-edge bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_core_slope_max"] != 3:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound_max"] != 6:
        raise AssertionError("variable new-slope probe non-fixed core defect-edge bound changed")
    return row


def verify_t3_unanchored_variable_line_probe() -> dict[str, object]:
    case = Case(
        "F13_order12_j5_t3_unanchored_variable_line_probe",
        p=13,
        n=12,
        j=5,
        t=3,
    ...
    domain, _, _ = cyclic_domain(case.p, case.n)
    f_values = (3, 10, 10, 9, 3, 4, 11, 9, 9, 4, 2, 3)
    g_values = (1, 8, 3, 8, 1, 7, 10, 6, 11, 12, 12, 4)
    f = dict(zip(domain, f_values, strict=True))
    g = dict(zip(domain, g_values, strict=True))
    row = verify_word_pair(case, "unanchored-variable-line-probe", f, g)
    if row["aperiodic_locators"] != 12:
        raise AssertionError("unanchored variable-line probe aperiodic count changed")
    if row["aperiodic_slopes"] != 4:
        raise AssertionError("unanchored variable-line probe slope count changed")
    if row["two_exchange_det_full_planes"] != 0:
        raise AssertionError("unanchored variable-line probe gained a full plane")
    if row["two_exchange_det_proper_lines"] != 6:
        raise AssertionError("unanchored variable-line probe proper-line count changed")
    if row["two_exchange_det_proper_line_product_mobius"] != 1:
        raise AssertionError("unanchored variable-line probe lost its product-Mobius line")
    if row["two_exchange_det_proper_line_variable_slope"] != 1:
        raise AssertionError("unanchored variable-line probe lost its variable line")
    if row["two_exchange_det_proper_line_variable_aperiodic_slopes"] != 3:
        raise AssertionError("unanchored variable-line probe aperiodic variable slopes changed")
    if row["two_exchange_det_proper_line_variable_new_slopes"] != 2:
        raise AssertionError("unanchored variable-line probe new-slope count changed")
    if row["two_exchange_det_proper_line_variable_new_slope_max"] != 2:
        raise AssertionError("unanchored variable-line probe per-line new slopes changed")
    if row["two_exchange_det_proper_line_variable_nonfixed"] != 1:
        raise AssertionError("unanchored variable-line probe non-fixed count changed")
    if row["two_exchange_det_proper_line_variable_anchored"] != 0:
        raise AssertionError("unanchored variable-line probe became anchored")
    if row["two_exchange_det_proper_line_variable_unanchored"] != 1:
        raise AssertionError("unanchored variable-line probe lost its unanchored line")
    if row["two_exchange_det_proper_line_variable_domain_pair_max"] != 3:
        raise AssertionError("unanchored variable-line probe domain packet changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_packet_pair_max"] != 3:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_new_slope_checks"] != 2:
        raise AssertionError("unanchored variable-line probe active new slope count changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_new_slope_image"] != 2:
        raise AssertionError("unanchored variable-line probe active new slope image changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_singletons"] != 0:
        raise AssertionError("unanchored variable-line probe gained an active singleton")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_domain_singletons"] != 0:
        raise AssertionError("unanchored variable-line probe gained an active domain singleton")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_two_packets"] != 0:
        raise AssertionError("unanchored variable-line probe gained an active two-packet")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_edge_bound"] != 6:
        raise AssertionError("unanchored variable-line probe active edge bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_sharp_edge_bound"] != 3:
        raise AssertionError("unanchored variable-line probe active sharp edge bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_core_slope_max"] != 2:
        raise AssertionError("unanchored variable-line probe active core slope max changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound_max"] != 3:
        raise AssertionError("unanchored variable-line probe active core sharp bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_singletons"] != 0:
        raise AssertionError("unanchored variable-line probe gained a singleton packet")
    if row["two_exchange_det_proper_line_variable_nonfixed_domain_singletons"] != 0:
        raise AssertionError("unanchored variable-line probe gained a domain-singleton packet")
    if row["two_exchange_det_proper_line_variable_nonfixed_quotient_defects"] != 0:
        raise AssertionError("unanchored variable-line probe gained a quotient defect")
    if row["two_exchange_det_proper_line_variable_nonfixed_aperiodic_slope_checks"] != 3:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound"] != 6:
        raise AssertionError("unanchored variable-line probe non-fixed defect-edge bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_core_slope_max"] != 3:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound_max"] != 6:
        raise AssertionError("unanchored variable-line probe non-fixed core defect-edge bound changed")
    return row
...
def verify_t3_active_domain_singleton_probe() -> dict[str, object]:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_involution_root_checks"] != 9:
        raise AssertionError("active domain-singleton probe root census size changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_domain_orbit_roots"] != 2:
        raise AssertionError("active domain-singleton probe domain orbit roots changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_contained_domain_roots"] != 2:
        raise AssertionError("active domain-singleton probe contained domain roots changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_contained_domain_pairs"] != 1:
        raise AssertionError("active domain-singleton probe contained domain pairs changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_core_hit_roots"] != 3:
        raise AssertionError("active domain-singleton probe core-hit roots changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_off_domain_roots"] != 0:
        raise AssertionError("active domain-singleton probe off-domain roots changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_fixed_or_pole_roots"] != 2:
        raise AssertionError("active domain-singleton probe fixed/pole roots changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_domain_singleton_product"] != 1:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots"] != 2:
        raise AssertionError("active domain-singleton probe singleton free escape roots changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower"] != 1:
        raise AssertionError("active domain-singleton probe singleton free escape lower bound changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs"] != 1:
        raise AssertionError("active domain-singleton probe singleton contained pairs changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_domain_singleton_off_domain_roots"] != 0:
        raise AssertionError("active domain-singleton probe singleton off-domain roots changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_boundary_edges"] != 1:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_roots"] != 2:
    ...
    if row["two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_off_domain_roots"] != 0:
        raise AssertionError("active domain-singleton probe active singleton off-domain roots changed")
    if row["two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_contained_boundary_edges"] != 1:
...
def verify_two_exchange_line_geometry_models() -> dict[str, int]:
    ...
    if two_root_line_fixed_or_pole_bound("product_mobius") != 3:
        raise AssertionError("product Mobius fixed/pole bound changed")
    ...
    if two_root_line_fixed_or_pole_bound("sum_mobius") != 1:
        raise AssertionError("sum Mobius fixed/pole bound changed")
...
def main() -> None:
    ...
    for summary in summaries:
        ...
        for row in summary["rows"]:
            print(
                ...
                "two_exchange_det_proper_line_variable_nonfixed_active_two_packets={two_exchange_det_proper_line_variable_nonfixed_active_two_packets} "
                "two_exchange_det_proper_line_variable_nonfixed_active_edge_bound={two_exchange_det_proper_line_variable_nonfixed_active_edge_bound} "
                ...
                "two_exchange_det_proper_line_variable_nonfixed_active_core_slope_max={two_exchange_det_proper_line_variable_nonfixed_active_core_slope_max} "
                "two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound_max={two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound_max} "
                ...
                "two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound={two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound} "
                ...
                "two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound_max={two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound_max} "
    ...
    print(
        ...
        "two_exchange_det_proper_line_variable_nonfixed_contained_domain_pairs={two_exchange_det_proper_line_variable_nonfixed_contained_domain_pairs} "
        ...
        "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots={two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots} "
        "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower={two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower} "
        "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs={two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs} "
        ...
        "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_roots={two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_roots} "
        "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_lower={two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_lower} "
        "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_contained_domain_pairs={two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_contained_domain_pairs} "
    ...
    max_two_exchange_det_proper_line_variable_nonfixed_packet_pair = max(
        row["two_exchange_det_proper_line_variable_nonfixed_packet_pair_max"]
        for row in all_rows
    )
    ...
    max_two_exchange_det_proper_line_variable_nonfixed_involution_root_checks = max(
        row["two_exchange_det_proper_line_variable_nonfixed_involution_root_checks"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_domain_orbit_roots = max(
        row["two_exchange_det_proper_line_variable_nonfixed_domain_orbit_roots"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_contained_domain_roots = max(
        row["two_exchange_det_proper_line_variable_nonfixed_contained_domain_roots"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_contained_domain_pairs = max(
        row["two_exchange_det_proper_line_variable_nonfixed_contained_domain_pairs"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_core_hit_roots = max(
        row["two_exchange_det_proper_line_variable_nonfixed_core_hit_roots"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_off_domain_roots = max(
        row["two_exchange_det_proper_line_variable_nonfixed_off_domain_roots"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_fixed_or_pole_roots = max(
        row["two_exchange_det_proper_line_variable_nonfixed_fixed_or_pole_roots"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_product = max(
        row["two_exchange_det_proper_line_variable_nonfixed_domain_singleton_product"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_sum = max(
    ...
    max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots"
        ]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower"
        ]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs"
        ]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_off_domain_roots = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_off_domain_roots"
        ]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_boundary_edges = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_boundary_edges"
        ]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_boundary_edge_injections = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_boundary_edge_injections"
        ]
        for row in all_rows
    ...
    max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_roots = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_roots"
        ]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_lower = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_lower"
        ]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_contained_domain_pairs = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_contained_domain_pairs"
        ]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_off_domain_roots = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_off_domain_roots"
        ]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_contained_boundary_edges = max(
        row[
            "two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_contained_boundary_edges"
        ]
        for row in all_rows
    ...
    max_two_exchange_det_proper_line_variable_nonfixed_new_slope_checks = max(
        row["two_exchange_det_proper_line_variable_nonfixed_new_slope_checks"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_new_slope_image = max(
        row["two_exchange_det_proper_line_variable_nonfixed_new_slope_image"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_singletons = max(
        row["two_exchange_det_proper_line_variable_nonfixed_active_singletons"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singletons = max(
        row["two_exchange_det_proper_line_variable_nonfixed_active_domain_singletons"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_two_packets = max(
        row["two_exchange_det_proper_line_variable_nonfixed_active_two_packets"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_edge_bound = max(
        row["two_exchange_det_proper_line_variable_nonfixed_active_edge_bound"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_sharp_edge_bound = max(
        row["two_exchange_det_proper_line_variable_nonfixed_active_sharp_edge_bound"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_core_slope = max(
        row["two_exchange_det_proper_line_variable_nonfixed_active_core_slope_max"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound = max(
        row["two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound_max"]
        for row in all_rows
    )
    ...
    max_two_exchange_det_proper_line_variable_nonfixed_domain_singletons = max(
        row["two_exchange_det_proper_line_variable_nonfixed_domain_singletons"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_nonfixed_quotient_defects = max(
        row["two_exchange_det_proper_line_variable_nonfixed_quotient_defects"]
        for row in all_rows
    )
    ...
    max_two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound = max(
        row["two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound"]
        for row in all_rows
    )
    ...
    max_two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound = max(
        row["two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound_max"]
        for row in all_rows
    )
    max_two_exchange_det_proper_line_variable_charged_slope_checks = max(
        row["two_exchange_det_proper_line_variable_charged_slope_checks"]
    ...
    print(
        ...
        f"max_two_exchange_det_proper_line_variable_nonfixed_involution_root_checks={max_two_exchange_det_proper_line_variable_nonfixed_involution_root_checks} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_domain_orbit_roots={max_two_exchange_det_proper_line_variable_nonfixed_domain_orbit_roots} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_contained_domain_roots={max_two_exchange_det_proper_line_variable_nonfixed_contained_domain_roots} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_contained_domain_pairs={max_two_exchange_det_proper_line_variable_nonfixed_contained_domain_pairs} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_core_hit_roots={max_two_exchange_det_proper_line_variable_nonfixed_core_hit_roots} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_off_domain_roots={max_two_exchange_det_proper_line_variable_nonfixed_off_domain_roots} "
        ...
        f"max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots={max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_roots} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower={max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_free_escape_lower} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs={max_two_exchange_det_proper_line_variable_nonfixed_domain_singleton_contained_domain_pairs} "
        ...
        f"max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_roots={max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_roots} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_lower={max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_free_escape_lower} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_contained_domain_pairs={max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singleton_contained_domain_pairs} "
        ...
        f"max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singletons={max_two_exchange_det_proper_line_variable_nonfixed_active_domain_singletons} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_active_two_packets={max_two_exchange_det_proper_line_variable_nonfixed_active_two_packets} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_active_edge_bound={max_two_exchange_det_proper_line_variable_nonfixed_active_edge_bound} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_active_sharp_edge_bound={max_two_exchange_det_proper_line_variable_nonfixed_active_sharp_edge_bound} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_active_core_slope={max_two_exchange_det_proper_line_variable_nonfixed_active_core_slope} "
        f"max_two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound={max_two_exchange_det_proper_line_variable_nonfixed_active_core_sharp_bound} "
        ...
        f"max_two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound={max_two_exchange_det_proper_line_variable_nonfixed_defect_edge_bound} "
        ...
        f"max_two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound={max_two_exchange_det_proper_line_variable_nonfixed_core_defect_edge_bound} "
