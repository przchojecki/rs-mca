#!/usr/bin/env python3
"""Verify the F_17^32 M3 rank-8 low-rank finite-slack family.

For each 385 <= A <= 426, put r=j+1=513-A and use the first r
descriptor-domain nodes as X, with the next eight descriptor nodes as Y.  The
low-rank template gives

    Delta_r(Z)=det(H_X) det(I+ZK),
    K_ab=sum_i L_i(y_a)L_i(y_b).

Rank 8 is beyond the v4 low-rank degree envelope: degree-only accounting gives
8 finite roots plus one possible projective infinity point.  This verifier
...
SCHEMA_VERSION = "f17-32-m3-low-rank8-slack-family-v1"
N = 512
K = 256
AGREEMENT_MIN = 385
AGREEMENT_MAX = 426
...
def low_rank_sidecar_from_prefix(
    ...
    sidecar = {
        "kind": "square_base_lagrange_kernel_rank8_finite_slack",
...
def build_records(
    ...
    require(
        UPDATE_RANK > max(finite_safe_ranks)
        and UPDATE_RANK not in finite_safe_ranks
        and UPDATE_RANK not in projective_safe_ranks,
        "rank-8 is not recorded as beyond the v4 low-rank degree envelope",
    )
    records = []
    base_nodes: list[tuple[int, ...]] = []
    ...
    for size in range(1, N - AGREEMENT_MIN + 2):
        ...
        require(
            ...
            f"A={agreement}: rank-8 polynomial is not degree 8",
        )
        require(
        ...
        records.append(
            {
                ...
                "projective_infinity": {
                    ...
                    "reason": (
                        ...
                        "direction has rank 8 < j+1, so det H(v)=0 and this "
                        "regular minor does not exclude [0:1]; the nonzero "
                        "compressed degree-8 leading coefficient only controls "
...
def build_certificate() -> dict[str, Any]:
    ...
    require(max_finite_roots == 4, "unexpected rank-8 finite-root maximum")
    ...
    return {
        ...
        "construction": {
            ...
            "update_nodes": "next eight descriptor-domain elements",
        ...
        "method": {
            ...
            "finite_slack": (
                "exact finite roots are at most 4, so rank 8 supplies "
        ...
        "aggregate": {
            ...
            "polynomial_degree_histogram": {"8": len(records)},
...
def check_certificate(certificate: dict[str, Any], path: Path) -> None:
    ...
    if actual != expected:
        raise AssertionError(f"rank-8 low-rank slack family mismatch: {path}")


def print_summary(certificate: dict[str, Any]) -> None:
    aggregate = certificate["aggregate"]
    print("F_17^32 M3 low-rank-8 finite-slack family")
