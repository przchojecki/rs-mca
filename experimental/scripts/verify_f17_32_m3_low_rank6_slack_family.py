#!/usr/bin/env python3
"""Verify the F_17^32 M3 rank-6 low-rank finite-slack family.

For each 385 <= A <= 426, put r=j+1=513-A and use the first r
descriptor-domain nodes as X, with the next six descriptor nodes as Y.  The
low-rank template gives

    Delta_r(Z)=det(H_X) det(I+ZK),
    K_ab=sum_i L_i(y_a)L_i(y_b).

Rank 6 is the first update rank that is not automatically projective-safe
under the v4 low-rank packet gate: degree-only accounting gives 6 finite roots
plus one possible projective infinity point.  This verifier supplies the
missing extra certificate for this synthetic family by computing
gcd(Delta_r(Z), Z^q-Z) over F_17^32.  The exact finite-root maximum is 2, so
the corrected projective bound is 2+1=3 <= 6.
"""
...
SCHEMA_VERSION = "f17-32-m3-low-rank6-slack-family-v1"
N = 512
K = 256
AGREEMENT_MIN = 385
AGREEMENT_MAX = 426
...
def low_rank_sidecar_from_prefix(
    ...
    sidecar = {
        "kind": "square_base_lagrange_kernel_rank6_finite_slack",
...
def build_records(
    ...
    require(
        UPDATE_RANK in gate["projective_requires_extra_certificate_update_ranks"],
        "rank-6 projective gate is not marked as needing extra evidence",
    )
    records = []
    base_nodes: list[tuple[int, ...]] = []
    ...
    for size in range(1, N - AGREEMENT_MIN + 2):
        ...
        require(
            ...
            f"A={agreement}: rank-6 polynomial is not degree 6",
        )
        require(
        ...
        require(
            0 <= root_count_value <= 2,
            f"A={agreement}: finite-root slack failed",
        )
        listed_roots = listed_roots_from_gcd(root_gcd, field)
        require(
            listed_roots is not None and len(listed_roots) == root_count_value,
            f"A={agreement}: listed finite roots mismatch",
        )
        records.append(
            {
                "A": agreement,
                ...
                "regular_budget_table": {
                    ...
                    "projective_budget_gap": projective_budget - root_count_value - 1,
                    "degree_only_projective_budget_gap": (
                        projective_budget - UPDATE_RANK - 1
                    ),
...
def build_certificate() -> dict[str, Any]:
    ...
    max_finite_roots = max(record["root_count"] for record in records)
    require(max_finite_roots == 2, "unexpected rank-6 finite-root maximum")
    require(max_finite_roots + 1 <= projective_budget, "projective budget fail")
    return {
        ...
        "construction": {
            ...
            "update_nodes": "next six descriptor-domain elements",
            "certificate_mode": "low_rank_update_finite_slack",
            "rank": UPDATE_RANK,
        },
        ...
        "method": {
            ...
            "finite_slack": (
                "exact finite roots are at most 2, so rank 6 supplies the "
                "finite-root slack required by the v4 packet gate"
            ),
            "projective_root_bound": (
                "corrected low-rank endpoint contributes one possible infinity "
                "point, so at most 3 projective regular roots"
            ),
        },
        "aggregate": {
            ...
            "polynomial_degree_histogram": {"6": len(records)},
            ...
            "degree_only_projective_bound_without_slack": UPDATE_RANK + 1,
            "degree_only_projective_bound_within_budget": False,
            "finite_slack_projective_gate_status": "closed_by_exact_root_count",
...
def check_certificate(certificate: dict[str, Any], path: Path) -> None:
    ...
    if actual != expected:
        raise AssertionError(f"rank-6 low-rank slack family mismatch: {path}")


def print_summary(certificate: dict[str, Any]) -> None:
    aggregate = certificate["aggregate"]
    print("F_17^32 M3 low-rank-6 finite-slack family")
    ...
    print(f"root histogram: {aggregate['linear_root_count_histogram']}")
    print(
        "max finite roots/agreement={finite}, max projective roots/agreement={projective}, budget={budget}".format(
            finite=aggregate["max_finite_roots_per_agreement"],
            projective=aggregate["max_projective_regular_roots_per_agreement"],
