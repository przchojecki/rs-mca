...
struct Vec8 {
    ...
    auto operator<=>(const Vec8&) const = default;
};

Vec8 fraction_free_null_normal(const std::array<Vec8, ROWS>& input, bool& independent) {
    ...
    std::array<int, SUPPORT> permutation{0, 1, 2, 3, 4, 5, 6, 7};
...
std::uint32_t dot(const Vec8& left, const Vec8& right) {
...
bool admissible(const Vec8& value) {
    std::uint32_t sum = 0U;
...
void batch_projectivize(std::vector<Vec8>& values) {
    if (values.empty()) {
        return;
    }
...
std::uint64_t choose7(int value) {
    return value < 7 ? 0U
        : static_cast<std::uint64_t>(value) * (value - 1) * (value - 2)
          * (value - 3) * (value - 4) * (value - 5) * (value - 6) / 5040U;
}
...
Result process_support(
    ...
    std::vector<Vec8> normals;
    normals.reserve(356356U);
    for (int group = 0; group <= FULL_GROUPS; ++group) {
                    ...
                    for (int d = c + 1; d < end; ++d) {
                        for (int e = d + 1; e < end; ++e) {
                            for (int f = e + 1; f < end; ++f) {
                                for (int g = f + 1; g < end; ++g) {
                                    const std::array<Vec8, ROWS> selected{
    ...
    if (result.observed >= result.forced) {
        throw std::runtime_error("support-eight candidate survives partition sieve");
    }
    return result;
}
                    ...
                    for (int e = d + 1; e < BRANCHES; ++e) {
                        for (int f = e + 1; f < BRANCHES; ++f) {
                            for (int g = f + 1; g < BRANCHES; ++g) {
                                for (int h = g + 1; h < BRANCHES; ++h) {
                                    answer.push_back({a, b, c, d, e, f, g, h});
                                }
                            }
                        }
                    }
...
void write_report(const std::string& path, const std::vector<Result>& results) {
    ...
    output << "{\n"
           << "  \"schema\": \"sp01zxaa-independent-support-eight-audit/v1\",\n"
           << "  \"claim_id\": \"SP01ZXAA_EXACT_SUPPORT_EIGHT_PARTITION_SIEVE\",\n"
           << "  \"status\": \"PASS_INDEPENDENT_EXACT_SUPPORT_EIGHT_EXCLUSION\",\n"
    ...
    output << "," << CORE_SIZE - FULL_GROUPS * GROUP_SIZE << "],\n"
           ...
           << "  \"dependent_within_group_seven_row_histogram\": " << histogram_json(dependent) << ",\n"
           ...
           << "  \"independent_seven_row_normals_generated\": " << normal_total << ",\n"
    ...
    output << "],\n"
           << "  \"new_split_locator_with_exact_coefficient_support_eight\": false,\n"
           << "  \"scope_guard\": \"Exact support-eight classification inside the affine hull of the sixteen known syndrome locators. Support nine or more and arbitrary syndrome-section split locators remain open.\"\n"
           << "}\n";
}
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        std::cout << "PASS independent SP01zxaa support-eight audit\n";
        return 0;
