...
In the fixed-basis coordinate

```text
x=c_1/c_0,
```

the forbidden endpoints are `A_1={x=0}` and `A_0={x=infty}`.  Thus the
generic affine subchart has

```text
x_0=lambda_0 in F^*,
x_1=c_1(E_1)/c_0(E_1) in F^*,        x_1 != x_0,
lambda_1=2x_0-x_0^2/x_1.                         (RKSQXPAR)
```

On this subchart the rows and norm have the normal form

```text
c_2=2x_0 c_1-x_0^2 c_0,
c_3=2lambda_1 c_2-lambda_1^2 c_1,
H_0=-(c_1-x_0 c_0)^2,
H_1=-(x_0^4/x_1^2)(c_1-x_1 c_0)^2.                (RKSQXNF)
```

If `E_1=A_0` is the endpoint at `x=infty`, then `lambda_1=2x_0` and

```text
H_1=-x_0^4 c_0^2.
```

Thus, after the fixed initial row basis is chosen, the entire nondegenerate
overlapping square-norm chart is the elementary two-parameter family
`(x_0,x_1)` with `x_0 in F^*` and `x_1 in P^1 \ {0,x_0}`.  The only remaining
global work is to control which initial row bases occur.

Equivalently, the endpoint coordinate can be eliminated in favor of the two
recurrence slopes.  The fixed-basis nondegenerate off-diagonal chart is

```text
lambda_0 in F^*,        lambda_1 in F \ {lambda_0}.       (RKSQSLOPE)
```

Given such a pair, set `x_0=lambda_0`.  If `lambda_1!=2lambda_0`, then

```text
x_1=lambda_0^2/(2lambda_0-lambda_1);
```

if `lambda_1=2lambda_0`, set `x_1=infty`.  In the first case
`x_1 in F^* \ {x_0}`, and in the second case `x_1` is the allowed endpoint
`A_0`; substituting into (RKSQXPAR) recovers the original `lambda_1`.
Thus the local branch has no hidden endpoint constraint after the initial row
...
The full fixed-basis support-packet family is an exact incidence design.
Use the same universe `U=P^1(F_q)\{0}`.  For each two-element support
`S={alpha,beta} subset U`, choose any degree-one parameter `M_S` with
zero-pole support `S`, and take the `e/2` packets

```text
P_S(theta)={x in P^1(F_q) \ S : chi(M_S(x))^2=theta},
        theta in 2Z/eZ.
```

This packet family has point incidences

```text
#{(S,theta): x in P_S(theta)}
  = binom(q,2)       if x=0,
  = binom(q-1,2)     if x in U.                  (RKSQDESIGN)
```

Indeed, for a fixed support `S` not containing `x`, exactly one square-coset
packet contains `x`, while supports containing `x` do not contain `x` in their
open packet.  Thus the full fixed-basis local family has no hidden point
concentration: the only larger incidence is the distinguished forbidden point
`x=0`, which is never an endpoint support.  Any nonuniformity beyond
(RKSQDESIGN) must therefore enter through the global row-basis/core-image
selection of supports, not through the local square-map packet geometry.

More generally, the same argument gives the selected-support reduction needed
by the global ledger.  Let `Sigma` be any family of two-element supports in
`U=P^1(F_q)\{0}`.  For each `S in Sigma`, let `Theta_S` be any subset of the
`e/2` square-coset packet classes.  Define the projective packet incidence

```text
I(x)=#{(S,theta): S in Sigma, theta in Theta_S, x in P_S(theta)}.
```

Then

```text
I(x) <= #{S in Sigma : x notin S},                 (RKSQAVOID)
```

with equality for every `x` when `Theta_S` is the full `e/2` palette for
every `S`.  Also

```text
sum_x I(x) = (2(q-1)/e) * sum_{S in Sigma} |Theta_S|.   (RKSQAVOIDMASS)
```

Thus an arbitrary global selection of fixed-basis square-map packets is
controlled by the support-avoidance profile of the selected support image.
...
There is an exact collision form of the same energy.  Let

```text
C_1(Sigma)=#{ {S,T} subset Sigma : S != T, |S cap T|=1 }.
```

Then

```text
sum_{x in U} d_Sigma(x)^2 = 2m + 2C_1(Sigma).       (RKSQSTAR)
```

Indeed, the left side counts ordered pairs `(S,T)` with a chosen common
endpoint.  The diagonal pairs contribute `2m`, and two distinct two-point
supports have at most one common endpoint.  Thus the excess term in
(RKSQDEG2) is exactly the number of endpoint-sharing support pairs.  High
full-palette incidence energy is therefore not a mysterious character-sum
...
The same star alternative controls arbitrary partial palettes.  With `I(x)` as
in (RKSQAVOID), let `m=|Sigma|` and
`Delta=max_x d_Sigma(x)`.  Since `0<=I(x)<=I_full(x)` pointwise,

```text
sum_x I(x)^2 <= sum_x I_full(x)^2
             <= (q-3)m^2 + 2mDelta.              (RKSQPARTSTAR)
```

Equivalently, if an arbitrary selected packet family satisfies

```text
sum_x I(x)^2 > (q-3)m^2 + 2mD,
```

then some endpoint lies in more than `D` selected supports.  Thus even after
the global row-basis/core image chooses only some square-coset classes, local
...
For full square-coset palettes, the residual coverage is exact and sharper
than Cauchy.  Since `I_full(x)=m-d_Sigma(x)`,

```text
{x : I_full(x)>0} = P^1(F_q) minus intersection_{S in Sigma} S,
B_full = q+1 - |intersection_{S in Sigma} S|.     (RKSQFULLCOVER)
```

The common intersection has size `2` when `m=1`, size `1` precisely for a
nontrivial common-endpoint star, and size `0` otherwise.  Therefore, after the
support-star pruning above, if `m_ap>D`, then no point can lie in every
residual support because `max_x d_{Sigma_ap}(x)<=D`; hence the full-palette
residual covers the whole projective slope line:

```text
m_ap>D and full palettes  =>  B_full,ap=q+1.       (RKSQFULLPRUNE)
```

Thus a pruned residual that still has small slope support must either have at
most `D` remaining supports or use genuinely partial square-coset palettes.
...
Combining the density and defect ledgers gives the local support-budget
alternative used by the global residue-line packing program.  Fix a slope
budget `R` with `0<=R<=q+1`.  After pruning endpoint stars with degree cap
`D`, either
...
or the residual simultaneously satisfies

```text
(2(q-1)/e)^2 K_ap^2
  <= R((q-3)m_ap^2 + 2m_apD),                    (RKSQSUPPBUDGET1)

(m_ap-D)(q+1-R)
  <= (2(q-1)/e) C_ap.                            (RKSQSUPPBUDGET2)
```

Here `K_ap` is the selected residual class count and `C_ap` is the missing
residual class count.  Thus a large pruned residual with small slope support
must pay twice: selected classes must be density-limited, and the uncovered
projective slopes must be accounted for by missing square-coset classes.  The
...
The certificate also has a normalized feasibility window.  Put `h=e/2` and

```text
alpha=K/(hm),        gamma=C/(hm)=1-alpha.
```

For a `(D,R)` sparse residual certificate, (RKSQSPCERT1) and
(RKSQSPCERT2) imply

```text
alpha^2 <= R((q-3)m^2+2mD)/((q-1)^2 m^2),        (RKSQSPWINDOW1)

gamma >= (m-D)(q+1-R)/((q-1)m).                  (RKSQSPWINDOW2)
```

Equivalently, any such certificate must satisfy

```text
alpha <= min(
  sqrt(R((q-3)m^2+2mD))/((q-1)m),
  1 - (m-D)(q+1-R)/((q-1)m)
).                                                (RKSQSPWINDOW)
```

Thus a global row-basis/core-image lower bound `alpha>alpha_0` excludes all
certificates in a parameter range as soon as `alpha_0` is above this window
...
The support budget can also be optimized subject to a chosen template
complexity.  Assume `q>=3` and `D>=1`.  For an endpoint-footprint allowance
`S>=4D-1`, put

```text
L_S = floor((S+1)/(2D)).                         (RKSQLS)
```

Then `L_S>=2` and `2L_S D-1<=S`.  Moreover `R_dens(a0,L)` is nondecreasing in
`L>=2`: the denominator `q-3+2/L` in `R_sel` decreases with `L`, while
`L/(L-1)` in `R_miss` also decreases with `L`.  Hence `L_S` gives the largest
support budget compatible with endpoint footprint at most `S`.  Under
`alpha_ap>=a0`, every sparse certificate with

```text
B_ap<=R<R_dens(a0,L_S)
```

is either impossible or has endpoint footprint at most `S`.  The number of
remaining residual palette templates is bounded by

```text
sum_{s=0}^{min(q+1,S)} binom(q+1,s) 2^{h binom(s,2)}.       (RKSQTRADE)
```

Thus the density problem can be parameterized by a single reviewable tradeoff:
