# M1 Cycle120 Self-Contained Counterexample Certificate

Status: EXPERIMENTAL / SELF-CONTAINED-CERTIFICATE / ABF-DEFINITION-VERIFIED.
...
This certificate removes both dependencies:
...
2. **The 26-billion census is not load-bearing.** Since
   ...
   This certificate exhibits and fully machine-checks **`M = 40` explicit distinct
...
The whole certificate was independently reproduced by an adversarial re-implementation
...
Matching this against the certificate, term by term:
...
support-wise vs. a single common support — resolves in the certificate's favour: ABF is
...
This certificate's row-specific bound `delta*_C < 125/256 = 250/512` lies `5/512` *inside* that
...
rejected archive. The model constants were recovered from the public fork
`DannyExperiments/rs-mca-prz-fork` (branch `cycle58-5p5-audit`); only the small published
constants were used, none of the generated census machinery.
