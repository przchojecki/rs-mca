...
We claim that this forces `G` to be identically zero.  For `r=0` this is
immediate.  For `r>=1`, put `E=D\{alpha}` and write

```text
F(x_1,...,x_r)=G(prod_i (X-x_i)).
```

Every coefficient of `prod_i(X-x_i)` is multilinear in the variables `x_i`,
so, since `G` has degree at most two in the coefficients, the polynomial `F`
has degree at most two in each individual variable.  It vanishes on all
ordered `r`-tuples of distinct elements of `E`.

Because `|E|>=r+2`, fixing any `r-1` distinct elements of `E` leaves at least
three choices for the last variable.  The resulting one-variable polynomial of
degree at most two has at least three roots, and is therefore identically
zero.  Repeating this argument one coordinate at a time shows that
`F` is the zero polynomial.

The elementary-symmetric map from ordered roots to monic degree-`r`
coefficients is dominant over an algebraic closure, so `G` is identically zero
on the monic degree-`r` coefficient slice.  Therefore `P_{rho,beta}` vanishes
on every monic degree-`m` locator divisible by `X-alpha`, which is exactly the
hyperplane `L(alpha)=0`.
