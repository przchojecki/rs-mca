...
**Verifier:** `experimental/scripts/verify_popular_fiber_probe_confound.py` (zero-arg, stdlib-only, `<90s`, `--tamper`). Five gates, independent reimplementation (imports no campaign module, reads no campaign JSON): (1) the image-collapse lemma at every probe order; (2) the dyadic hierarchy + exact `Gamma_2/3/4` at rows A, D; (3) the confound scan (0/152 coprime vs 24/132 gcd>1); (4) row E's `C(6,2)=15` coset-union identity, member-by-member, over the full `C(24,8)=735471` enumeration; (5) the twist-stabilizer formula and the antipodal last-eyes mechanism. Exact `Fraction`/`int` throughout; floats only in human-readable log lines.
...
as reduced `Fraction`s (gates 2, 4):
...
Sec. 1 (values `Gamma_r^{1/(r-1)}`, `R = R_max`):
...
- **Last-eyes finding (a caveat the JSONs carry that the campaign's one-line summary compresses).**
  ...
  exactly **5 of 7 members are antipodal** (`M = -M`), and *antipodal `<=>` `p_5(M) = 0` exactly*
  (gate 5): an antipodal support is a union of `{x,-x} = x mu_2` pairs, a `mu_2`-quotient object whose
  every odd power sum vanishes. So the coprime probe `p_5` collapses to a near-singleton -- a
