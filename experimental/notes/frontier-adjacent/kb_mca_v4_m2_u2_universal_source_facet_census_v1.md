...
impact: GIVES_EXACT_TRIVIAL_SOURCE_INTERFACES_AND_REDUCES_ORDER_TWO_TO_ONE_COORDINATE_ROUTE_PLUS_FOUR_DIAGONAL_MIXING_ROWS_WITH_C6_AT_ONE_QUOTIENT_SYSTEM_THE_202_ROW_EMPTY_AND_SATURATED_112_AT_123_OR_12_EDGE_ORBITS_WITH_ONE_EXPLICIT_SOURCE_LINE_QUOTIENT_QUADRATIC_FOUR_ODD_PART_LABEL_TESTS_NO_RAMIFIED_COEFFICIENT_ESCAPE_AT_MOST_EIGHT_SOURCE_DECK_PAIRS_PER_PACKET_ONE_DEGREE_EIGHT_Q_SLICE_PREFILTER_AND_NEGATIVE_AB_OR_ABC_FACTOR_LOCI
...
fixed-point-freeness, and `tau(J_1) subset I`. Since the coefficient matrix
has rank four, negative candidates exist exactly on `AB=0` in the first
template and `ABC=0` in the second. These loci are retained for (9.24); no
factor `A,B,C` is divided out.
